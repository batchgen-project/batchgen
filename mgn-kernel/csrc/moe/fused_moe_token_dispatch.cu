...
#include <vector>
#include <torch/all.h>
#include <cub/cub.cuh>  // CUB for efficient scan operations
...
std::vector<torch::Tensor> fused_moe_token_dispatch_cuda(
    ...
    // Step 2: OPTIMIZED - Use CUB for exclusive scan to avoid cat() and get total in one pass
    
    // Allocate output tensor for offsets (size = num_local_experts + 1)
    auto expert_offsets = torch::empty(
        {num_local_experts + 1},
        torch::TensorOptions().dtype(torch::kInt32).device(global_x.device()));
    
    // Allocate device memory for total count
    auto total_tokens_device = torch::empty(
        {1}, torch::TensorOptions().dtype(torch::kInt32).device(global_x.device()));
    ...
    // Run exclusive scan: expert_offsets[i] = sum(expert_counts[0:i])
    // This gives us [0, counts[0], counts[0]+counts[1], ...]
    cub::DeviceScan::ExclusiveSum(
        d_temp_storage, temp_storage_bytes,
        expert_counts.data_ptr<int32_t>(),
        expert_offsets.data_ptr<int32_t>(),
        num_local_experts + 1  // +1 to get the total sum at the end
    );

    // The last element now contains the total count
    // Copy just this single element to host (much faster than .item())
    int32_t total_local_tokens;
    cudaMemcpy(&total_local_tokens, 
               expert_offsets.data_ptr<int32_t>() + num_local_experts,
               sizeof(int32_t), 
               cudaMemcpyDeviceToHost);

    // Step 3: Allocate output tensors with exact size
    auto output_x = torch::empty({total_local_tokens, hidden_size},
                                 torch::TensorOptions()
                                     .dtype(global_x.dtype())
                                     .device(global_x.device()));

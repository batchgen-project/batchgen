...
if __name__ == "__main__":
    ...
    for task_name in task_names:
        dataset = datasets.load_dataset(
            "/root/.cache/huggingface/datasets/THUDM___long_bench", split="test"
        )
    ...
    if len(queries) < max_prompts:
        queries = (
            queries * (max_prompts // len(queries))
            + queries[: max_prompts % len(queries)]
        )
    ...
    if len(queries) != args.max_prompts:
        logging.warning(
            f"Number of prompts {len(queries)} is not equal to max_prompts {args.max_prompts}."
        )
	...
	"""
    logging.info(
        f"Connecting to parameter server at {args.server_host}:{args.server_port}"
    )

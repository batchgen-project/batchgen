...
torch::Tensor simple_rmsnorm_forward(
    ...
    if (input.scalar_type() == torch::kFloat32) {
        simple_rmsnorm_kernel<float><<<blocks, threads, 0, stream>>>(
        ...
        simple_rmsnorm_kernel<at::Half><<<blocks, threads, 0, stream>>>(
        ...
        simple_rmsnorm_kernel<at::BFloat16><<<blocks, threads, 0, stream>>>(

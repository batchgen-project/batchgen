...
std::vector<torch::Tensor> expert_bincount_cuda(
    ...
    // Get current CUDA stream for graph compatibility
    ...
    expert_bincount_kernel<<<blocks, threads, 0, stream>>>(
    ...
    if (experts_per_rank <= 512) {
        ...
        compact_active_experts_single_block_kernel<<<1, 256, 0, stream>>>(
            expert_counts.data_ptr<int32_t>(),
        ...
        compact_active_experts_parallel_kernel<<<compact_blocks, threads, 0, stream>>>(
            expert_counts.data_ptr<int32_t>(),
            activated_group_idx.data_ptr<int32_t>(),
            group_size.data_ptr<int32_t>(),
            group_start_indices.data_ptr<int32_t>(),
        ...
        compute_start_indices_kernel<<<1, 256, 0, stream>>>(
            group_size.data_ptr<int32_t>(),
            group_start_indices.data_ptr<int32_t>(),
            num_active_experts.data_ptr<int32_t>(),
            static_cast<int32_t>(experts_per_rank)
...
// -----------------------------------------------------------------
// compact_expert_data: takes expert_counts and finds active expert
// indices and metadata (replaces torch.nonzero() + torch.count_nonzero())
// -----------------------------------------------------------------
std::vector<torch::Tensor> compact_expert_data_cuda(
    ...
    if (experts_per_rank <= 512) {
        const int threads = 256;
        compact_active_experts_single_block_kernel<<<1, threads, 0, stream>>>(
            expert_counts.data_ptr<int32_t>(),
            activated_group_idx.data_ptr<int32_t>(),
            group_size.data_ptr<int32_t>(),
            group_start_indices.data_ptr<int32_t>(),

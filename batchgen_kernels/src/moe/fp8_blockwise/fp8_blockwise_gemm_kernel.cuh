...
#pragma unroll
  ...
  int cu_seqlen = cu_seqlens_ptr[igroup];
  if (idx == 0 &&
      (num_seq < 0 || cu_seqlen < 0 || (cu_seqlen % kTileM) != 0 ||
       static_cast<int64_t>(cu_seqlen) + num_seq > m)) {
    fp8_blockwise_layout_trap();
  }
  ...
  if (idx == 0) {
    auto gX = make_tensor(make_gmem_ptr(x_ptr + static_cast<int64_t>(cu_seqlen) * k),
                          make_shape(num_seq, k), make_stride(k, Int<1>{}));
    update_tma_gtensor<TmaX>(smem_tma_desc[0], gX);
  } else if (idx == 1) {
    auto gY = make_tensor(make_gmem_ptr(y_ptr + static_cast<int64_t>(cu_seqlen) * n),
                          make_shape(n, num_seq), make_stride(Int<1>{}, n));
    update_tma_gtensor<TmaY>(smem_tma_desc[1], gY);
  } else if (idx == 2) {
    const int64_t address = weight_ptrs[igroup];
    if (address <= 0 || (address & 0xf) != 0) {
      fp8_blockwise_layout_trap();
    }
    auto *w_ptr = reinterpret_cast<const Tin *>(address);
    ...
    const int64_t address = w_scale_ptrs[igroup];
    if (address <= 0 || (address & 0xf) != 0) {
      fp8_blockwise_layout_trap();
    }
    auto *ws_ptr = reinterpret_cast<const TS *>(address);
...
__global__ void __launch_bounds__(384, 1)
    fp8_blockwise_grouped_gemm_kernel(const __grid_constant__ TmaB tma_b,
                                      ...
                                      cute::TmaDescriptor *td_xy, const int *seqlens_ptr,

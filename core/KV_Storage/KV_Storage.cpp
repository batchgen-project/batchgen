...
// KV_Storage::KV_Storage(EngineConfig& engine_config, ModelConfig&
// model_config,
...
//         } else {
//             this->logger_->warn("NUMA is not available. Falling back to
//             default memory allocation.");
//         }

//         this->logger_->info("Starting KV_Storage Initialization on device {}
//         using NUMA node {}.",
//                             device_id, numa_node);

//         const auto& storage_size =
//         this->engine_config_.kv_storage_config.storage_byte_size; auto
//         per_layer_storage_size = storage_size /
//         this->model_config_.num_hidden_layers;

//         auto bar = tq::trange(this->model_config_.num_hidden_layers);
//         bar.set_prefix("Allocating Pinned Memory for KV cache");

//         bool is_deepseek = (this->model_config_.model_type.find("deepseek")
//         != std::string::npos);

//         for (auto layer_idx : bar) {
//             auto allocate = [&](void** ptr) {
//                 if (numa_node >= 0) {
//                     *ptr = numa_alloc_onnode(per_layer_storage_size,
//                     numa_node); if (*ptr != nullptr) {
//                         // Touch the pages to commit allocation
//                         volatile char* p = static_cast<volatile char*>(*ptr);
//                         for (size_t i = 0; i < per_layer_storage_size; i +=
//                         4096)
//                             p[i] = 0;

//                         if (cudaHostRegister(*ptr, per_layer_storage_size,
//                         cudaHostRegisterDefault) != cudaSuccess) {
//                             this->logger_->warn("cudaHostRegister failed on
//                             NUMA memory, falling back to cudaHostAlloc.");
...
//                 if (*ptr == nullptr) {
//                     CUDA_CHECK(cudaHostAlloc(ptr, per_layer_storage_size,
//                     cudaHostAllocDefault));
//                 }
//             };
...
//         this->logger_->info("KV Storage Pinned Memory Allocated on NUMA node
//         {}.", numa_node);
//     } catch (const std::exception& e) {
//         this->logger_->error("KV_Storage: Exception during initialization:
//         {}", e.what()); throw;
//     } catch (...) {
//         this->logger_->error("KV_Storage: Unknown exception during
//         initialization"); throw std::runtime_error("KV_Storage: Failed to
//         initialize storage.");
//     }
...
//         int node = -1;
//         get_mempolicy(&node, nullptr, 0, this->k_pinned_memory[0],
//         MPOL_F_NODE | MPOL_F_ADDR); this->logger_->info("Verify device {}
...
KV_Storage::KV_Storage(EngineConfig& engine_config, ModelConfig& model_config,
    ...
    try {
        ...
        // this->engine_config_.kv_storage_config.storage_byte_size; const
        // size_t per_layer_size     = storage_size /
        // this->model_config_.num_hidden_layers; const size_t alignment = 2 *
        // 1024 * 1024;  // 2 MiB

        // bool is_deepseek =
        //     (this->model_config_.model_type.find("deepseek") !=
        //     std::string::npos);

        // auto allocate_numa_wc = [&](void** out_ptr) -> bool {
        //     if (!numa_avail || numa_node < 0) {
        //         // Fallback to regular cudaHostAlloc if NUMA not available
        //         cudaError_t err = cudaHostAlloc(out_ptr, per_layer_size,
        //         cudaHostAllocWriteCombined); if (err == cudaSuccess) {
        //             this->logger_->info("Allocated {} bytes using
        //             cudaHostAlloc (no NUMA)", per_layer_size); return true;
        //         }
        //         // memset
        //         // CUDA_CHECK(cudaMemset(*out_ptr, 9999999.0,
        //         per_layer_size)); return false;
        //     }
        ...
        //         int actual_node = -1;
        //         if (get_mempolicy(&actual_node, nullptr, 0, *out_ptr,
        //         MPOL_F_NODE | MPOL_F_ADDR) == 0) {
        //             if (actual_node == numa_node) {
        //                 this->logger_->info("Successfully allocated {} bytes
        //                 on NUMA node {}",
        //                                   per_layer_size, actual_node);
        //             } else {
        //                 this->logger_->warn("Allocated on NUMA node {}
        //                 instead of target node {}",
        //                                   actual_node, numa_node);

        //                 // Attempt to migrate the memory to correct NUMA node
        //                 unsigned long nodemask = 1UL << numa_node;
        //                 if (mbind(*out_ptr, per_layer_size, MPOL_BIND,
        //                 &nodemask,
        //                         sizeof(nodemask) * 8, MPOL_MF_MOVE |
        //                         MPOL_MF_STRICT) == 0) {
        //                     this->logger_->info("Successfully migrated memory
        //                     to NUMA node {}", numa_node);
        //                 } else {
        //                     this->logger_->warn("Failed to migrate memory to
        //                     NUMA node {}", numa_node);
        //                 }
        //             }
        //         }

        //         // Touch pages to ensure physical allocation and proper NUMA
        //         placement
        ...
        //         for (size_t offset = 0; offset < per_layer_size; offset +=
        //         page_size) {
        //             ptr[offset] = 0;
        //         }
        ...
        //             this->logger_->debug("Memory is properly aligned to {}
        //             bytes", alignment);
        //         } else {
        //             this->logger_->warn("Memory alignment is not optimal
        ...
        //         throw std::runtime_error("Failed to allocate K cache memory
        ...
        //             memory for layer " + std::to_string(layer_idx));
        //         }
        //         this->v_pinned_memory.push_back(v_ptr);
        //     }
        // }
        ...
        // start_time).count(); this->logger_->info("KV Storage Pinned Memory
        // Allocated in {} seconds on NUMA node {}.",
        //                     duration, numa_node);
        ...
        //                     MPOL_F_NODE | MPOL_F_ADDR) == 0) {
        //         this->logger_->info("Final verification: Device {} memory
        //         allocated on NUMA node {} (target was {}).",
        //                           device_id, actual_node, numa_node);
        //     }

        //     // Optional: Verify multiple pages for large allocations
        //     this->verify_numa_placement(this->k_pinned_memory[0],
        //     per_layer_size, numa_node);
        // }
...
void KV_Storage::verify_numa_placement(void* ptr, size_t size,
    ...
    for (size_t i = 0; i < pages_to_check - start_idx &&
                       (num_pages - pages_to_check + start_idx + i) < num_pages;
         i++) {
        pages[start_idx + i] =
            static_cast<char*>(ptr) +
            (num_pages - pages_to_check + start_idx + i) * page_size;
    }

    if (move_pages(0, pages_to_check, pages.data(), nullptr, status.data(),
                   0) == 0) {
        ...
        for (size_t i = 0; i < pages_to_check; i++) {
            if (status[i] == expected_node) {
                ...
                this->logger_->debug("Page {} on NUMA node {} (expected {})", i,
                                     status[i], expected_node);
            }
        }

        double placement_ratio =
            static_cast<double>(correct_placement) / pages_to_check;
        if (placement_ratio >= 0.9) {
            this->logger_->info(
                "NUMA placement verification: {:.1f}% pages on correct node",
                placement_ratio * 100);
        } else {
            this->logger_->warn(
                "NUMA placement verification: Only {:.1f}% pages on correct "
                "node {}",
...
//         if (numa_available() < 0) {
//             this->logger_->warn("NUMA is not available. Falling back to
//             default memory allocation."); numa_node = -1; // Disable NUMA
//             binding
//         }
...
//                 if (k_ptr == nullptr) {
//                     this->logger_->warn("NUMA allocation failed for K cache.
//                     Falling back to cudaHostAlloc.");
//                     CUDA_CHECK(cudaHostAlloc(&k_ptr, per_layer_storage_size,
//                     cudaHostAllocDefault));
//                 } else {
//                     // Register the NUMA memory with CUDA for pinned access
//                     CUDA_CHECK(cudaHostRegister(k_ptr,
...
//                 if (numa_node >= 0) {
//                     // Use NUMA-aware allocation
//                     v_ptr = numa_alloc_onnode(per_layer_storage_size,
//                     numa_node); if (v_ptr == nullptr) {
//                         this->logger_->warn("NUMA allocation failed for V
//                         cache. Falling back to cudaHostAlloc.");
//                         CUDA_CHECK(cudaHostAlloc(&v_ptr,
//                         per_layer_storage_size, cudaHostAllocDefault));
//                     } else {
//                         // Register the NUMA memory with CUDA for pinned
//                         access CUDA_CHECK(cudaHostRegister(v_ptr,
//                         per_layer_storage_size,
//                                                    cudaHostRegisterDefault));
//                     }
//                 } else {
...
//         this->logger_->error("KV_Storage: Unknown exception during
//         initialization"); throw std::runtime_error("KV_Storage: Failed to
//         update K and V to the storage.");
//     }

//     // Add memory allocation verification (optional)
//     if (!this->k_pinned_memory.empty()) {
//         int node;
//         get_mempolicy(&node, NULL, 0, this->k_pinned_memory[0], MPOL_F_NODE |
//         MPOL_F_ADDR); this->logger_->info("Verify device {} memory allocation
//         on NUMA node {}.",
//                            this->engine_config_.basic_config.device, node);
//     }
// };
...
void KV_Storage::Init() {
    try {
        ...
        this->logger_->info(
            "Starting KV_Storage Initialization on device {} targeting NUMA "
            "node {}.",
            device_id, numa_node);

        const size_t storage_size =
            this->engine_config_.kv_storage_config.storage_byte_size;
        const size_t per_layer_size =
            storage_size / this->model_config_.num_hidden_layers;
        ...
        auto allocate_numa_wc = [&](void** out_ptr) -> bool {
            if (!numa_avail || numa_node < 0) {
                ...
                if (err == cudaSuccess) {
                    this->logger_->info(
                        "Allocated {} bytes using cudaHostAlloc (no NUMA)",
                        per_layer_size);
                    return true;
                }
                // memset
            ...
            if (err == cudaSuccess) {
                ...
                if (get_mempolicy(&actual_node, nullptr, 0, *out_ptr,
                    ...
                    if (actual_node == numa_node) {
                        ...
                        this->logger_->warn(
                            "Allocated on NUMA node {} instead of target node "
                            "{}",
                            actual_node, numa_node);
                        ...
                        if (mbind(*out_ptr, per_layer_size, MPOL_BIND,
                            ...
                            this->logger_->info(
                                "Successfully migrated memory to NUMA node {}",
                                numa_node);
                        } else {
                            this->logger_->warn(
                                "Failed to migrate memory to NUMA node {}",
                                numa_node);
                        }
                    }
                }
                ...
                // Touch pages to ensure physical allocation and proper NUMA
                // placement Use first-touch with proper alignment
                volatile char* ptr = static_cast<volatile char*>(*out_ptr);
                size_t page_size = getpagesize();

#pragma omp parallel for if (per_layer_size > 1024 * 1024)
                for (size_t offset = 0; offset < per_layer_size;
                     offset += page_size) {
                    ptr[offset] = 0;
                }
                ...
                if (reinterpret_cast<uintptr_t>(*out_ptr) % alignment == 0) {
                    this->logger_->debug(
                        "Memory is properly aligned to {} bytes", alignment);
                } else {
                    this->logger_->warn(
                        "Memory alignment is not optimal (requested {} bytes)",
                        alignment);
                }
            }
        ...
        for (auto layer_idx : bar) {
            ...
            if (!allocate_numa_wc(&k_ptr)) {
                throw std::runtime_error(
                    "Failed to allocate K cache memory for layer " +
            ...
            if (!is_deepseek) {
                ...
                if (!allocate_numa_wc(&v_ptr)) {
                    throw std::runtime_error(
                        "Failed to allocate V cache memory for layer " +
                        std::to_string(layer_idx));
                }
                this->v_pinned_memory.push_back(v_ptr);
            }
        ...
        auto duration = std::chrono::duration_cast<std::chrono::seconds>(
                            ...
                            .count();
        this->logger_->info(
            "KV Storage Pinned Memory Allocated in {} seconds on NUMA node {}.",
            duration, numa_node);
        ...
        if (!this->k_pinned_memory.empty() && numa_available) {
            ...
            if (get_mempolicy(&actual_node, nullptr, 0,
                              ...
                              MPOL_F_NODE | MPOL_F_ADDR) == 0) {
                this->logger_->info(
                    "Final verification: Device {} memory allocated on NUMA "
                    "node {} (target was {}).",
                    device_id, actual_node, numa_node);
            }
...
//     int64_t layer_idx, std::vector<int64_t> cur_batch, int64_t
//     padding_length)
// {
//     CUDA_CHECK(cudaSetDevice(this->engine_config_.basic_config.device));
//     torch::Device device(torch::kCUDA,
//     this->engine_config_.basic_config.device); auto opt =
//     torch::TensorOptions()
//         .dtype(torch::kFloat32)
//         .device(device)  // Use the device object instead of a raw integer
//         .requires_grad(false);
//     auto k_quantize_scale = torch::ones({cur_batch.size(), padding_length,
//     5}, opt); // TODO:
...
//         }
//         k_quantize_scale.index({i, torch::indexing::Slice(0, scale_size)}) =
//         scale.index({0, torch::indexing::Slice(0, scale_size)});
//         // k_quantize_scale_vec.push_back(scale.index({0,
//         torch::indexing::Slice(0, scale_size)}).unsqueeze(0));
//     }
...
//     {
//     //             this->logger_->debug("k_quantize_scale[{}] has nan values,
//     rank: {}, layer_idx: {}",
//     //                                  i,
...
//     //     auto padding = torch::ones({k_quantize_scale.size(0),
//     padding_length - k_quantize_scale.size(1),k_quantize_scale.size(2)},
//     opt);
...
torch::Tensor KV_Storage::get_k_quantize_scale(
    int64_t layer_idx, const std::vector<int64_t>& cur_batch,
    ...
    if (uniform_sizes && first_size > 0 && first_size <= padding_length) {
        ...
        if (first_size < padding_length) {
            auto result = torch::ones(
                {static_cast<int64_t>(cur_batch.size()), padding_length, 5},
                opt);
            result.index({torch::indexing::Slice(),
                          torch::indexing::Slice(0, first_size)}) = stacked;
            return result;
        } else {
            return stacked;
        }
    } else {
        // Fall back to individual copies (but still more efficient than
        // original)
        auto k_quantize_scale = torch::ones(
            {static_cast<int64_t>(cur_batch.size()), padding_length, 5}, opt);

        for (int64_t i = 0; i < static_cast<int64_t>(tensor_info.size()); i++) {
            const auto& [scale, scale_size] = tensor_info[i];
            if (scale_size > 0) {
                k_quantize_scale
                    .index({i, torch::indexing::Slice(0, scale_size)})
                    .copy_(scale.index(
                        {0, torch::indexing::Slice(0, scale_size)}));
            }
        }
...
//     //             this->logger_->debug("k[{}] has nan values, rank: {},
//     layer_idx: {}",
//     //                                  i,
//     this->engine_config_.basic_config.device,
//     //                                  layer_idx);
...
void KV_Storage::offload(int64_t layer_idx,
    ...
    SAFE_CALL(
        [&]() {
            auto worker =
                std::thread(&KV_Storage::offload_helper_, this, layer_idx,
                            query_global_idx, k, v, attention_mask);
            worker.detach();
            // worker.join();
        },
        this->logger_);
...
void KV_Storage::offload_helper_(int64_t layer_idx,
    ...
    try {
        if (this->model_config_.model_type.find("deepseek") ==
            ...
            //         if(torch::any(torch::isnan(k_quantize_scale[i])).item<bool>())
            //         {
            //             this->logger_->debug("k_quantize_scale[{}] has nan
            //             values, rank: {}, layer_idx: {}",
            //                                  i,
            //                                  this->engine_config_.basic_config.device,
            //                                  layer_idx);
            //         }
            //     }
            //     // throw std::runtime_error("k_quantize_scale has nan
            //     values");
            // }

            // If k_quantize_scale[layer_idx] is torch.empty({0,0}), assign it
            // to k_quantize_scale other wise concatenate it. if
            // (this->k_quantize_scale[layer_idx].numel() == 0) {
            ...
            for (int64_t i = 0; i < query_global_idx.size(); i++) {
...

                {
                        ...
                        k_quantize_scale
                            .index(
                                {i, torch::indexing::Slice(
                                        0, this->k_storage[slot_idx][layer_idx]
                                               .num_tokens)})
                            .clone()
                            .unsqueeze(0);
                    ...
                    // this->logger_->info("k_storage[{}][{}].num_tokens: {},
                    // used_byte_size: {}",
                    //                     slot_idx, layer_idx,
                    //                     this->k_storage[slot_idx][layer_idx].num_tokens,
                    //                     this->k_storage[slot_idx][layer_idx].used_byte_size);
            ...
            for (int64_t i = 0; i < query_global_indices.size(); i++) {
                    ...
                    this->k_storage[slot_idx][layer_idx].used_byte_size;
                {
                    ...
                    this->k_storage[slot_idx][layer_idx].used_byte_size +=
                        k_token_byte_size;
                    this->k_storage[slot_idx][layer_idx].num_tokens += 1;
                    this->k_storage[slot_idx][layer_idx]
                        .quantize_scale = torch::cat(
                        {this->k_storage[slot_idx][layer_idx].quantize_scale,
                         k_quantize_scale.index({i}).unsqueeze(0)},
                        1);
...
std::vector<c10::Float8_e4m3fn*> KV_Storage::get_k_ptrs_fp8(
    int64_t layer_idx, std::vector<int64_t> batch) {
    try {
        std::vector<c10::Float8_e4m3fn*> k_ptrs;
        for (int64_t i = 0; i < static_cast<int64_t>(batch.size()); i++) {
            auto query_idx = batch[i];
    ...
    // Catch PyTorch/CUDA specific exceptions
        ...
        this->logger_->debug(
            "KV_Storage get_k_ptrs_fp8(): CUDA/PyTorch error: {}", e.what());
        throw;
    }
    // Catch CUDA runtime errors
    catch (const cudaError_t& err) {
        this->logger_->debug(
            "KV_Storage get_k_ptrs_fp8(): CUDA runtime error: {}",
            cudaGetErrorString(err));
        throw std::runtime_error(cudaGetErrorString(err));
    }
    // Catch standard C++ exceptions
...
void KV_Storage::clear_kv_storage() {
    ...
    try {
        ...
        if (this->model_config_.model_type.find("deepseek") ==
            ...
            // for(auto layer_idx = 0; layer_idx <
            // this->model_config_.num_hidden_layers; layer_idx++) {
            //     auto options = torch::TensorOptions()
            //         .dtype(torch::kFloat32)
            //         .device(this->engine_config_.basic_config.device_torch)
            //         .requires_grad(false);
            //     torch::Tensor k_quantize_scale = torch::empty({0,0},
            //     options); this->k_quantize_scale.push_back(k_quantize_scale);
            // }
        }
        this->logger_->debug(
...
void KV_Storage::save_compressed_kv() {
    ...
    this->logger_->info(
        "KV_Storage save_compressed_kv(): Saving compressed kv.");
    std::vector<torch::Tensor> compressed_kv;
    for (int64_t slot_idx = 0;
         slot_idx < this->engine_config_.kv_storage_config.num_host_slots;
         slot_idx++) {
        for (int64_t layer_idx = 0;
             layer_idx < this->model_config_.num_hidden_layers; layer_idx++) {
            ...
            auto k_tensor =
                torch::from_blob(
                    ...
                    torch::TensorOptions()
                        .dtype(
                            torch::kBFloat16)  // move dtype into TensorOptions
    ...
    this->logger_->info(
        "KV_Storage save_compressed_kv(): Compressed kv saved.");
}

void KV_Storage::copy_kv_to_worker(std::vector<int64_t> query_global_idx,
                                   int64_t max_length) {
    /*
    ...
    */
    this->logger_->info(
        "KV_Storage copy_kv_to_worker(): Copying KV to worker.");
    CUDA_CHECK(cudaSetDevice(this->engine_config_.basic_config.device));
    try {
        // Init the k_gpu_storage and v_gpu_storage
        int64_t num_slots = query_global_idx.size();
        // int64_t max_length = this->engine_config_.basic_config.padding_length
        // + this->engine_config_.basic_config.max_decoding_length;
        // TODO:
        ...
        this->logger_->info(
            "KV_Storage copy_kv_to_worker(): num_slots: {}, per_layer_size: {}",
            num_slots, per_layer_size);

        this->k_gpu_storage.resize(num_slots);
        for (int64_t slot_idx = 0; slot_idx < num_slots; slot_idx++) {
            this->k_gpu_storage[slot_idx].resize(
                this->model_config_.num_hidden_layers);
            for (int64_t layer_idx = 0;
                 layer_idx < this->model_config_.num_hidden_layers;
                 layer_idx++) {
        ...
        this->logger_->info(
            "KV_Storage copy_kv_to_worker(): GPU storage initialized.");
        ...
        for (int64_t idx = 0; idx < query_global_idx.size(); idx++) {
            int64_t cpu_slot_idx = -1;
            {
                std::lock_guard<std::mutex> lock(this->mutex_);
                cpu_slot_idx =
                    this->query_idx_to_slot_idx_map[query_global_idx[idx]];
            }
            for (int64_t layer_idx = 0;
                 layer_idx < this->model_config_.num_hidden_layers;
                 layer_idx++) {
                auto host_k_ptr =
                    this->k_storage[cpu_slot_idx][layer_idx].start_ptr;
                // auto host_v_ptr =
                // this->v_storage[slot_idx][layer_idx].start_ptr;
                auto k_size =
                    this->k_storage[cpu_slot_idx][layer_idx].used_byte_size;
                // auto v_size =
                // this->v_storage[slot_idx][layer_idx].used_byte_size; Copy the
                // k and v to the worker
                CUDA_CHECK(cudaMemcpyAsync(
                    this->k_gpu_storage[idx][layer_idx].start_ptr, host_k_ptr,
                    k_size, cudaMemcpyHostToDevice, this->stream_));
                // CUDA_CHECK(cudaMemcpyAsync(
                //     this->v_gpu_storage[slot_idx][layer_idx].start_ptr,
                //     host_v_ptr, v_size, cudaMemcpyHostToDevice,
                //     this->stream_));
                // Update the used byte size and num tokens in the gpu storage
                this->k_gpu_storage[idx][layer_idx].used_byte_size = k_size;
                this->k_gpu_storage[idx][layer_idx].num_tokens =
                    this->k_storage[cpu_slot_idx][layer_idx].num_tokens;
                this->k_gpu_storage[idx][layer_idx].quantize_scale =
                    this->k_storage[cpu_slot_idx][layer_idx]
                        .quantize_scale.clone();
                // this->v_gpu_storage[slot_idx][layer_idx].used_byte_size =
                // v_size; this->v_gpu_storage[slot_idx][layer_idx].num_tokens =
                // this->v_storage[slot_idx][layer_idx].num_tokens;
            }
        }
        // Synchronize the stream
        CUDA_CHECK(cudaStreamSynchronize(this->stream_));
        this->logger_->info(
            "KV_Storage copy_kv_to_worker(): KV copied to worker.");
    }
    // Catch PyTorch/CUDA specific exceptions
    catch (const c10::Error& e) {
        this->logger_->debug("KV_Storage get_v_ptrs(): CUDA/PyTorch error: {}",
                             e.what());
        throw std::runtime_error("KV_Storage: Failed to copy KV to worker.");
    }
    // Catch CUDA runtime errors
    catch (const cudaError_t& err) {
        this->logger_->debug("KV_Storage get_v_ptrs(): CUDA runtime error: {}",
...
torch::Tensor KV_Storage::get_k(int64_t layer_idx,
    ...
    try {
        ...
        int64_t seq_byte_size =
            tensor_shape[1] * tensor_shape[2] * k_tensor.element_size();
        ...
        for (int64_t i = 0; i < static_cast<int64_t>(cur_batch.size()); i++) {
            ...
            // this->engine_config_.basic_config.kv_dtype_torch* k_ptr =
            // nullptr;
            ...
            CUDA_CHECK(cudaMemcpyAsync(dst_ptr, src_ptr, seq_byte_size,
...
void KV_Storage::gpu_kv_update(int64_t layer_idx,
                               std::vector<int64_t> query_global_indices,
                               torch::Tensor k, torch::Tensor v,
                               torch::Tensor k_quantize_scale) {
...
void KV_Storage::gpu_kv_update_func(int64_t layer_idx,
                                    std::vector<int64_t> query_global_indices,
                                    torch::Tensor k, torch::Tensor v,
                                    torch::Tensor k_quantize_scale) {
    try {
        CUDA_CHECK(cudaSetDevice(this->engine_config_.basic_config.device));
        k = k.contiguous();
        int64_t k_token_byte_size = k.size(1) * k.size(2) * k.element_size();
        this->logger_->debug("k_token_byte_size: {}", k_token_byte_size);
        for (int64_t i = 0; i < query_global_indices.size(); i++) {
            auto query_idx = query_global_indices[i];
                ...
                this->k_gpu_storage[slot_idx][layer_idx].used_byte_size;
            {
                std::lock_guard<std::mutex> lock(
                    this->per_element_mutex_[slot_idx * this->model_config_
                                             ...
                                             layer_idx]);
                CUDA_CHECK(
                    cudaMemcpyAsync(dst_k_ptr, src_ptr, k_token_byte_size,
                                    cudaMemcpyDeviceToDevice, this->stream_));
                this->k_gpu_storage[slot_idx][layer_idx].used_byte_size +=
                    k_token_byte_size;
                this->k_gpu_storage[slot_idx][layer_idx].num_tokens += 1;
                this->k_gpu_storage[slot_idx][layer_idx]
                    .quantize_scale = torch::cat(
    ...
    // Catch standard C++ exceptions
        ...
        this->logger_->debug(
            ...
            e.what());
        throw std::runtime_error(
            "KV_Storage gpu_kv_update(): Failed to update K and V to the "
            "storage.");
    }
    // Catch any other unexpected errors
    catch (...) {
...
void KV_Storage::clear_kv_gpu_storage() {
    ...
    this->logger_->debug(
        "KV_Storage clear_kv_gpu_storage(): Clearing KV GPU storage.");
    CUDA_CHECK(cudaSetDevice(this->engine_config_.basic_config.device));
    for (int64_t layer_idx = 0; layer_idx < this->k_gpu_memory.size();
         layer_idx++) {
        CUDA_CHECK(cudaFree(this->k_gpu_memory[layer_idx]));
    };
    this->logger_->info(
        "KV_Storage clear_kv_gpu_storage(): KV GPU storage cleared.");
}

// std::vector<torch::Tensor>
// KV_Storage::get_past_key_states(std::vector<int64_t> query_global_indices,
// int64_t max_seq_len) {
//     // Currently only support deepseek models. TODO:
//     // From blob in cpu to get the torch tensor representation of the kv
//     cache in kv_storage.
//     // The slot for the same layer should be concatenated together.
//     // Copy the tensor to GPU device.
//     // Return a vector of torch::Tensor, each tensor is the past key states
//     for a layer.
//     // The result tensor shape is [batch_size, max_seq_len, 576]

//     auto start_time = std::chrono::high_resolution_clock::now();
//     this->logger_->debug("KV_Storage get_past_key_states(): Getting past key
//     states."); std::vector<torch::Tensor> past_key_states; for (int64_t
//     layer_idx = 0; layer_idx < this->model_config_.num_hidden_layers;
//     layer_idx++) {
//         // std::vector<torch::Tensor> k_tensors;
//         torch::Tensor k_tensor = torch::zeros(
//             {static_cast<int64_t>(query_global_indices.size()), max_seq_len,
//             576}, torch::TensorOptions()
//                 .dtype(this->engine_config_.basic_config.kv_dtype_torch)
//                 .device(torch::kCPU)
//                 .requires_grad(false));
//         // Fill the tensor with data from k_storage
//         for (int64_t i = 0; i <
//         static_cast<int64_t>(query_global_indices.size()); i++) {
//             auto query_idx = query_global_indices[i];
//             int64_t slot_idx = -1;
//             {
//                 std::lock_guard<std::mutex> lock(this->mutex_);
...
//             }
//             // Copy the data from k_ptr to k_tensor
//             auto dst_ptr = k_tensor.data_ptr() + i * max_seq_len * 576 *
//             sizeof(this->engine_config_.basic_config.kv_dtype_torch);
//             CUDA_CHECK(cudaMemcpyAsync(
//                 dst_ptr, k_ptr, max_seq_len * 576 *
//                 sizeof(this->engine_config_.basic_config.kv_dtype_torch),
...
//     }
//     this->logger_->debug("KV_Storage get_past_key_states(): Past key states
//     retrieved."); auto end_time = std::chrono::high_resolution_clock::now();
//     auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time
//     -
//         start_time);
//     this->logger_->info("KV_Storage get_past_key_states(): Time taken to get
//     past key states: {} seconds", duration.count()); return past_key_states;
// }

std::vector<torch::Tensor> KV_Storage::get_past_key_states(
    std::vector<int64_t> query_global_indices, int64_t max_seq_len) {
    // Currently only support deepseek models. TODO:
    // From blob in cpu to get the torch tensor representation of the kv cache
    // in kv_storage. The slot for the same layer should be concatenated
    // together. Copy the tensor to GPU device. Return a vector of
    // torch::Tensor, each tensor is the past key states for a layer. The result
    // tensor shape is [batch_size, max_seq_len, 576]

    auto start_time = std::chrono::high_resolution_clock::now();
    this->logger_->debug(
        "KV_Storage get_past_key_states(): Getting past key states.");
    ...
    this->logger_->debug(
        "KV_Storage get_past_key_states(): Past key states retrieved.");
    auto end_time = std::chrono::high_resolution_clock::now();
    auto duration =
        std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time);
    this->logger_->info(
        "KV_Storage get_past_key_states(): Time taken to get past key states: "
        "{} seconds",
        duration.count());
    return past_key_states;
}

std::vector<torch::Tensor> KV_Storage::get_kv_scale(
    ...
    for (int64_t layer_idx = 0;
        ...
        for (int64_t i = 0;
            ...
            // If the scale's seq_len is less than the seq_len, pad it with
            // zeros.
            ...
            if (scale_tensor.size(1) < seq_len) {
                ...
                torch::Tensor padded_scale_tensor = torch::zeros(
                    {scale_tensor.size(0), seq_len - scale_tensor.size(1),
                     scale_tensor.size(2)},
                    options);
                scale_tensor =
                    torch::cat({scale_tensor, padded_scale_tensor}, 1);
            }
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time);
    this->logger_->info(
        "KV_Storage get_kv_scale(): Time taken to get kv scale: {} seconds",
        duration.count());
    return kv_scale;
}

// std::vector<torch::Tensor> KV_Storage::get_kv_scale(std::vector<int64_t>
// query_global_indices, int64_t seq_len) {
//     // Return the quantization scale for k
//     this->logger_->debug("KV_Storage get_kv_scale(): Getting kv scale.");
...
//     for (int64_t layer_idx = 0; layer_idx <
//     this->model_config_.num_hidden_layers; layer_idx++) {
//         // Get the first tensor to determine the shape for num_blocks
//         dimension int64_t query_idx = query_global_indices[0]; int64_t
//         slot_idx = -1;
//         {
//             std::lock_guard<std::mutex> lock(this->mutex_);
//             slot_idx = this->query_idx_to_slot_idx_map[query_idx];
//         }
//         torch::Tensor first_scale =
//         this->k_storage[slot_idx][layer_idx].quantize_scale; int64_t
//         num_blocks = first_scale.size(2);

//         // Pre-allocate tensor with shape [bsz, seq_len, num_blocks]
//         torch::Tensor k_scale_tensor = torch::zeros({bsz, seq_len,
//         num_blocks}, options);
...
//             torch::Tensor scale_tensor =
//             this->k_storage[slot_idx][layer_idx].quantize_scale; int64_t
//             actual_seq_len = scale_tensor.size(1);

//             // Copy the existing data into the pre-allocated tensor
//             // The remaining positions stay as zeros (padding)
//             if (actual_seq_len > 0) {
//                 k_scale_tensor[i].slice(0, 0, std::min(actual_seq_len,
//                 seq_len)) =
//                     scale_tensor[0].slice(0, 0, std::min(actual_seq_len,
//                     seq_len));
//             }
//         }

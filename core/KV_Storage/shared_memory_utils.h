...
class ScopedPthreadMutexLock {
   public:
    explicit ScopedPthreadMutexLock(pthread_mutex_t* mutex) : mutex_(mutex) {
        ...
        if (rc == EOWNERDEAD) {
            const int unlock_rc = pthread_mutex_unlock(mutex_);
            if (unlock_rc != 0) {
                throw std::system_error(unlock_rc, std::generic_category(),
                                        "pthread_mutex_unlock failed after "
                                        "owner death");
            }
            throw std::runtime_error(
                "shared-memory mutex owner died; state may be inconsistent");
        } else if (rc != 0) {

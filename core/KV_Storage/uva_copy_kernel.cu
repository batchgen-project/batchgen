...
//
__device__ void UvaCopyBytes(uint8_t* src, uint8_t* dst, size_t num_bytes) {
    ...
    if (can_vectorize) {
        const size_t num_words = num_bytes / sizeof(uint4);
        ...
        for (size_t i = vector_bytes + threadIdx.x; i < num_bytes;
             i += blockDim.x) {
            dst[i] = src[i];
        }
        return;
    ...
    for (size_t i = threadIdx.x; i < num_bytes; i += blockDim.x) {
        dst[i] = src[i];
    }
}
...
__global__ void UvaPageCopyKernel(uint8_t** src_ptrs, uint8_t** dst_ptrs,
    ...
    UvaCopyBytes(src_ptrs[page_idx], dst_ptrs[page_idx], page_size_bytes);
}

__global__ void UvaVariableByteCopyKernel(uint8_t** src_ptrs,
                                          uint8_t** dst_ptrs,
                                          const size_t* copy_sizes_bytes,
                                          int num_copies) {
    const int copy_idx = blockIdx.x;
    if (copy_idx >= num_copies) {
        return;
    }
    UvaCopyBytes(src_ptrs[copy_idx], dst_ptrs[copy_idx],
                 copy_sizes_bytes[copy_idx]);
}
...
void LaunchUvaVariableByteCopyKernel(uint8_t** src_ptrs, uint8_t** dst_ptrs,
    ...
    if (src_ptrs == nullptr || dst_ptrs == nullptr ||
        copy_sizes_bytes == nullptr || num_copies <= 0) {
        return;
    }
    constexpr int kThreadsPerBlock = 256;
    const dim3 grid(static_cast<unsigned int>(num_copies));
    const dim3 block(kThreadsPerBlock);
    UvaVariableByteCopyKernel<<<grid, block, 0, stream>>>(
        src_ptrs, dst_ptrs, copy_sizes_bytes, num_copies);
    CUDA_CHECK(cudaGetLastError());
}

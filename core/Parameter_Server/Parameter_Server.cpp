    ...
    std::unordered_map<std::string,std::unordered_map<std::string, std::string>>& state_dict_name_map)
{
    ...
    */
    ...
    if (dir != nullptr) {
        ...
        throw std::runtime_error("Failed to open directory: " + model_weights_path);
    };
    ...
    for(const auto& json_path : bar) {
        ...
        try {
            ...
            throw std::runtime_error("Failed to parse JSON file: " + json_path + " - " + e.what());
        }

        // // Step 3: Load the corresponding .bin file to weight_ptr + global_offset
        // std::string bin_file_path = model_weights_path + "/" + metadata.file_name;
        // std::ifstream weight_bin_file(bin_file_path, std::ios::binary);
        // if (!weight_bin_file) {
        //     throw std::runtime_error("Failed to open binary file: " + bin_file_path);
        // };
        ...
        // if (fd == -1) {
        //     throw std::runtime_error("Failed to open file: " + bin_file_path);
        // }
        ...
        // // Memory map the file
        // void* mapped_data = mmap(nullptr, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
        ...
        //     std::memcpy(dest_ptr + offset, static_cast<char*>(mapped_data) + offset, to_copy);
        //     offset += to_copy;
        //     remaining -= to_copy;
        // }
        ...
        // if (fd == -1) {
        //     throw std::runtime_error("Failed to open file with direct I/O: " + bin_file_path);
        // }

        // // For O_DIRECT, buffer must be aligned to sector size (usually 512 bytes or 4KB)
        ...
        //     size_t aligned_read_size = (to_read + alignment - 1) & ~(alignment - 1);
        ...
        //     size_t actual_copy = std::min(to_read, static_cast<size_t>(bytes_read));
        //     std::memcpy(dest_ptr + total_read, aligned_buffer, actual_copy);
        ...
        bool dest_aligned = (reinterpret_cast<uintptr_t>(dest_ptr) % alignment) == 0;
        ...
        if (!dest_aligned) {
            throw std::runtime_error("Destination memory not aligned for direct I/O. "
                                "Global offset alignment failed.");
        }

        // Calculate aligned and tail portions
        size_t aligned_size = (metadata.total_byte_size / alignment) * alignment; // Round down
        ...
        if (aligned_size > 0) {
            int fd = open(bin_file_path.c_str(), O_RDONLY | O_DIRECT | O_NOATIME);
            if (fd == -1) {
                throw std::runtime_error("Failed to open file with direct I/O: " + bin_file_path);
            }
            ...
            while (remaining > 0) {
                size_t to_read = std::min(remaining, buffer_size);
                // Ensure read size is aligned (should already be, but just in case)
                to_read = (to_read / alignment) * alignment;
                
                ssize_t bytes_read = pread(fd, dest_ptr + file_offset, to_read, file_offset);
                if (bytes_read <= 0) {
                    close(fd);
                    throw std::runtime_error("Failed to read aligned portion from file");
                }
            ...
            close(fd);
            this->logger->info("Direct I/O completed for {} bytes", aligned_size);
        }
        ...
        if (tail_size > 0) {
            ...
            if (!tail_file) {
                throw std::runtime_error("Failed to open file for tail read: " + bin_file_path);
            }
            ...
            if (!tail_file || tail_file.gcount() != static_cast<std::streamsize>(tail_size)) {
                throw std::runtime_error("Failed to read tail portion (" + 
                                        std::to_string(tail_size) + " bytes)");
            }

            tail_file.close();
            this->logger->info("Regular I/O completed for tail {} bytes", tail_size);
        }
        ...
        this->logger->info("Load file {} in {:.2f} seconds ({}, {}/{} bytes aligned/total)", 
                        metadata.file_name, elapsed.count(), method_info, 
                        aligned_size, metadata.total_byte_size);

        // Step 4: Update this->module_weights_storage_ and this->skeleton_state_dict_
        ...
        for (const auto& [tensor_name, tensor_info] : metadata.state_dict) {
            ...
            if (state_dict_name_map.find(tensor_name) != state_dict_name_map.end()) {
                // If it exists, update module_weights_storage_
                std::vector<int64_t> tensor_shape = tensor_info.shape;
                int64_t tensor_global_offset = global_offset + tensor_info.offset;
                ...
                // Create a tensor from the weight_ptr + global_offset + tensor_info.offset
                ...
                torch::Tensor tensor =
                    torch::from_blob(static_cast<char*>(weight_ptr) + global_offset + tensor_info.offset,
        ...
        this->logger->info("Updated module_weights_storage_ and skeleton_state_dict_ in {:.2f} seconds",
                           elapsed.count());
        global_offset += metadata.total_byte_size;
        global_offset = (global_offset + 4095) & ~4095; // Round up to 4KB boundary
    }
};
...
//         auto start_time = std::chrono::high_resolution_clock::now();
//         py::object tmp_state_dict_py = load(file_path, py::arg("weights_only") = true);
...
//                            tmp_state_dict.size(), elapsed.count(), file_path);
...
//         end_time = std::chrono::high_resolution_clock::now();
//         elapsed = end_time - start_time;
//         this->logger->info("Copied {} tensors to pinned memory in {:.2f} seconds from {}",
//                             tmp_state_dict.size(), elapsed.count(), file_path);

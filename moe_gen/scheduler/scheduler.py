...
class Scheduler:
    ...
    def generate_config(self) -> EngineConfig:
        ...
        attn_decoding_micro_batch_size = 2 ** (
            attn_decoding_micro_batch_size.bit_length() - 1
        )
        est_kv_cp_t_per_micro_batch = (
            attn_decoding_micro_batch_size
        ...
        non_static_memory_usage = (
            k_buffer_size + model_skeleton_size + cuda_page_table_default_size
        )
        available_memory_for_expert_cache = (
            available_gpu_mem - non_static_memory_usage
        )
        num_local_expert_per_layer = min(
            EXPERT_PER_RANK, int(available_memory_for_expert_cache // 2.4)
        )  # Each expert cache is around 2.4GB
        num_decoding_module_buffer_routed_expert = (
            EXPERT_PER_RANK - num_local_expert_per_layer + 2
        )
        ...
        self.config.Module_Batching_Config.attn_decoding_micro_batch_size = (
            attn_decoding_micro_batch_size
        )
        ...
        self.config.EP_Config.num_local_expert_per_layer = (
        ...
        if self.Max_Prompt_Length > 14000:
            ...
            self.config.Module_Batching_Config.attn_prefill_micro_batch_size = int(
                self.config.Module_Batching_Config.attn_prefill_micro_batch_size
                / factor
            )
            self.config.Module_Batching_Config.MoE_prefill_micro_batch_size = int(
                self.config.Module_Batching_Config.MoE_prefill_micro_batch_size
                / factor
            )
    ...
    def _set_default_configs(self):
        ...
        self.config.Basic_Config.module_types = [
            "attn",
            "routed_expert",
            "shared_expert",
        ]
        ...
        self.config.GPU_Buffer_Config.num_k_buffer = (
            None  # This will be set later based on available memory
        )
        self.config.GPU_Buffer_Config.num_v_buffer = (
            0  # No value buffer for now
        )

...
class DeepseekV3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # check if nan and inf in variance:
        if torch.isnan(variance).any() or torch.isinf(variance).any():
            print("variance has nan or inf", flush=True)
        hidden_states = hidden_states * torch.rsqrt(
            variance + self.variance_epsilon
        )
        if torch.isnan(hidden_states).any() or torch.isinf(hidden_states).any():
            print("hidden_states has nan or inf", flush=True)

        if (
            torch.isnan(self.weight.data).any()
            or torch.isinf(self.weight.data).any()
        ):
            print("weight has nan or inf", flush=True)

        # end_time.record()
        # torch.cuda.synchronize(7)
        # print("DeepseekV3RMSNorm time: ", start_time.elapsed_time(end_time), flush=True)
...
class DeepseekV3MoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        flat_topk_idx = topk_idx.view(-1)  # noqa F841
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not output_attentions:
            attn_weights = None
        # Convert attn_output[0] to list and print.

...
class DeepseekV3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        return F.rms_norm(
            hidden_states, (self.dim,), self.weight, self.variance_epsilon
        )
...
class DeepseekV3RotaryEmbedding(nn.Module):
    def __init__(
        self, dim, max_position_embeddings=30000, base=10000, device=None
    ):
        super().__init__()
        ...
        inv_freq = 1.0 / (
            self.base
            ** (
                torch.arange(0, self.dim, 2, device=self.init_device).float()
                / self.dim
            )
        )
        self.register_buffer("inv_freq", inv_freq, persistent=False)
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        t = torch.arange(
            self.max_seq_len_cached, device=device, dtype=torch.float32
        )
    ...
    def forward(self, x, seq_len=None):
        ...
        if self.max_seq_len_cached is None or seq_len > self.max_seq_len_cached:
            ...
            logger.warning(
                f"Recomputing cos/sin cache for rotary embedding. THIS SHOULD NOT HAPPEN. seq_len: {seq_len}, max_seq_len_cached: {self.max_seq_len_cached}"
            )
            self._set_cos_sin_cache(
...
class DeepseekV3MLP(nn.Module):
    ...
    def fused_fp8_forward(self, x, scale, out=None, offset=None):
        up = fused_fp8_bf16_gemm(
            x,
            self.up_proj.weight.data,
            scale["up_proj.weight_scale_inv"],
            block_size=[64, 16, 64],
        )
        gate = fused_fp8_bf16_gemm(
            x,
            self.gate_proj.weight.data,
            scale["gate_proj.weight_scale_inv"],
            block_size=[64, 16, 64],
        )
        ...
        if out is not None:
            fused_fp8_bf16_gemm(
                ...
                scale["down_proj.weight_scale_inv"],
                out=out,
                offset=offset,
                block_size=[64, 16, 64],
            )
        else:
            res = fused_fp8_bf16_gemm(
                intermediate,
                self.down_proj.weight.data,
                scale["down_proj.weight_scale_inv"],
                block_size=[64, 16, 32],
    ...
    def deepgemm_forward(self, x, scale):
        up = w8a16_gemm(
            self.up_proj.weight.data, scale["up_proj.weight_scale_inv"], x
        )
        gate = w8a16_gemm(
            self.gate_proj.weight.data, scale["gate_proj.weight_scale_inv"], x
        )
        intermediate = self.act_fn(gate) * up
        return w8a16_gemm(
            self.down_proj.weight.data,
...
class DeepseekV3MoE_Decoding(nn.Module):
    def __init__(self, config):
        ...
        self.device = torch.device(
            "cuda", self.rank % torch.cuda.device_count()
        )
        # They are the same size every call, so we keep them as buffers
        self.register_buffer(
            "send_counts_buf", torch.zeros(self.world_size, dtype=torch.int64)
        )
        self.register_buffer(
            "recv_counts_buf", torch.zeros(self.world_size, dtype=torch.int64)
        )
        self.act_fn = ACT2FN[config.hidden_act]
    ...
    def moe_infer(self, x, topk_idx, topk_weight):
        ...
        recv_x = torch.empty(
            recv_total, hidden_size, device=device, dtype=x.dtype
        )
        recv_eid = torch.empty(
            recv_total, device=device, dtype=sorted_eids.dtype
        )
        ...
        if recv_total:
            ...
            res = self.grouped_dequant_moe(
                recv_x[local_sort_idx], recv_eid_sorted
            )
    ...
    def grouped_dequant_moe(self, recv_x, recv_eid):
        ...
        for e in range(
            ...
            gate_scale_list.append(
                self.experts[e].weight_dequant_scale[
                    "gate_proj.weight_scale_inv"
                ]
            )
            up_scale_list.append(
        ...
        group_sizes = sorted(
            (idx, sz) for idx, sz in enumerate(counts.tolist()) if sz
        )
        group_size = torch.tensor(
            [size for _, size in group_sizes],
            dtype=torch.int32,
            device=self.device,
        )
...
class DeepseekV3Attention_FlashMLA_Decoding_CUDAGraph(nn.Module):
    ...
    def _copy_inputs_to_static(self, static_inputs, **kwargs):
        ...
        for key, value in kwargs.items():
            if isinstance(value, torch.Tensor):
                ...
                for dict_key, dict_value in value.items():
                    if isinstance(dict_value, torch.Tensor):
                        static_inputs[key][dict_key].copy_(
                            dict_value, non_blocking=True
                        )
    ...
    def _validate_input_shapes(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if isinstance(value, torch.Tensor):
                ...
                if (
                    key in self.input_shapes
                    and self.input_shapes[key] != current_shape
                ):
                    raise ValueError(
        ...
        with torch.cuda.stream(stream):
            for i in range(10):
                with torch.no_grad():
                    try:
                        outputs = (
                            self.base_model.mla_decoding_flashmla_attn_mode_3(
                                **self.static_inputs
                            )
                        )
                        if i == 0:
                            print(
                                f"Warmup successful, output types: {[type(o) for o in outputs]}"
                            )
    ...
    def forward(
        ...
        if isinstance(self.static_outputs, tuple):
            attn_output, updated_past_key_states, updated_scale = (
                self.static_outputs
            )
            return (
                attn_output.clone(),
                self.static_inputs[
                    "past_key_states"
                ].clone(),  # This was modified in-place
                self.static_inputs[
                    "scale"
                ].clone(),  # This was modified in-place
            )

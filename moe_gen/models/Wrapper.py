        ...
        if Attn_Wrapper.phase == "prefill":
            ...
            if self.get_weights:
                weights_dict = self.core_engine.get_weights(
                    self.attn_module_id, Attn_Wrapper.phase
                )
                for name, param in self.module.named_parameters():
                    param.data = weights_dict[name]
            ...
            if self.engine_config.Basic_Config.attn_mode != 3:
                ...
                for i in range(num_micro_batch):
                    ...
                    attn_result, kv, scale = self.module.decoding_attn_mode_3(
                        ...
                        past_value_states[start_ids:end_ids]
                        if past_value_states is not None
                        else None,
            ...
            if self.get_weights:
                ...
                self.module.q_a_proj.weight.data = self.fp8_q_a_proj
                self.module.q_b_proj.weight.data = self.fp8_q_b_proj
                self.module.kv_a_proj_with_mqa.weight.data = (
                    self.fp8_kv_a_proj_with_mqa
                )
                self.module.kv_b_proj.weight.data = self.fp8_kv_b_proj
                self.module.o_proj.weight.data = self.fp8_o_proj
            logging.debug(
...
class Expert_Wrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.get_weights:
            weights_dict = self.core_engine.get_weights(
                self.expert_weights_idx, Attn_Wrapper.phase
            )
            # torch.cuda.current_stream(self.engine_config.Basic_Config.device_torch).synchronize()
            # for name, param in self.module.named_parameters():
            #     if (
        ...
        num_micro_batch = math.ceil(
            hidden_states.size(0) / token_num_upper_bound
        )
        for i in range(num_micro_batch):
            ...
            if Attn_Wrapper.phase == "prefill":
                """deepgemm kernel"""
                result[start:end] = self.module.deepgemm_forward(

...
def fgemm_fp8_e4m3_bf16_kernel(
    ...
    stride_ak,
    stride_bk,
    stride_bn,  # Transposed from original
    stride_cm,
    stride_cn,
    # Meta-parameters
...
def fused_fp8_bf16_gemm(
    ...
    if out is not None:
        ...
        if out.dim() == 2:
            assert (
                out.shape[0] == A.shape[0] and out.shape[1] == B_fp8.shape[0]
            ), f"Output shape {out.shape} does not match expected shape ({A.shape[0]}, {B_fp8.shape[0]})"
        elif out.dim() == 3:
            assert (
                out.shape[0] == A.shape[0]
                and out.shape[1] == A.shape[1]
                and out.shape[2] == B_fp8.shape[0]
    ...
    ), "Input tensor B_fp8 must be of type float8_e4m3fn"
    assert (
        B_scale.dtype == torch.float32
    ), "Scale tensor B_scale must be of type float32"
    ...
    assert (
        K == Kb
    ), f"Incompatible dimensions: A has dims {A.shape}, B has dims {B_fp8.shape}"
    ...
    assert (
        B_scale.shape == expected_scale_shape
    ...
    fgemm_fp8_e4m3_bf16_kernel[grid](
        ...
        A.stride(1),
        B_fp8.stride(1),
        B_fp8.stride(0),  # Note the swap for transposition
        out.stride(0),
        out.stride(1),
    ...
    return (
        out.view(original_shape[0], original_shape[1], -1)

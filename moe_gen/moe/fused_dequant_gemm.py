...
def fused_fp8_bf16_gemm(A, B_fp8, B_scale, block_size=(64, 64, 32), scale_block_size=(128, 128), group_size_m=8, out=None, offset=0):
	"""
	...
	Notes:
		- This function handles both 2D and 3D input tensors for A. And the out tensor if set should have the same shape as A.
	"""
	assert A.dim() in [2, 3], "Input tensor A must be 2D or 3D"
	if out is not None:
		assert out.dim() in [2, 3], "Output tensor out must be 2D or 3D"
		if out.dim() == 2:
			assert out.shape[0] == A.shape[0] and out.shape[1] == B_fp8.shape[0], \
				f"Output shape {out.shape} does not match expected shape ({A.shape[0]}, {B_fp8.shape[0]})"
		elif out.dim() == 3:
			assert out.shape[0] == A.shape[0] and out.shape[1] == A.shape[1] and out.shape[2] == B_fp8.shape[0], \
				f"Output shape {out.shape} does not match expected shape ({A.shape[0]}, {A.shape[1]}, {B_fp8.shape[0]})"
	assert A.dtype == torch.bfloat16, "Input tensor A must be of type bfloat16"
	assert B_fp8.dtype == torch.float8_e4m3fn, "Input tensor B_fp8 must be of type float8_e4m3fn"
	assert B_scale.dtype == torch.float32, "Scale tensor B_scale must be of type float32"
	...
	assert B_scale.shape == expected_scale_shape, \
		f"Expected B_scale to have shape {expected_scale_shape}, got {B_scale.shape}"
	...
	return out.view(original_shape[0], original_shape[1], -1) if len(original_shape) == 3 else out
	...
	# if out is None:
	# 	res = torch.empty((M, N), dtype=torch.bfloat16, device=A.device)
	# else:
	# 	res = out
	...
	# 	# N, 1,
	# 	(out if out is not None else C).stride(0), (out if out is not None else C).stride(1),
	# 	BLOCK_SIZE_M=block_size[0], 
	# 	BLOCK_SIZE_N=block_size[1], 
	# 	BLOCK_SIZE_K=block_size[2],
	# 	SCALE_BLOCK_M=scale_block_size[0], 
...
#     # ----------------------------------------------------------
#     # Create base offsets for A, B, and C
#     offs_am = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
#     offs_bn = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
#     offs_k = tl.arange(0, BLOCK_SIZE_K)
    
#     # Create base pointers for A and B
#     a_ptrs_base = a_ptr + (offs_am[:, None] * stride_am)
#     b_ptrs_base = b_ptr + (offs_bn[:, None] * stride_bn)
    
#     # Pre-calculate scale block parameters
#     n_scale_k = tl.cdiv(K, SCALE_BLOCK_K)
#     n_pos = pid_n * BLOCK_SIZE_N
#     scale_n_block = n_pos // SCALE_BLOCK_M
...
#         # Create current K offsets and masks
#         current_k_offs = offs_k + k_start
#         k_mask = current_k_offs < K
        
#         # Create pointers for current iteration
#         a_ptrs = a_ptrs_base + current_k_offs[None, :] * stride_ak
#         b_ptrs = b_ptrs_base + current_k_offs[None, :] * stride_bk
...
#         scale_k_block = k_start // SCALE_BLOCK_K
#         scale_idx = scale_n_block * n_scale_k + scale_k_block
#         scale = tl.load(scale_ptr + scale_idx)
        
#         # Convert B from fp8 to bf16 with scaling
#         b_fp32 = tl.cast(b_fp8, tl.float32)
#         b_scaled = b_fp32 * scale
#         b = tl.cast(b_scaled, tl.bfloat16)
...
#         offset: optional offset for output pointer
...
#     # Launch kernel with proper stride handling
#     fgemm_fp8_e4m3_bf16_kernel[grid](
#         A, B_fp8, output_tensor, B_scale,
#         M, N, K,
#         A.stride(0), A.stride(1),
#         B_fp8.stride(1), B_fp8.stride(0),  # Note the swap for transposition
#         output_tensor.stride(0), output_tensor.stride(1),  # Fixed: use actual strides
#         BLOCK_SIZE_M=block_size[0], 
#         BLOCK_SIZE_N=block_size[1], 
#         BLOCK_SIZE_K=block_size[2],
#         SCALE_BLOCK_M=scale_block_size[0], 
#         SCALE_BLOCK_K=scale_block_size[1],
#         GROUP_SIZE_M=group_size_m,
#         c_ptr_offset=offset if offset is not None else 0,
#     )
    
#     # Reshape output if necessary
#     if out is None:
#         if len(original_shape) == 3:
#             bsz, seq_len = original_shape[0], original_shape[1]
#             return C.reshape(bsz, seq_len, N)  # Fixed: correct reshape
#         else:
#             return C
#     else:
#         return out

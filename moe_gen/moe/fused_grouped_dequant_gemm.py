...
def fused_dequant_grouped_gemm_bf16_fp8_kernel(
    ...
    for g in range(num_groups):
        ...
        rhs_base_ptr = tl.load(rhs_ptrs_ptr + group_idx * stride_rhs_ptrs).to(
            tl.pointer_type(rhs_dtype)
        )
        rhs_scale_base_ptr = tl.load(
            rhs_scale_ptrs_ptr + group_idx * stride_rhs_scale_ptrs
        ...
        while tile_id < num_tiles:
            ...
            offs_lhs_m = tile_m * GEMM_BLOCK_SIZE_M + tl.arange(
                0, GEMM_BLOCK_SIZE_M
            )
            offs_rhs_n = tile_n * GEMM_BLOCK_SIZE_N + tl.arange(
                0, GEMM_BLOCK_SIZE_N
            )
            ...
            lhs_ptrs = base_ptr_g + (
                offs_lhs_m[:, None] * stride_lhs_m
                + offs_k[None, :] * stride_lhs_k
            )
            rhs_ptrs = rhs_base_ptr + (
            ...
            for k in range(0, tl.cdiv(K, GEMM_BLOCK_SIZE_K)):
                ...
                lhs_mask = (offs_lhs_m[:, None] < gm) & (
                    k_idx + offs_k[None, :] < K
                )
                rhs_mask = (offs_rhs_n[:, None] < N) & (
                    k_idx + offs_k[None, :] < K
                )
                ...
                rhs_fp8 = tl.load(
                    rhs_ptrs, mask=rhs_mask, other=0.0, cache_modifier=".cg"
                )
            ...
            offs_output_n = tile_n * GEMM_BLOCK_SIZE_N + tl.arange(
                0, GEMM_BLOCK_SIZE_N
            )
            ...
            output_mask = (offs_output_m[:, None] < start_idx + gm) & (
...
def fused_dequant_grouped_gemm_bf16_fp8_triton(
    ...
    assert all(
        r.dtype == torch.float8_e4m3fn for r in rhs_list
    ), "All rhs matrices must be of dtype float8_e4m3fn"
    assert all(
        s.dtype == torch.float32 for s in rhs_scale_list
    ), "All scale tensors must be of dtype float32"
    assert len(rhs_list) == len(
        rhs_scale_list
    ), "rhs_list and rhs_scale_list must have the same length"

    device = lhs.device
    N = rhs_list[0].shape[0]
    K = lhs.shape[1]
    rhs_ptrs_ptr = torch.tensor(
        [r.data_ptr() for r in rhs_list], dtype=torch.int64, device=device
    )
    rhs_scale_ptrs_ptr = torch.tensor(
        [s.data_ptr() for s in rhs_scale_list], dtype=torch.int64, device=device
    )
    ...
    try:
        ...
        print(
            f"Error launching fused_dequant_grouped_gemm_bf16_fp8_kernel: {e}"
        )
        raise
    return output

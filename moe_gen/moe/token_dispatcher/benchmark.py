...
class FusedMoETokenDispatch:
    ...
    - torch.float32 (fp32)
    - torch.float16 (fp16) 
    - torch.bfloat16 (bf16) - Recommended for modern LLMs
    """
...
# Example usage and benchmark
def benchmark_dispatch(num_tokens=8192, hidden_size=4096, K=2, num_experts=64, num_local_experts=8, dtype=torch.bfloat16):
    """Benchmark the fused vs original implementation."""
    ...
    print(f"Average time per dispatch ({dtype}): {(end_time - start_time) / 100 * 1000:.2f} ms")
    print(f"Result shapes: input_x={result[0].shape}, input_eids={result[1].shape}")
...
if __name__ == "__main__":
    ...
    for dtype_name, dtype in [("bfloat16", torch.bfloat16), ("float16", torch.float16), ("float32", torch.float32)]:
        print(f"\n--- {dtype_name.upper()} ---")
        try:
            benchmark_dispatch(dtype=dtype)
        except Exception as e:

...
class ParameterServerClient:
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if skeleton_state_dict_file and os.path.exists(
            ...
            if (
                skeleton_state_dict_shm_name
                and skeleton_state_dict_shm_name.endswith(".bin")
            ):
                ...
                file_path = os.path.join(
                    backup_dir, skeleton_state_dict_shm_name
                )

                if os.path.exists(file_path):
                    try:
                        logging.info(
                            f"Loading skeleton state dict from backup file: {file_path}"
                        )
                        ...
                        raise RuntimeError(
                            f"Failed to load skeleton state dict: {e}"
                        )
                else:
                    raise RuntimeError(
                        f"Skeleton state dict file not found: {file_path}"
                    )

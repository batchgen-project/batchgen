...
def compressed_kv_bf16_to_fp8_per_token(
    x: torch.Tensor,
    ...
    assert (
        x.dim() == 3
    ), f"Expected x to be 3D tensor with last dimension of size 576, got {x.dim()}D with shape {x.shape}"
...
def per_token_blocked_quantize_bf16_to_fp8_kernel(
    ...
    block_offsets = (
        pid_seq * q_stride0
        + pid_token * q_stride1
        + (pid_block * block_size + tl.arange(0, block_size)) * q_stride2
    )
    # The size of the block can be less than 128 for the last block
    q_bf16 = tl.load(
        q_ptr + block_offsets,
        mask=(pid_block * block_size + tl.arange(0, block_size)) < dim,
        other=0.0,
    )
    # x_amax = x_view.abs().float().amax(dim=2).view(m, -1).clamp(1e-4)
    ...
    out_offsets = (
        pid_seq * out_stride0
        + pid_token * out_stride1
        + (pid_block * block_size + tl.arange(0, block_size)) * out_stride2
    )
    tl.store(
        out_ptr + out_offsets,
        q_fp8,
    ...
    tl.store(
        scale_ptr
        + pid_seq * scale_stride0
        + pid_token * scale_stride1
        + pid_block * scale_stride2,
        scale,
    )


def per_token_blocked_quantize_bf16_to_fp8(
    ...
    assert (
        x.dim() == 3
    ), f"Expected x to be 3D tensor got {x.dim()}D with shape {x.shape}"
    ...
    out = torch.empty(
        (bsz, seq_len, dim), device=x.device, dtype=torch.float8_e4m3fn
    )
    scale = torch.empty(
        (bsz, seq_len, num_blocks), device=x.device, dtype=torch.float32
    )
    # Call the kernel
...
def dequant_compressed_kv_per_token_with_length_kernel(
    ...
    block_offsets = (
        pid_seq * kv_stride0
        + pid_token * kv_stride1
        + (pid_block * block_size + tl.arange(0, block_size)) * kv_stride2
    )
    # The size of the block can be less than 128 for the last block
    # Load the quantized KV tensor
    kv_fp8 = tl.load(
        kv_ptr + block_offsets,
        mask=(pid_block * block_size + tl.arange(0, block_size)) < dim,
    )
    ...
    out_offsets = (
        pid_seq * output_stride0
        + pid_token * output_stride1
        + (pid_block * block_size + tl.arange(0, block_size)) * output_stride2
    )
...
def dequant_compressed_kv_per_token_with_length(
    ...
    assert (
        q.dtype == torch.float8_e4m3fn and scale.dtype == torch.float32
    ...
    assert seq_len <= q.size(
        1
    ), f"seq_len must be <= max_seq_len, got {seq_len} > {q.size(1)}"
    assert (
        q.is_contiguous() and scale.is_contiguous()
    ), "q and scale must be contiguous tensors"

    bsz, max_seq_len, dim = q.shape
    padded_seq_len = ceil(seq_len / 64) * 64  # Nearest multiple of 64

    result = torch.empty(
        (bsz, padded_seq_len, dim), device=q.device, dtype=torch.bfloat16
    )
    ...
    for token_id in range(num_tokens):
        ...
        for pid_block in range(num_blocks_per_token):
            ...
            block_offsets = (
                pid_seq * kv_stride0
                + pid_token * kv_stride1
                + (pid_block * block_size + tl.arange(0, block_size))
                * kv_stride2
            )
            # The size of the block can be less than 128 for the last block
            # Load the quantized KV tensor
...
def dequant_compressed_kv_per_token_with_length_(
    q: torch.Tensor, scale: torch.Tensor, seq_len: int, BLOCK_SIZE: int = 128
):
    """
    Dequantize FP8 KV-Cache tensor with scale.
    ...
    # Call the kernel
    dequant_compressed_kv_per_token_with_length_kernel_v3[grid](
        q,
        scale,
        result,
        dim,

...
class ParameterServer:
    ...
    def create_skeleton_state_dict_shared_memory(self, skeleton_state_dict):
        ...
        try:
            ...
            logging.info(
                f"Successfully wrote state dict to file, size: {actual_size} bytes"
            )
    ...
    def start(self):
        ...
        if self.initial_model_name:
            ...
            logging.info(
                "No initial model specified. Server starting without preloaded model."
            )
    ...
    def handle_shutdown(self, signum, frame):
        ...
        os.unlink(
            os.path.join("/dev/hugepages", self.model_info.get("shm_name"))
        )
    ...
    def handle_load_model(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if "deepseek" in huggingface_ckpt_name:
                from moe_gen.models.deepseek.deepseek_parameter_server import (
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Standalone Parameter Server for MoE-Gen"
    )
    parser.add_argument(
        "--host",
        type=str,
    ...
    parser.add_argument(
        "--port", type=int, default=9090, help="Port to listen on"
    )
    parser.add_argument(
        "--model",
        type=str,

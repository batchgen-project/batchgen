...
def rope_kernel(
	...
	for i in range(start_idx, end_idx):
		...
		first_half_cos_offset = position_id * cos_stride_seq + tl.arange(0, half_dim) * cos_stride_dim
		second_half_cos_offset = position_id * cos_stride_seq + (tl.arange(half_dim, head_dim) * cos_stride_dim)
		...
		first_half_sin_offset = position_id * sin_stride_seq + tl.arange(0, half_dim) * sin_stride_dim
		second_half_sin_offset = position_id * sin_stride_seq + (tl.arange(half_dim, head_dim) * sin_stride_dim)
		...
		offsets_x_first_half = i * stride_x_batch + tl.arange(0, half_dim) * stride_x_head
		offsets_x_second_half = i * stride_x_batch + (tl.arange(half_dim, head_dim) * stride_x_head)
		...
		first_half_out_offset = i * stride_out_batch + tl.arange(0, half_dim) * stride_out_head
		second_half_out_offset = i * stride_out_batch + (tl.arange(half_dim, head_dim) * stride_out_head)
...
def fused_rotary_embedding(
	...
	position_ids = position_ids.contiguous()  # Ensure position_ids is contiguous in memory
	...
	return result.view(B, H, S, D)  # Reshape back to original shape [batch, heads, seq_len, head_dim]
...
def rope_inplace_kernel(
	...
	for i in range(start_idx, end_idx):
		...
		first_half_intermediate -= second_half_x * first_half_sin_values
		# Store the result in the output tensor
		first_half_out_offset = i * stride_out_batch + (result_offset + tl.arange(0, half_dim)) * stride_out_head
		second_half_out_offset = i * stride_out_batch + (result_offset + (tl.arange(half_dim, head_dim)) * stride_out_head)
		tl.store(out_ptr + first_half_out_offset, first_half_intermediate)
		tl.store(out_ptr + second_half_out_offset, second_half_intermediate)
...
def fused_rotary_embedding_inplace(
	...
	return result.view(RB, RS, RD)  # Reshape back to original shape [batch, heads, seq_len, head_dim]
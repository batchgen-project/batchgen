...
def mla_prefill_flashattention3(
    ...
    key_states = k_pe.new_empty(bsz, seq_len, self.num_heads, self.q_head_dim)
    k_pe = k_pe.view(bsz, seq_len, 1, self.qk_rope_head_dim)
    ...
    attn_output = (
        pad_input(attn_output_unpad, indices_q, bsz, seq_len)
        .view(bsz, seq_len, self.num_heads * self.v_head_dim)
        .contiguous()
    )
...
def mla_prefill_flashattention3_fused_dequant(
    ...
    # 	query_states
    # )
    query_states = fused_fp8_bf16_gemm(
        query_states,
        self.q_b_proj.weight.data,
        weight_scale["q_b_proj.weight_scale_inv"],
    )

...
def fused_bhd_hdc_kernel(
	...
	# Matrix dimensions
	B,H,
	D: tl.constexpr, C: tl.constexpr,
	# Strides for A, B, and C
	stride_a_b, stride_a_h, stride_a_d,
	stride_b_h, stride_b_d, stride_b_c,
	stride_c_b, stride_c_h, stride_c_c,
	# Meta-parameters
	...
	"""
		...
		BHC = BHD @ HDC
		C[:, h, :] = A[:, h, :] @ B[h, :, :] -> [b,d] @ [d,c]
		The workload is splitted along the h dimention to thread-blocks.
		Args:
	...
	num_b_blocks = tl.cdiv(B, BLOCK_SIZE_B) # Number of blocks along h dimension.
	offsets_b = (pid * stride_b_h + tl.arange(0, D)[:,None] * stride_b_d + tl.arange(0, C)[None,:] * stride_b_c)
	# Load B
	b = tl.load(b_ptr + offsets_b)
	for block in range(num_b_blocks):
		# Load the b-th block of A.
		offsets_a = (block * BLOCK_SIZE_B + tl.arange(0, BLOCK_SIZE_B)[:,None] * stride_a_b + pid * stride_a_h + tl.arange(0, D)[None,:] * stride_a_d)
		a = tl.load(a_ptr + offsets_a, mask=(block * BLOCK_SIZE_B + tl.arange(0, BLOCK_SIZE_B)[:,None] < B), other=0.0)
		# Perform the dot product
		c = tl.dot(a, b)
		# Store the result in C
		offsets_c = (block * BLOCK_SIZE_B + tl.arange(0, BLOCK_SIZE_B)[:,None] * stride_c_b + pid * stride_c_h + tl.arange(0, C)[None,:] * stride_c_c)
		tl.store(c_ptr + offsets_c, c, mask=(block * BLOCK_SIZE_B + tl.arange(0, BLOCK_SIZE_B)[:,None] < B))
...
def fused_bhd_hdc_kernel_with_c_offset(
	...
	# Matrix dimensions
	B,H,
	D: tl.constexpr, C: tl.constexpr, K: tl.constexpr,
	# Strides for A, B, and C
	stride_a_b, stride_a_h, stride_a_d,
	stride_b_h, stride_b_d, stride_b_c,
	stride_c_b, stride_c_h, stride_c_c,
	...
	for block in range(num_b_blocks):
		...
		c = tl.dot(a, b)
		# Store the result in C
		offsets_c = (block * BLOCK_SIZE_B + tl.arange(0, BLOCK_SIZE_B)[:,None] * stride_c_b + pid * stride_c_h + (K + tl.arange(0, C))[None, :] * stride_c_c)
		tl.store(c_ptr + offsets_c, c, mask=(block * BLOCK_SIZE_B + tl.arange(0, BLOCK_SIZE_B)[:,None] < B))

...
def dequant_per_token_triton(
    q: torch.Tensor, scale: torch.Tensor, BLOCK_SIZE: int = 128
):
...
def dequant_per_token_return_with_max_seqlen_pad(
    ...
    assert max_seqlen_pad >= q.size(
        1
    ), f"max_seqlen_pad must be >= seq_len, got {max_seqlen_pad} < {q.size(1)}"
    ...
    x_flat = torch.zeros(
        (bsz * max_seqlen_pad, dim), device=q.device, dtype=torch.bfloat16
    )

...
def test_fused_decode_projection_matches_native_projection_values():
    ...
    # The stub's fused N is 4*6 + 3 + 2 = 29; the fused weight is padded with
    # zero rows to a multiple of 16 so cuBLAS keeps the aligned sm90 kernel
    # (K3 TP8: 6284 -> 6288), and the split ignores the padding.
    fused = attn._kda_decode_fused_weight
    assert fused.shape[0] == 32
    assert torch.count_nonzero(fused[29:]) == 0

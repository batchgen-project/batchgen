#!/usr/bin/env bash
# ---------------------------------------------------------------------------- #
#  Kimi-K3 M2 — staged GPU validation, <gpu-host>, GPU 0 (reserved for K3) #
#                                                                              #
#  Run ON <gpu-host> from the repo root, inside the    #
#  `batchgen` conda env, AFTER syncing the branch (git push local +            #
#  git pull on remote — NEVER scp/rsync into the repo):                        #
#                                                                              #
#    ssh <gpu-host> 'source /root/miniconda3/etc/profile.d/conda.sh && \
#      conda activate batchgen && \
#      cd <REMOTE_CHECKOUT> && \
#      bash tests/gpu/run_kimi_k3_kda_gpu.sh'                            #
#                                                                              #
...
#  The log path is printed below.                      #
# ---------------------------------------------------------------------------- #
set -euo pipefail

LOG_DIR="${K3_GPU_LOG_DIR:-/tmp/k3_gpu_logs}"

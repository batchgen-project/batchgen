"""Benchmark GLM-5 DSA q projection + RoPE/Hadamard + score/top-k graph slice."""
...
def _make_case(batch_size: int, max_seqlen: int, topk: int, module):
    n_heads = 32
    head_dim = 128
    q_rank = 2048
    ...
    topk_out = torch.empty(batch_size, topk, device="cuda", dtype=torch.long)

    def allocating_path():
        q = cuda_wq_b_proj(q_a, q_weights, module).view(batch_size, n_heads, head_dim)
        q = rope_hadamard_q(q, cos, sin, positions)
        return fused_score_and_topk(q, cached_k, head_gates, cache_seqlens, topk=topk)
    ...
    def out_path():
        ...
        fused_score_and_topk_out(q_rope, cached_k, head_gates, cache_seqlens, agg, topk_out, topk=topk)
        return topk_out
...
def main() -> None:
    ...
    for batch_size in args.batch_sizes:
        for max_seqlen in args.seq_lens:
            ...
            allocating_path, out_path = _make_case(batch_size, max_seqlen, topk, module)

...
def test_synchronous_host_to_gpu_load_uses_grouped_kv_path():
    source = WORKER.read_text()
    start = source.index("\tdef _load_host_kv_to_gpu(")
    end = source.index("\n\tdef _release_gpu_kv_pages", start)
    body = source[start:end]

    grouped_branch = body.index("if isinstance(manager, GroupedGPUKVCoordinator):")
    ...
    assert grouped_branch < dual_prepare < dual_launch < primary_only_call

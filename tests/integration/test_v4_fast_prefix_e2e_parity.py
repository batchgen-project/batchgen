...
def _run_decode(fast_enabled, seq_len=160, layer_idx=0):
    ...
    try:
        ...
        backend = DeepseekV4AttnBackend(
            ...
            flashmla_backend=adapter.DeepSeekV4FlashMLADecodeAdapter(
                coordinator
            ),
        )
        ...
        for step in range(seq_len):
            metadata = adapter.build_v4_decode_attn_metadata(
...
def test_fast_prefix_e2e_matches_slow():
    import batchgen.attention.dsa.v4_flashmla_adapter as adapter

    saved = adapter._V4_FAST_PREFIX_INDICES
    try:
        out_fast = _run_decode(fast_enabled=True)
        out_slow = _run_decode(fast_enabled=False)
    finally:

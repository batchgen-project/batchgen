...
def test_grouped_fp8_blockwise_moe_uses_shared_extension_loader(monkeypatch):
    ...
    # The shared loader is consulted once and memoized in _kernel_module, so a
    # second getter must not re-enter load_extension.
    assert calls == ["batchgen_kernels.moe._C_fp8_blockwise_gemm"]

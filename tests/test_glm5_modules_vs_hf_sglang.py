...
def _bg_linear_fp8(x_bf16, weight_name, device):
    ...
    raw FP8 weight + FP32 blockwise scale from disk. Calls BatchGen's
    wrapper directly so any bug in that wrapper (e.g. wrong scale layout,
    wrong dispatch) surfaces here rather than being papered over."""
    ...
    # w8a16_gemm accepts (B, L, K) or (M, K); our x is (1, L, 6144).

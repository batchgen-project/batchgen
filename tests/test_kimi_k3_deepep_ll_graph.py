...
def test_planner_gate_requires_sharded_latent(monkeypatch):
    ...
    # default (--enable-deepep absent) = NCCL, even when the DeepEP build is present
    ...
    assert k3_deepep_low_latency(sharded_latent=True, enable_deepep=False) is False
    # --enable-deepep on, sharded-latent gate + build present -> DeepEP
    assert k3_deepep_low_latency(sharded_latent=True, enable_deepep=True) is True
    # --enable-deepep on but the gate is absent -> fail fast (no silent NCCL fallback)
    with pytest.raises(RuntimeError, match="enable-deepep"):
        k3_deepep_low_latency(sharded_latent=False, enable_deepep=True)
    # build absent -> off stays NCCL; on fails fast
    monkeypatch.setattr("batchgen.moe.deepep_ll.deepep_available", lambda: False)
    assert k3_deepep_low_latency(sharded_latent=True, enable_deepep=False) is False
    with pytest.raises(RuntimeError, match="enable-deepep"):
        k3_deepep_low_latency(sharded_latent=True, enable_deepep=True)

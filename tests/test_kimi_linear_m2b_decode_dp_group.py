...
def test_prefill_selection_groups_candidates_before_requiring_a_group():
    """Every prefill candidate must be stamped with a decode_dp_group first.

    Regression for `RuntimeError: sequence <uuid> has no decode_dp_group before
    TP prefill selection`. `_prepare_prefill_batch` builds
    `all_candidates = evicted_uuids + queueing_uuids`; under G>1
    `_make_prefill_selection_request` maps each candidate to a node THROUGH its
    group, but newly queued sequences are only grouped later at the decode
    transition. A wave mixing queued with evicted candidates therefore raised.
    `_assign_decode_dp_groups` must be called on the candidate list before the
    selection request is built; it is idempotent, so re-entrant sequences keep
    the group their head-sharded KDA state already lives on.
    """
    tree = ast.parse(WORKER.read_text())
    worker = next(
        node for node in tree.body
    ...
    method = next(
        node for node in worker.body
        if isinstance(node, ast.FunctionDef) and node.name == "_prepare_prefill_batch"
    )
    assign_line = selection_line = None
    for node in ast.walk(method):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            if node.func.attr == "_assign_decode_dp_groups" and assign_line is None:
                assign_line = node.lineno
            if node.func.attr == "_make_prefill_selection_request" and selection_line is None:
                selection_line = node.lineno
    assert assign_line is not None, (
        "_prepare_prefill_batch must assign decode_dp_groups to its candidates"
    )
    assert selection_line is not None, "selection request call not found"
    assert assign_line < selection_line, (
        "_assign_decode_dp_groups must run BEFORE _make_prefill_selection_request, "
        "which raises on any candidate lacking a group under G>1"
    )

...
def compact_prefill_chunk_rows(num_global, configured_rows, large_pass_rows=256):
    """Choose the bounded resident-prefill expert chunk size.

    ``large_pass_rows`` caps the chunk once a pass exceeds 16K global rows.
    The 256 default is the tight-HBM rule below; the K3 H200 planner raises
    it (k3_resident_prefill_chunk_rows) because every chunk is a full
    dispatch + two grouped Marlin launches whose cost is launch-bound at
    small rows: p4 (2x8 H200, 96-sequence handoff wave, 32K-row passes)
    spent 41.6 s of a 66.9 s wave in 80,408 chunk calls of ~0.28 ms.
    """
    ...
    if int(num_global) > 16384:
        ...
        # per-token arithmetic or the single final reduce.
        rows = min(rows, int(large_pass_rows))
    if rows <= 0:
        raise ValueError("compact_chunk_rows must be positive")
    return rows


def rank_padded_row_map(num_rows, group_size, ntp, device):
    """Map the TP group's ``num_rows`` group rows to a rank-padded layout of
    ``group_size * ntp`` rows: rank r's i-th row (``balanced_row_split``)
    sits at ``r * ntp + i``; padding rows map to ``-1``. Returns
    (padded_to_group [G*ntp] with -1 for padding, group_to_padded [num_rows]).
    This is the row order every rank's EP-gather contribution uses when the
    latent projections are TP-sharded, so the gathered buffer assembles into
    the rank-major ``[world * ntp, K_latent]`` layout the dispatch consumes
    without any cross-node bookkeeping."""
    from batchgen.models.moonshotai.kimi_linear.moe_tp_reshard import balanced_row_split
    padded_to_group = torch.full((group_size * ntp,), -1, dtype=torch.long, device=device)
    group_to_padded = torch.empty((num_rows,), dtype=torch.long, device=device)
    for r, (s, e) in enumerate(balanced_row_split(num_rows, group_size)):
        n = e - s
        if n > ntp:
            raise ValueError(f"rank {r} owns {n} rows > ntp {ntp}")
        if n:
            idx = torch.arange(s, e, device=device)
            padded_to_group[r * ntp : r * ntp + n] = idx
            group_to_padded[s:e] = torch.arange(r * ntp, r * ntp + n, device=device)
    return padded_to_group, group_to_padded


def pad_rows_rank_major(x_group, padded_to_group):
    """[num_rows, H] group rows -> [G*ntp, H] rank-padded rows (zeros pad)."""
    out = x_group.new_zeros((padded_to_group.numel(), x_group.shape[1]))
    valid = padded_to_group >= 0
    out[valid] = x_group[padded_to_group[valid]]
    return out


def unpad_rows_rank_major(y_padded, group_to_padded):
    """Inverse of :func:`pad_rows_rank_major` (drops the padding rows)."""
    return y_padded[group_to_padded]


def assemble_gathered_latent_slices(gathered, world, group_size, ntp, cols):
    """EP-gathered column slices ``[world * G*ntp, cols]`` (rank-major; rank
    n*G+c holds node n's rank-padded rows x latent columns [c*cols, (c+1)*cols))
    -> ``[world * ntp, G*cols]`` full-latent rows in the rank-major order the
    dispatch expects (global row (n*G + r)*ntp + i)."""
    nodes = world // group_size
    g = gathered.view(nodes, group_size, group_size * ntp, cols)
    # g[n, c, row, :] -> out[n, row, c, :]
    out = g.permute(0, 2, 1, 3).reshape(nodes * group_size * ntp, group_size * cols)
...
class ResidentEPMXFP4MoELayer:
    ...
    def __init__(self, layer_idx, shard, down_proj, norm, up_proj,
        ...
        # chunk cap for passes above 16K global rows (see compact_prefill_chunk_rows)
        self.compact_large_pass_rows = 256
        # TP-sharded latent projections (DECODE_CONCURRENCY_PLAN.md): when
        # latent_tp_size > 1, down_proj holds latent columns
        # [c*cols, (c+1)*cols) for c = latent_tp_rank and up_proj the matching
        # input rows; the EP gather carries column slices of ALL group rows in
        # rank-padded order and the up-proj output is a TP partial.
        self.latent_tp_size = 1
        self.latent_tp_rank = 0
        self.latent_tp_group = None

    def set_latent_tp(self, tp_size, tp_rank, tp_group):
        self.latent_tp_size = int(tp_size)
        self.latent_tp_rank = int(tp_rank)
        self.latent_tp_group = tp_group
    ...
    def forward(self, x, gate, x_group=None):
        ...
        if self.world_size != 1:
            return self._forward_ep(x, gate, x_group=x_group)
        T, H = x.shape
        if T == 0:
    ...
    def _forward_ep(self, x, gate, x_group=None):
        ...
        K = topk_idx.shape[-1]
        G = self.latent_tp_size
        sharded = G > 1
        if sharded:
            # TP-sharded latent projections: this rank's down_proj holds latent
            # columns [c*cols, (c+1)*cols). Apply it to ALL of the TP group's
            # rows (group-replicated ``x_group``) in rank-padded order, so the
            # EP gather of every rank's [G*ntp, cols] slice assembles into the
            # usual rank-major [world*ntp, K_latent] global latent (same bytes
            # on the wire as gathering [ntp, K_latent] per rank).
            if x_group is None:
                raise RuntimeError(
                    "sharded latent projections need the group-replicated rows")
            cols = self.down_proj.weight.shape[0]
            if cols * G != K_latent:
                raise RuntimeError(
                    f"down_proj holds {cols} latent columns x {G} ranks != K_latent {K_latent}")
            padded_to_group, group_to_padded = rank_padded_row_map(
                x_group.shape[0], G, ntp, x.device)
            x_pad = pad_rows_rank_major(x_group, padded_to_group)  # [G*ntp, H]
            x_latent = self.down_proj(x_pad).contiguous()          # [G*ntp, cols]
        else:
        ...
        if sharded:
            gathered = x_latent.new_empty((world * G * ntp, cols))
            with self.comm.change_state(enable=True):
                self.comm.all_gather(gathered, x_latent)
                self.comm.all_gather(all_idx, topk_idx)
                self.comm.all_gather(all_weight, topk_weight)
            all_latent = assemble_gathered_latent_slices(gathered, world, G, ntp, cols)
        else:
        ...
        if self.compact_dispatch:
            ...
            chunk_rows = compact_prefill_chunk_rows(
                num_global, self.compact_chunk_rows, self.compact_large_pass_rows
            )
        ...
        # Each rank consumes only its own ntp-row slice, so a reduce_scatter
        # (rank r receives rows [r*ntp, (r+1)*ntp) summed over all ranks,
        # FP32 partial sums as before) moves half the bytes of the all_reduce
        # it replaces. Resident-EP prefill waves are collective-bound: r28 spent
        # 104 s of a 675 s D512 wall in three mid-run waves at ~1.6K tok/s.
        y_local = y.new_empty((ntp, K_latent))
        with self.comm.change_state(enable=True):
            self.comm.reduce_scatter(y_local, y, op=ReduceOp.SUM)
        y = y_local

        # --- own rows -> bf16 -> post-combine norm + up-proj: latent->hidden ---
        start = 0
        if sharded:
            import torch.distributed as dist
            # Norm needs the full latent per row (this rank's own rows, as
            # today); then one node-local gather of [ntp, K_latent] rows puts
            # the whole group's rows (rank-padded order) on every rank, and the
            # row-parallel up_proj on this rank's column slice yields a
            # [G*ntp, H] TP partial the caller folds into the shared-expert
            # all_reduce. Returns rows in GROUP order (padding dropped).
            y_rows = y[start:start + ntp].to(torch.bfloat16)      # [ntp, K_latent]
            if T < ntp:
                y_rows = y_rows.clone()
                y_rows[T:] = 0
            if self.norm is not None and T > 0:
                y_rows[:T] = self.norm(y_rows[:T])
            gathered_rows = y_rows.new_empty((G * ntp, K_latent))
            dist.all_gather_into_tensor(gathered_rows, y_rows, group=self.latent_tp_group)
            c = self.latent_tp_rank
            part = self.up_proj(gathered_rows[:, c * cols:(c + 1) * cols])  # [G*ntp, H]
            return unpad_rows_rank_major(part, group_to_padded)     # [num_rows, H] partial

...
def load_fused_moe_cuda():
    ...
    try:
        ...
        fused_moe_cuda = load(
            name="fused_moe_cuda",
            sources=[
                os.path.join(current_dir, "fused_moe_permutation.cu")
            ],
            extra_cuda_cflags=[
                "-O3",
                "--use_fast_math",
...
def benchmark_permutation(num_tokens=8192, hidden_size=4096, K=2, num_experts=64, num_local_experts=8, dtype=torch.bfloat16):
    """Benchmark the fused vs original implementation."""
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Create test data
    global_x = torch.randn(num_tokens, hidden_size, device=device, dtype=dtype)
    ...
    # Initialize permutationer
    permutationer = FusedMoETokenPermutation(use_cuda_if_available=True)
    
    # Warm up
    for _ in range(10):
        result = permutationer(global_x, topk_idx, token_idx, topk_pos,
                          routed_expert_start_idx, routed_expert_end_idx)
    ...
    print(f"Average time per permutation ({dtype}): {(end_time - start_time) / 100 * 1000:.2f} ms")
...
if __name__ == "__main__":
    # Run benchmark with different dtypes
    print("Testing different data types:")
    print("=" * 40)
    
    for dtype_name, dtype in [("bfloat16", torch.bfloat16), ("float16", torch.float16), ("float32", torch.float32)]:
        print(f"\n--- {dtype_name.upper()} ---")
        try:
            benchmark_permutation(dtype=dtype)
        except Exception as e:

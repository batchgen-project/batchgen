...
def fused_fp8_moe_stage_1_kernel_optimized(
    ...
    Works when GEMM_BLOCK_SIZE_K is 2x SCALE_BLOCK_SIZE_K (e.g., 256 vs 128).
    """
    pid = tl.program_id(axis=0)
    lhs_dtype = tl.bfloat16
    rhs_dtype = tl.float8e4nv
    ...
    # Compile-time constant: how many quant blocks per GEMM tile?
    NUM_QUANT_PER_GEMM: tl.constexpr = GEMM_BLOCK_SIZE_K // SCALE_BLOCK_SIZE_K
    ...
    for g in range(num_groups):
        ...
        for sub_group_idx in range(num_sub_groups):
            ...
            for gemm_k_idx in range(num_gemm_k_blocks):
                ...
                # UNROLLED: Process both quantization blocks explicitly
                # This eliminates the inner loop when NUM_QUANT_PER_GEMM=2
                
                # ===== FIRST QUANTIZATION BLOCK =====
                sub_k_start_0 = gemm_k_start
                offsets_k_0 = sub_k_start_0 + tl.arange(0, SCALE_BLOCK_SIZE_K)
                k_valid_0 = offsets_k_0 < K
                
                scale_k_0 = sub_k_start_0 // SCALE_BLOCK_SIZE_K
                ...
                lhs_scale_k_0 = sub_k_start_0 // SCALE_BLOCK_SIZE_K
                lhs_scale_0 = tl.load(
                    lhs_scale_ptr + (abs_row_indices[:, None] * stride_lhs_scale_m + lhs_scale_k_0 * stride_lhs_scale_k),
                    mask=(abs_row_indices[:, None] < M), other=1.0, cache_modifier='.cg'
                )
                ...
                lhs_mask_0 = (abs_row_indices[:, None] < M) & k_valid_0[None, :] & (offsets_m[:, None] < valid_rows_this_block)
                rhs_mask_0 = (offsets_n[:, None] < N) & k_valid_0[None, :]
                
                lhs_0 = tl.load(lhs_ptrs_0, mask=lhs_mask_0, other=0.0, cache_modifier='.cg')
                gate_fp8_0 = tl.load(gate_ptrs_0, mask=rhs_mask_0, other=0.0, cache_modifier='.cg')
                up_fp8_0 = tl.load(up_ptrs_0, mask=rhs_mask_0, other=0.0, cache_modifier='.cg')
                ...
                # ===== SECOND QUANTIZATION BLOCK (only if NUM_QUANT_PER_GEMM >= 2) =====
                if NUM_QUANT_PER_GEMM >= 2:
                    sub_k_start_1 = gemm_k_start + SCALE_BLOCK_SIZE_K
                    
                    # Early exit if second block is beyond K
                    if sub_k_start_1 < K:
                        offsets_k_1 = sub_k_start_1 + tl.arange(0, SCALE_BLOCK_SIZE_K)
                        k_valid_1 = offsets_k_1 < K
                        
                        scale_k_1 = sub_k_start_1 // SCALE_BLOCK_SIZE_K
                        ...
                        lhs_scale_k_1 = sub_k_start_1 // SCALE_BLOCK_SIZE_K
                        lhs_scale_1 = tl.load(
                            lhs_scale_ptr + (abs_row_indices[:, None] * stride_lhs_scale_m + lhs_scale_k_1 * stride_lhs_scale_k),
                            mask=(abs_row_indices[:, None] < M), other=1.0, cache_modifier='.cg'
                        )
                        ...
                        lhs_mask_1 = (abs_row_indices[:, None] < M) & k_valid_1[None, :] & (offsets_m[:, None] < valid_rows_this_block)
                        rhs_mask_1 = (offsets_n[:, None] < N) & k_valid_1[None, :]
                        
                        lhs_1 = tl.load(lhs_ptrs_1, mask=lhs_mask_1, other=0.0, cache_modifier='.cg')
                        gate_fp8_1 = tl.load(gate_ptrs_1, mask=rhs_mask_1, other=0.0, cache_modifier='.cg')
                        up_fp8_1 = tl.load(up_ptrs_1, mask=rhs_mask_1, other=0.0, cache_modifier='.cg')
...
def fused_fp8_moe_stage_1_v2(
	...
	# Launch configuration: persistent kernel
	grid = (num_sms, )
	
	try:
		fused_fp8_moe_stage_1_v2_kernel[grid](
			hidden_states, hidden_states_scale,
			gate_ptrs_ptr, up_ptrs_ptr,
			gate_scale_ptrs_ptr, up_scale_ptrs_ptr,
			activated_group_idx, group_size, group_start_indices,

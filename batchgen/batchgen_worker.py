    ...
    for pattern_len in range(min_pattern, max_check):
        ...
        for offset in range(pattern_len):
            ...
            for rep in range(1, min_count):
                if (
                    token_ids[
                        decoded_length - 1 - offset - pattern_len * rep
                    ].item()
                    != target
                ):
                    is_repeat = False
...
BATCHGEN_ENABLE_NAN_CHECK = (
    os.environ.get("BATCHGEN_ENABLE_NAN_CHECK", "0") == "1"
)

# Optional gate for expensive/critical diagnostics (default off in production)
BATCHGEN_ENABLE_CRITICAL_DIAGS = (
    os.environ.get("BATCHGEN_ENABLE_CRITICAL_DIAGS", "0") == "1"
)
...
BATCHGEN_NSYS_DECODE_PROFILE = (
    os.environ.get("BATCHGEN_NSYS_DECODE_PROFILE", "0") == "1"
)
...
if BATCHGEN_NSYS_DECODE_PROFILE_FORWARD_LIMIT <= 0:
    raise ValueError(
        "BATCHGEN_NSYS_DECODE_PROFILE_FORWARD_LIMIT must be positive"
    )
BATCHGEN_NSYS_DECODE_PROFILE_EXIT = (
    os.environ.get("BATCHGEN_NSYS_DECODE_PROFILE_EXIT", "1") == "1"
)
...
if (
    BATCHGEN_NSYS_DECODE_PROFILE
    and not BATCHGEN_NSYS_DECODE_PROFILE_CONTROLLER_RANKS
):
    raise ValueError(
        "BATCHGEN_NSYS_DECODE_PROFILE_CONTROLLER_RANKS must not be empty"
    )
...
HOST_KV_WATERMARK_PERCENT = int(
    os.environ.get("BATCHGEN_HOST_KV_WATERMARK", "70")
)
ENABLE_DECODE_PREEMPTION = (
    os.environ.get("BATCHGEN_ENABLE_DECODE_PREEMPTION", "1") == "1"
)  # Default ON
...
class QueryBookBufferPool:
    ...
    def __init__(
        ...
        self.input_ids_buffer = torch.zeros(
            (num_sequences, model_context_length), dtype=torch.long
        )
        self.decoded_tokens_buffer = torch.full(
            (num_sequences, max_decoding_length),
            pad_token_id,
            dtype=torch.int64,
        )
    ...
    def get_input_ids_view(
        self, slot: int, seq_extended_size: int
    ) -> torch.Tensor:
...
class InputArguments:
    ...
    padding_length: Optional[int] = (
        None  # Deprecated alias for older initializers.
    )
...
class BatchGenWorkerArgs:
    ...
    watchdog_timeout: Optional[float] = (
        600.0  # Seconds before declaring process stuck (10 min for long inference)
    )
    ...
    enable_ep_with_offloading: bool = (
        False  # Enable Expert Parallelism with offloading
    )
    ep_offloading_ratio: float = (
        0.0  # Ratio of experts per layer to offload (0.0-1.0)
    )
    ...
    kv_aux_memfd_fd: int = (
        -1
    )  # Separate memfd fd for auxiliary (indexer) KV cache
...
class BatchGenWorker:
    ...
    DECISION_INTERVAL = (
        DECISION_FREQUENCY_PAGES * 64
    )  # Tokens between boundary checks
    ...
    def __init__(self, args: BatchGenWorkerArgs):
        ...
        if args.skeleton_state_dict_file:
            logging.info(
                f"Rank {args.global_rank}: Loading skeleton state dict from {args.skeleton_state_dict_file}"
            )
            self.skeleton_state_dict = torch.load(args.skeleton_state_dict_file)
            logging.info(
                f"Rank {args.global_rank}: Loaded skeleton state dict with {len(self.skeleton_state_dict)} keys"
            )
        else:
        ...
        self.detokenization_include_special_tokens = getattr(
            args, "detokenization_include_special_tokens", False
        )

        # 4. Initialize Weights Storage (cudaHostRegister for weights)
        logging.info(
            f"Rank {self.rank}: Initializing shared memory segments (local_rank={self.local_rank})."
        )
        logging.info(
        ...
        logging.info(
            f"Rank {self.rank}: [startup] Weights storage init: {_time.monotonic() - _t0:.2f}s"
        )
        ...
        if dual_host is not None:
            ...
            logging.info(
                f"Rank {self.rank}: Initializing DualHostKVCoordinator with parallel cudaHostRegister (local_rank={self.local_rank})"
            )
            dual_host.initialize(
                device_index=self.local_rank, create_region=False
            )
            logging.info(
                f"Rank {self.rank}: DualHostKVCoordinator cudaHostRegister completed (local_rank={self.local_rank})"
            )
        else:
            ...
            if worker_kv_config.num_v_heads == 0:
                self.host_paged_kv_worker_view = (
                    core_engine.MLAHostPagedKVWorkerView(worker_kv_config)
                )
            else:
                self.host_paged_kv_worker_view = (
                    core_engine.DefaultHostPagedKVWorkerView(worker_kv_config)
                )

            # Initialize Host KV view (parallel cudaHostRegister for all local ranks)
            _t0 = _time.monotonic()
            logging.info(
                f"Rank {self.rank}: Initializing Host KV view with cudaHostRegister (local_rank={self.local_rank}, fast_init={args.fast_init})"
            )
            self.host_paged_kv_worker_view.initialize(
                device_index=self.local_rank, create_region=False
            )
            logging.info(
                f"Rank {self.rank}: [startup] Host KV init (cudaHostRegister): {_time.monotonic() - _t0:.2f}s"
            )
        ...
        self._free_local_indices: Set[int] = (
            set()
        )  # Track freed indices for O(1) allocation
        ...
        self.max_context_length = (
            None  # Set per-batch from client; None = use model max
        )
        self.model_context_length = (
            None  # Updated from model config during init
        )
        self.num_global_queries = 0
        self.num_local_queries = 0
        self._ignore_eos: bool = False
        self._temperature: Optional[float] = (
            None  # Sampling temperature (None = greedy)
        )
        self._top_p: Optional[float] = (
            None  # Nucleus sampling threshold (None = disabled)
        )
        self._logged_greedy: bool = (
            False  # Track if we've logged greedy mode this batch
        )
        self._logged_sampling: bool = (
            False  # Track if we've logged sampling mode this batch
        )
        ...
        self._nccl_group = (
            None  # StatelessProcessGroup for PyNccl (stores TCPStore)
        )
        ...
        self.gpu_kv_cache_size_gb: Optional[float] = (
            None  # Calculated in _calculate_gpu_kv_cache_size()
        )
    ...
    def Init(
        ...
        if self.adaptive_chunk_sizer is not None and max_decoding_length > 0:
            capped_max = min(
                self.adaptive_chunk_sizer.max_chunk, max_decoding_length
            )
            capped_max = (
                math.ceil(capped_max / SequenceEntry.PAGE_SIZE)
                * SequenceEntry.PAGE_SIZE
            )
            self.adaptive_chunk_sizer.max_chunk = capped_max

        logging.info(
            f"Initializing batchgen with global rank {self.args.global_rank} and world size {self.args.world_size} with PID: {os.getpid()}"
        )
        ...
        logging.info(
            f"Engine on device {self.device} initialized/reconfigured."
        )
    ...
    def _calculate_gpu_kv_cache_size(self) -> float:
        ...
        size_tensor = torch.tensor(
            [gpu_kv_cache_gb], dtype=torch.float32, device=self.torch_device
        )
    ...
    def _is_deepseek_v4_kv_manager(self, manager=None) -> bool:
        from batchgen.kv_cache.deepseek_v4_kv_coordinator import (
            DeepSeekV4KVCoordinator,
        )

        target = (
            manager if manager is not None else self.gpu_paged_kv_cache_manager
        )
        return isinstance(target, DeepSeekV4KVCoordinator)

    def _get_deepseek_v4_compress_ratios(self) -> List[int]:
        ...
        if not compress_ratios:
            raise ValueError(
                f"DeepSeek-V4 model {self.huggingface_ckpt_name!r} has no compress_ratios"
            )
        num_layers = None
        for config in (
            getattr(self, "model_config", None),
        ...
        invalid = sorted(set(compress_ratios) - {0, 4, 128})
        if invalid:
            raise ValueError(
                f"DeepSeek-V4 compress_ratios must be in {{0, 4, 128}}, got {invalid}"
            )
    ...
    def _initialize_gpu_kv_manager_fixed_size(self) -> GPUPagedKVCacheManager:
        ...
        if is_v4_model(self.huggingface_ckpt_name):
            ...
            compress_ratios = self._get_deepseek_v4_compress_ratios()
            bytes_per_page_unit = (
                DeepSeekV4KVCoordinator.bytes_per_page_unit_for(
            ...
            num_pages = total_bytes // bytes_per_page_unit
            if num_pages <= 0:
                raise ValueError(
                    f"GPU KV cache size {self.gpu_kv_cache_size_gb:.2f} GB is too small "
                    f"for one DeepSeek-V4 page-unit ({bytes_per_page_unit} bytes)"
                )
            manager = DeepSeekV4KVCoordinator(
                compress_ratios=compress_ratios,
                num_pages=num_pages,
            ...
            if self.rank == 0:
                allocated_gb = (bytes_per_page_unit * num_pages) / (1024**3)
                logging.info(
                    f"[GPU-KV] DeepSeekV4KVCoordinator initialized: "
                    f"{num_pages} page-units, allocated={allocated_gb:.2f} GB, "
                    f"layers={len(compress_ratios)}, "
                    f"c4_layers={sum(1 for r in compress_ratios if r == 4)}, "
                    f"c128_layers={sum(1 for r in compress_ratios if r == 128)}"
                )
            return manager
        ...
        if is_dsa_model(self.huggingface_ckpt_name):
            ...
            primary_bytes_per_page = (
                primary_profile.bytes_per_page() * primary_profile.num_layers
            )
            aux_bytes_per_page = (
                aux_profile.bytes_per_page() * aux_profile.num_layers
            )
            combined_bytes_per_page = (
                primary_bytes_per_page + aux_bytes_per_page
            )
            ...
            primary_config = self._with_cuda_graph_page_table_capacity(
                primary_config
            )
            ...
            primary = GPUPagedKVCacheManager(
                config=primary_config, device=self.local_rank
            )
            primary.initialize()
            auxiliary = GPUPagedKVCacheManager(
                config=aux_config, device=self.local_rank
            )
    ...
    def set_sampling_params(
        ...
        if self.rank == 0:
            if temperature is not None or top_p is not None:
                logging.warning(
                    f"[SAMPLING] temperature={temperature}, top_p={top_p} - will use sampling"
                )
            else:
                logging.info(
                    f"[SAMPLING] temperature=None, top_p=None - will use greedy decoding"
                )
    ...
    def set_batchgen_debug(self, debug: Optional[dict]) -> None:
        self._batchgen_debug = (
            debug if isinstance(debug, dict) and debug else None
        )
        if self.rank == 0 and self._batchgen_debug:
            logging.warning(
                f"[BATCHGEN_DEBUG] enabled flags: {sorted(self._batchgen_debug.keys())}"
            )

    def _active_batchgen_debug_for_sequences(
    ...
    def _glm5_dispatch_trace_enabled(self, debug: Optional[dict]) -> bool:
        if isinstance(debug, dict) and self._debug_flag_enabled(
            debug.get("glm5_dispatch_trace")
        ):
            return True
    ...
    def _flush_glm5_dispatch_trace_summary(self, reason: str) -> None:
        if not getattr(AttnWrapperBase, "glm5_dispatch_trace_enabled", False):
            return
        counts = dict(
            getattr(AttnWrapperBase, "glm5_dispatch_counts", {}) or {}
        )
        if not counts:
            return
        context = (
            getattr(AttnWrapperBase, "glm5_dispatch_trace_context", None) or {}
        )
        ...
        logging.warning(
            ...
            reason,
            getattr(AttnWrapperBase, "glm5_dispatch_trace_id", None)
            or "unknown",
    ...
    def _configure_glm5_dispatch_trace(self, batch_sequences) -> None:
        ...
        if (
            ...
            or getattr(AttnWrapperBase, "glm5_dispatch_trace_id", None)
            != trace_id
        ):
    ...
    def _admit_sequences_from_message(self, msg: dict) -> None:
        ...
        if max_prompt > self.max_input_length:
            ...
            if self.rank == 0:
                logging.info(
                    f"[ADMIT] Updated max_input_length to {self.max_input_length}"
                )
    ...
    def _tokenize_admitted_sequences(self, uuids: List[str]) -> None:
        ...
        for i, seq in enumerate(sequences):
            ...
            try:
                input_ids_view = self._buffer_pool.get_input_ids_view(
                    slot, seq_extended_size
                )
                input_ids_view[0, :actual_prompt_len] = torch.tensor(
                    input_ids_list, dtype=torch.long
                )
                seq.input_ids = input_ids_view
                seq.decoded_tokens = self._buffer_pool.get_decoded_tokens_view(
                    slot
                )
            except Exception:
    ...
    def _assign_admitted_sequences_to_ranks(self, uuids: List[str]) -> None:
        ...
        if use_l2:
            ...
            for L, uuid in pending:
                min_rank = min(
                    range(self.world_size), key=lambda r: rank_load[r]
                )
    ...
    def _report_completion(self, uuid: str, gathered_text: str = None) -> None:
        ...
        if (
            text == ""
            and seq.decoded_tokens is not None
        ...
        for uuid in completed_uuids:
            if uuid in self._uuid_to_local_map:
                ...
                if seq is not None and local_idx in self.query_book:
                    token_ids = (
                        self.query_book[local_idx]
                        .decoded_tokens[0, : seq.decoded_length]
                        .tolist()
                    )
                    try:
                        text = self.tokenizer.decode(token_ids)
    ...
    def _build_sampling_tensors(self, batch_sequences: list) -> tuple:
        ...
        if (
            self._per_sequence_sampling_params is None
            and not has_sequence_params
        ):
            return None, None, None
        ...
        for seq in batch_sequences:
            ...
            if (
                seq_params is None
                and self._per_sequence_sampling_params is not None
            ):
    ...
    def _select_tokens(
        self, logits: torch.Tensor, batch_sequences: Optional[list] = None
    ) -> torch.Tensor:
        """
        ...
        if self._per_sequence_sampling_params is not None or (
            ...
            temps, top_ps, top_ks = self._build_sampling_tensors(
                active_sequences
            )
            if not getattr(self, "_logged_sampling", False) and self.rank == 0:
                logging.info(
                    f"Using PER-SEQUENCE sampling for {logits.shape[0]} sequences"
                )
                self._logged_sampling = True
            if temps is not None:
                return sample_tokens(
                    logits, temperature=temps, top_p=top_ps, top_k=top_ks
                )
        ...
        if self._temperature is None or self._temperature <= 0:
            ...
            if not getattr(self, "_logged_greedy", False) and self.rank == 0:
                logging.debug(
                    f"Using GREEDY decoding (temperature={self._temperature})"
                )
        ...
        if not getattr(self, "_logged_sampling", False) and self.rank == 0:
            logging.info(
                f"Using SAMPLING: temperature={self._temperature}, top_p={self._top_p}"
            )
            self._logged_sampling = True
        return sample_tokens(
            logits, temperature=self._temperature, top_p=self._top_p
        )

    def _log_prefill_timing(self):
        """Log prefill timing stats if available (GPT-OSS specific)."""
        try:
            from batchgen.models.openai.gpt_oss_120b.wrappers import (
                PrefillTimingStats,
            )
    ...
    def enable_decode_watchdog(self) -> None:
        ...
        if self._decode_watchdog is not None and hasattr(
            self._decode_watchdog, "_active"
        ):
    ...
    def _get_finish_reason(self, seq) -> str:
        ...
        if seq._rep_detected:
            seq.log_event(
                SeqEvent.COMPLETED, self.rank, "finish_reason=repetition"
            )
    ...
    def _compute_two_page_buffer_tokens(
    ...
    def _allocate_gpu_kv_two_page_buffer(
        self, local_sequence_ids: List[int], load_from_host: bool = True
    ) -> bool:
        """
        ...
        if (
            alloc_details
            and BATCHGEN_CB_DEBUG
            and BATCHGEN_ENABLE_CRITICAL_DIAGS
        ):
            logging.debug(
        ...
        if (
            manager
            and manager._gpu_page_table_manager
            and BATCHGEN_CB_DEBUG
            and BATCHGEN_ENABLE_CRITICAL_DIAGS
        ):
            final_slot_order = (
                list(manager._gpu_page_table_manager.slot_to_seq_id)
                if manager._gpu_page_table_manager.slot_to_seq_id
                else []
            )
            logging.debug(
    ...
    def _select_sequences_for_onhold(
        self, active_uuids: List[str], required_free_pages: int
    ) -> List[str]:
        """
        ...
        for uuid in active_uuids:
            ...
            seq = self.global_batch.get_sequence(uuid)
            candidates.append(
                (uuid, seq.decoded_length, seq.gpu_pages_allocated)
            )

        candidates.sort(
            key=lambda x: (x[1], x[0])
        )  # ascending by decoded_length, then uuid for determinism
    ...
    def _put_sequences_onhold(self, uuids: List[str]) -> None:
        ...
        if manager is not None and manager.is_initialized:
            remaining_uuids = [
                u for u in self._sequences_with_gpu_kv if u not in set(uuids)
            ]
            if remaining_uuids:
                remaining_local = self._get_local_indices_for_uuids(
                    remaining_uuids
                )
                remaining_global = self._local_indices_to_global_seq_ids(
                    remaining_local
                )
    ...
    def _flush_deferred_kv_to_host(self) -> None:
        ...
        sequence_ids, sequence_lengths = (
            batch_info if batch_info is not None else (None, None)
        )
        ...
        _use_uva_kernel = (
            os.environ.get("BATCHGEN_KV_OFFLOAD_UVA_KERNEL", "1") == "1"
        )

        if _use_uva_kernel and hasattr(
            worker_view, "async_append_decode_kv_to_host_batched_kernel"
        ):
    ...
    def _ensure_host_kv_append_capacity(
        ...
        for global_idx, write_pos in zip(sequence_ids, sequence_lengths):
            ...
            if seq is None:
                raise RuntimeError(
                    f"host KV append for unknown global_idx={global_idx}"
                )
            if int(write_pos) < 0:
                raise RuntimeError(
            ...
            if required_tokens > int(seq.host_token_capacity):
                growth_pages = math.ceil(
                    (required_tokens - int(seq.host_token_capacity))
                    / seq.PAGE_SIZE
                )
                grow_requests.append((int(global_idx), growth_pages))
                grow_metadata.append(
                    (
                        seq,
                        growth_pages,
                        int(seq.host_token_capacity),
                        required_tokens,
                    )
                )
    ...
    def _append_decode_kv_to_host_async(
        ...
        if (
            BATCHGEN_ENABLE_NAN_CHECK
            and layer_idx == 0
            and torch.isnan(k_tensor).any()
        ):
            nan_mask = (
                torch.isnan(k_tensor).any(dim=-1).any(dim=-1).any(dim=-1)
            )  # [batch]
    ...
    def _initialize_core_components(self, num_queries: int) -> None:
        ...
        """
        logging.info(
            f"Rank {self.rank}: Performing one-time core initialization"
        )
        ...
        model_max = getattr(
            self.model_config, "max_position_embeddings", 131072
        )
        ...
        self.eos_token_ids = getattr(
            self.tokenizer, "eos_token_ids", {self.eos_token_id}
        )
        self.pad_token_id = getattr(self.tokenizer, "pad_token_id", 0)
        logging.info(
            f"Rank {self.rank}: EOS token IDs set to {self.eos_token_ids}, pad_token_id={self.pad_token_id}"
        )
        ...
        self.global_host_kv_cache_size_gb = (
            self.args.global_host_kv_cache_size_gb
        )
        ...
        if isinstance(self.host_paged_kv_worker_view, DualHostKVCoordinator):
            self.core_engine.host_paged_kv_worker_view = (
                self.host_paged_kv_worker_view.primary
            )
            self.host_paged_kv_worker_view_aux = (
                self.host_paged_kv_worker_view.auxiliary
            )
        else:
            self.core_engine.host_paged_kv_worker_view = (
                self.host_paged_kv_worker_view
        ...
        if self.args.disable_cuda_graphs:
            ...
            glm5_segmented_cuda_graph_requested_for_model(
                ...
                enable_cuda_graph=getattr(
                    self.args, "enable_cuda_graph", False
                ),
            )
        ...
        self.engine_config.EP_Config.enable_offloading = (
            self.args.enable_ep_with_offloading
        )
        self.engine_config.EP_Config.offloading_ratio = (
        ...
        if hasattr(self.model_config, "pre_dequantize_weights"):
            self.model_config.pre_dequantize_weights = (
                self.args.pre_dequantize_weights
            )
        ...
        self.parallel_manager = get_parallel_strategy_manager(
        ...
        logging.info(
            f"Rank {self.rank}: One-time core initialization completed"
        )
    ...
    def _update_batch_config(self, num_queries: int) -> None:
        ...
        self.engine_config.Basic_Config.set_max_prompt_length(
        ...
        logging.info(
            f"Rank {self.rank}: Batch config updated (max_input={self.max_input_length}, max_decode={self.max_decoding_length}, num_queries={num_queries})"
        )
    ...
    def _update_config_after_tokenization(self) -> None:
        ...
        old_max_prompt_length = (
            self.engine_config.Basic_Config.get_max_prompt_length()
        )
        if old_max_prompt_length != self.max_input_length:
            ...
            if (
                hasattr(self, "input_arguments")
                and self.input_arguments is not None
            ):
                self.input_arguments.max_prompt_length = self.max_input_length
                self.input_arguments.padding_length = self.max_input_length
    ...
    def _get_sequence_token_budget(self, sequence_id: int) -> int:
        ...
        if not hasattr(self, "query_book") or self.query_book is None:
            raise RuntimeError(
                "query_book is not initialized before KV allocation"
            )
        ...
        if seq is None:
            raise KeyError(
                f"No sequence metadata available for sequence {sequence_id}"
            )
    ...
    def _compute_host_kv_sequence_tokens(
        ...
        return [
            self._get_sequence_token_budget(sequence_id)
    ...
    def _cuda_graph_page_table_token_capacity(
        ...
        if sequence_tokens:
            candidates.extend(
                int(tokens) for tokens in sequence_tokens if int(tokens) > 0
            )
    ...
    def _with_cuda_graph_page_table_capacity(
        ...
        token_capacity = self._cuda_graph_page_table_token_capacity(
            sequence_tokens
        )
        page_capacity = max(
            1,
        ...
        slot_capacity = max(
            ...
            min(
                int(config.num_pages),
                self._cuda_graph_page_table_slot_capacity(),
            ),
        )
        return replace(
            config,
    ...
    def _ensure_gpu_paged_kv_manager(
        ...
        if aux_config is not None:
            ...
            logging.info(
                ...
                gpu_config.k_head_dim,
                aux_config.num_pages,
                aux_config.k_head_dim,
            )
        else:
            manager = primary
            manager.initialize()
    ...
    def _prepare_gpu_paged_kv_cache(
        self, local_sequence_ids: List[int]
    ) -> None:
        ...
        global_sequence_ids = self._local_indices_to_global_seq_ids(
            local_sequence_ids
        )
        ...
        manager.allocate_pages_for_sequences(
    ...
    def _launch_aux_host_kv_load(self, sequence_tensor: torch.Tensor):
        ...
        if aux_view is None:
            return None
        if not isinstance(
            self.gpu_paged_kv_cache_manager, DualKVCacheCoordinator
        ):
            return None
    ...
    def _prepare_dual_kv_load_pointers(
        ...
        if not isinstance(gpu_manager, DualKVCacheCoordinator):
            raise RuntimeError(
                "DSA dual KV load requires DualKVCacheCoordinator"
            )
        if not new_global_ids:
            raise ValueError(
                "_prepare_dual_kv_load_pointers requires non-empty sequence ids"
            )

        sequence_tensor = torch.tensor(
            new_global_ids, dtype=torch.int64, device="cpu"
        )
        try:
            gpu_manager.rebuild_page_table(new_global_ids)

            primary_order = list(
                gpu_manager.primary._gpu_page_table_manager.slot_to_seq_id
            )
            aux_order = list(
                gpu_manager.auxiliary._gpu_page_table_manager.slot_to_seq_id
            )
            ...
            primary_k, primary_v = (
                gpu_manager.primary.get_padded_3d_page_pointers()
            )
            primary_counts = (
                gpu_manager.primary.export_active_sequence_page_counts()
            )
            aux_k, aux_v = gpu_manager.auxiliary.get_padded_3d_page_pointers()
            aux_counts = (
                gpu_manager.auxiliary.export_active_sequence_page_counts()
            )
    ...
    def _launch_dual_host_kv_load(
        self, pointers: _DualKVLoadPointers
    ) -> DualAsyncKVTask:
        host_view = self.host_paged_kv_worker_view
        if not isinstance(host_view, DualHostKVCoordinator):
            raise RuntimeError(
                "DSA dual KV load requires DualHostKVCoordinator"
            )
    ...
    def _load_host_kv_to_gpu(
        ...
        if worker_view is None:
            raise RuntimeError(
                "Host paged KV worker view is not bound to the core engine"
            )
    ...
    def _destroy_gpu_paged_kv_cache(
        ...
        if self.global_batch is not None:
            ...
            for seq in self.global_batch:
                if (
                    seq.gpu_pages_allocated > 0
                    or seq.had_initial_gpu_reservation
                ):
    ...
    def _get_or_create_gloo_group(self):
        ...
        """
        if (
            not hasattr(self, "_gloo_migration_group")
            or self._gloo_migration_group is None
        ):
            logging.debug(
                f"Rank {self.rank}: Creating Gloo process group for CPU migrations"
            )
    ...
    def _destroy_gloo_group(self):
        ...
        if (
            hasattr(self, "_gloo_migration_group")
            and self._gloo_migration_group is not None
        ):
    ...
    def _get_host_kv_utilization(self) -> Dict[str, int]:
        ...
        node_rank_end = min(
            node_rank_start + NUM_GPUS_PER_NODE, self.world_size
        )
        ...
        for rank_on_node in range(node_rank_start, node_rank_end):
            for status in valid_statuses:
                seqs = self.global_batch.get_sequences_for_rank_with_status(
                    rank_on_node, status
                )
    ...
    def _gather_host_kv_stats_by_node(
        self, worker_view: Optional[object]
    ) -> List[Dict[str, int]]:
        ...
        gathered = [
            torch.zeros_like(stats_tensor) for _ in range(self.world_size)
        ]
    ...
    def _check_host_kv_watermark_trigger(self) -> bool:
        ...
        if self.rank == 0:
            ...
            global_used_percent = (
                int((total_used_pages / total_pages) * 100)
                if total_pages > 0
                else 0
            )
    ...
    def _plan_kv_migration(self) -> List[MigrationOp]:
        ...
        if len(node_stats) <= 1:
            ...
            if self.rank == 0:
                logging.info(
                    "MIGRATION: Single node detected, skipping rebalancing"
                )
            return []
        ...
        if not overloaded or not underutilized:
            ...
            if self.rank == 0:
                logging.info(
                    "MIGRATION: Already balanced, no migrations needed"
                )
            return []
        ...
        migrations = []
        used_by_node = {
            nid: s["num_used_pages"] for nid, s in node_stats.items()
        }
            ...
            while used_by_node[src_node_id] > target_per_node and underutilized:
                ...
                for gpu_offset in range(NUM_GPUS_PER_NODE):
                    ...
                    for status in [
                        ...
                        for uuid in self.global_batch.get_sequences_for_rank_with_status(
                            src_rank, status
                        ):
                ...
                if not candidate_sequences:
                    if self.rank == 0:
                        if BATCHGEN_CB_DEBUG:
                            logging.debug(
                                f"MIGRATION: No more candidates on node {src_node_id}, stopping"
                            )
                    break
        ...
        if len(migration_uuids) != len(set(migration_uuids)):
            duplicate_uuids = [
                u for u in migration_uuids if migration_uuids.count(u) > 1
            ]
            logging.error(
            ...
            if self.rank == 0:
                logging.warning(
                    f"MIGRATION: Removed duplicates, {len(migrations)} unique migrations remain"
                )

        if self.rank == 0:
            if migrations:
                logging.info(
                    f"MIGRATION: Planned {len(migrations)} sequence migrations"
                )
                ...
                if len(migrations) > 5:
                    logging.info(
                        f"MIGRATION:   ... and {len(migrations) - 5} more"
                    )
            else:
    ...
    def _execute_kv_migrations_parallel(
        ...
        if self.rank == 0:
            logging.info(
                f"MIGRATION: Executing {len(migrations)} migrations in {len(rounds)} parallel rounds"
            )

        for round_idx, round_migrations in enumerate(rounds):
            if self.rank == 0:
                logging.info(
                    f"MIGRATION: Round {round_idx + 1}/{len(rounds)}: {len(round_migrations)} parallel migrations"
                )
        ...
        if self.rank == 0:
            logging.info(
                f"MIGRATION: All {len(rounds)} parallel rounds completed"
            )

    def _group_migrations_for_parallel_execution(
        self, migrations: List[MigrationOp]
    ...
    def _execute_single_kv_migration(
        self, uuid: str, from_rank: int, to_rank: int
    ) -> None:
        ...
        if seq is None:
            logging.error(
                f"Rank {self.rank}: Cannot migrate {uuid[:8]}... - sequence not found"
            )
            return
        ...
        if pages_needed <= 0:
            logging.error(
                f"Rank {self.rank}: Cannot migrate {uuid[:8]}... - no host pages allocated"
            )
            return
        ...
        if self.rank == from_rank:
            ...
            k_cpu_aux = (
                aux_view.read_sequence_kv_to_cpu(global_idx)[0]
            ...
            if v_cpu.numel() > 0:
                dist.send(
                    tensor=v_cpu.contiguous(), dst=to_rank, group=gloo_group
                )
            if k_cpu_aux is not None:
                dist.send(
                    tensor=k_cpu_aux.contiguous(), dst=to_rank, group=gloo_group
                )
            t_send = time.perf_counter()
            if BATCHGEN_CB_DEBUG:
                logging.debug(
                    f"MIGRATION: Rank {self.rank}: Gloo send: {(t_send - t_read) * 1000:.1f}ms"
                )
            ...
            if local_idx is not None and local_idx in self.query_book:
                ...
                dist.send(
                    tensor=qb.encoded["input_ids"].clone(),
                    dst=to_rank,
                    group=gloo_group,
                )
                dist.send(
                    tensor=qb.decoded_tokens.clone(),
                    dst=to_rank,
                    group=gloo_group,
                )
                ...
                if (
                    hasattr(seq_for_slot, "_buffer_slot")
                    and seq_for_slot._buffer_slot >= 0
                ):
                    self._buffer_pool.free_slot(seq_for_slot._buffer_slot)
                    seq_for_slot._buffer_slot = -1
                if BATCHGEN_CB_DEBUG:
                    logging.debug(
                        f"MIGRATION: Rank {self.rank}: Sent query_book for {uuid[:8]}..."
                    )
            else:
                logging.warning(
                    f"MIGRATION: Rank {self.rank}: No query_book entry for {uuid[:8]}... (local_idx={local_idx})"
                )
            ...
            if aux_view is not None:
                ...
                aux_view.write_sequence_kv_from_cpu(
                    global_idx, k_recv_aux, None
                )
            logging.info(
            ...
            input_ids_recv = torch.empty(
                input_ids_shape, dtype=seq.input_ids.dtype, device="cpu"
            )
            decoded_tokens_recv = torch.empty(
                decoded_tokens_shape,
                dtype=seq.decoded_tokens.dtype,
                device="cpu",
            )
            dist.recv(tensor=input_ids_recv, src=from_rank, group=gloo_group)
            dist.recv(
                tensor=decoded_tokens_recv, src=from_rank, group=gloo_group
            )
    ...
    def _rebalance_host_kv(self) -> None:
        ...
        if not migrations:
            if self.rank == 0:
                logging.info(
                    "REBALANCE: No migrations needed, host KV already balanced"
                )
            return
            ...
            try:
                ...
                if seq_for_log:
                    ...
                    if old_rank == self.rank:
                        seq_for_log.log_event(
                            SeqEvent.MIGRATE_SEND,
                            self.rank,
                            f"to_rank={new_rank}",
                        )
                    elif new_rank == self.rank:
                        seq_for_log.log_event(
                            SeqEvent.MIGRATE_RECV,
                            self.rank,
                ...
                logging.error(
                    f"Rank {self.rank}: Cannot update ownership for {uuid[:8]}... - sequence not found"
                )
                continue
        ...
        for mig in migrations:
            ...
            if self.rank == new_rank:
                ...
                if (
                    hasattr(self, "_pending_migrated_query_book")
                    ...
                    if existing_slot < 0:
                        logging.info(
                            f"Rank {self.rank}: Migration receive {uuid[:8]} has no buffer slot (expected for cross-rank migration), allocating new"
                        )
                        existing_slot = self._buffer_pool.allocate_slot()
                        seq._buffer_slot = existing_slot
                    self._buffer_pool.input_ids_buffer[
                        existing_slot, :budget
                    ] = pending["input_ids"][0, :budget]
                    self._buffer_pool.decoded_tokens_buffer[
                        existing_slot, :
                    ] = pending["decoded_tokens"][0, :]
                    input_ids_view = self._buffer_pool.get_input_ids_view(
                        existing_slot, budget
                    )
                    decoded_view = self._buffer_pool.get_decoded_tokens_view(
                        existing_slot
                    )
                    seq.input_ids = input_ids_view
                    ...
                    logging.debug(
                        f"Rank {self.rank}: Created query_book[{new_local_idx}] for migrated {uuid[:8]}..."
                    )
                else:
                    logging.error(
                        f"Rank {self.rank}: No pending query_book data for migrated {uuid[:8]}..."
                    )

                logging.debug(
                    f"Rank {self.rank}: Added {uuid[:8]}... to local mappings (new local_idx={new_local_idx})"
                )
    ...
    def process_new_batch(
        ...
        for idx, text in enumerate(global_prompts):
            ...
            if per_sequence_max_tokens is not None and idx < len(
                per_sequence_max_tokens
            ):
                max_dec = (
                    per_sequence_max_tokens[idx]
                    if per_sequence_max_tokens[idx] is not None
                    else self.max_decoding_length
                )
            seq = SequenceEntry(
                uuid=f"seq_{idx}",
                global_idx=idx,
            ...
            if self._per_sequence_sampling_params is not None and idx < len(
                self._per_sequence_sampling_params
            ):
        ...
        local_batch_size = torch.tensor(
            [len(self.global_batch)],
            dtype=torch.int64,
            device=self.torch_device,
        )
        all_sizes = [
            torch.zeros_like(local_batch_size) for _ in range(self.world_size)
        ]
        ...
        logging.info(
            f"Rank {self.rank}: All ranks have {all_sizes_list[0]} sequences in global_batch"
        )
        ...
        with self.disable_watchdog():
            ...
            logging.info(
                f"Rank {self.rank}: [INIT TIMING] Step 2 _tokenize_global_batch: {time.perf_counter() - t_step:.2f}s"
            )
            ...
            if len(self.global_batch) == 0:
                logging.info(
                    f"Rank {self.rank}: All sequences rejected. Skipping inference."
                )
            ...
            self._init_incremental_writer()
            logging.info(
                f"Rank {self.rank}: [INIT TIMING] Step 2.1 _init_incremental_writer: {time.perf_counter() - t_step:.2f}s"
            )

            # Step 2.15: Write rejection errors via incremental writer
            if (
                self.rank == 0
                and self._incremental_writer
                and self._rejected_sequences
            ):
                for global_idx, prompt_length in self._rejected_sequences:
                    self._incremental_writer.submit_error(
                        global_idx,
                ...
                logging.info(
                    f"Rank 0: Wrote {len(self._rejected_sequences)} rejection errors to incremental output"
                )
            ...
            self._update_config_after_tokenization()
            logging.info(
                f"Rank {self.rank}: [INIT TIMING] Step 2.5 _update_config_after_tokenization: {time.perf_counter() - t_step:.2f}s"
            )
            ...
            logging.info(
                f"Rank {self.rank}: [INIT TIMING] Step 3 _assign_sequences_to_ranks: {time.perf_counter() - t_step:.2f}s"
            )
            ...
            logging.info(
                f"Rank {self.rank}: [INIT TIMING] Step 4 _build_local_query_book: {time.perf_counter() - t_step:.2f}s"
            )

            # Step 5: Set counts for compatibility
            self.num_global_queries = len(global_prompts)
            self.num_local_queries = len(
    ...
    def _local_indices_to_global_seq_ids(
        self, local_indices: List[int]
    ) -> List[int]:
    ...
    def _get_my_sequences_by_status(self, status: SequenceStatus) -> List[str]:
        ...
        return self.global_batch.get_sequences_for_rank_with_status(
            self.rank, status
        )
    ...
    def _update_batch_status(
        self, uuids: List[str], new_status: SequenceStatus
    ):
        ...
        for uuid in uuids:
            ...
            if seq is None:
                logging.warning(
                    f"Rank {self.rank}: Sequence {uuid} not found in global_batch"
                )
                continue
            ...
            try:
                ...
                logging.warning(
                    f"Rank {self.rank}: Invalid status transition for {uuid}: {e}"
                )
        ...
        for uuid in decode_uuids:
            if uuid in self._uuid_to_local_map:
                ...
                seq.validate_metadata(
                    f"rank {self.rank} _sync_sequence_metadata/send"
                )
                ...
                for uuid, state in rank_state.items():
                    if uuid not in self._uuid_to_local_map:
                        ...
                        if seq is not None:
                            ...
                            seq.current_context_length = state[
                                "current_context_length"
                            ]
                            seq.gpu_pages_allocated = state[
                                "gpu_pages_allocated"
                            ]
        ...
        for global_idx in sorted(idx_to_uuid.keys()):
            ...
            if completion_tensor[global_idx].item() == 1:
                ...
                if seq is not None:
                    ...
                    if seq.status != SequenceStatus.COMPLETED:
                        try:
                            self.global_batch.update_status(
                                uuid, SequenceStatus.COMPLETED
                            )
                        except ValueError as e:
                            logging.debug(
    ...
    def _tokenize_global_batch(self) -> None:
        ...
        logging.debug(
            f"Rank {self.rank}: Local tokenization of {len(my_texts)} sequences in {local_tokenize_time:.2f}s"
        )

        # DEBUG: Print tokenized prompts
        if (
            os.environ.get("BATCHGEN_DEBUG_TOKENIZE", "0") == "1"
            and self.rank == 0
            and my_tokenized
        ):
            ...
            for i in range(min(3, len(my_tokenized))):
                ...
                try:
                    ...
                    print(
                        f"[TOKENIZE DEBUG] Start of prompt (decoded): {repr(decoded_start[:300])}"
                    )
                    print(
                        f"[TOKENIZE DEBUG] End of prompt (decoded): {repr(decoded_end[-300:])}"
                    )
                ...
                found_special = [
                    tid for tid in token_ids if tid in special_token_ids
                ]
                if found_special:
                    print(
                        f"[TOKENIZE DEBUG] Special tokens found: {found_special}"
                    )
        ...
        prompt_lengths = [
            tokenized_by_idx[i]["length"] for i in range(num_sequences)
        ]
        ...
        if num_sequences > 0:
            remaining_lengths = [
                tokenized_by_idx[seq.global_idx]["length"]
                for seq in self.global_batch
            ]
        ...
        if num_sequences == 0:
            logging.info(
                f"Rank {self.rank}: All sequences rejected, skipping Phase 3 buffer allocation"
            )
            return
        ...
        pool_capacity = (
            max(num_seqs, self._max_pool_size)
    ...
    def _assign_sequences_to_ranks(self) -> None:
        ...
        sequences.sort(
            key=lambda s: s.prompt_length + s.max_decode_length, reverse=True
        )
        ...
        if self.rank == 0:
            imbalance = (
                (max(rank_tiles) - min(rank_tiles)) / max(rank_tiles) * 100
                if max(rank_tiles) > 0
                else 0
            )
            logging.info(
    ...
    def _build_local_query_book(self) -> None:
        ...
        self._next_local_idx = len(
            my_uuids
        )  # Next available index after initial assignment
    ...
    def _get_effective_chunk_size(self) -> int:
        ...
        chunk = (
            math.ceil(chunk / SequenceEntry.PAGE_SIZE) * SequenceEntry.PAGE_SIZE
        )
        return chunk
    ...
    def _prepare_prefill_batch(self) -> List[str]:
        ...
        if self.enable_host_kv_eviction:
            evicted_uuids = self.global_batch.get_sequences_by_status(
        ...
        queueing_uuids = self.global_batch.get_sequences_by_status(
        ...
        gathered = [
            torch.zeros_like(free_tensor) for _ in range(self.world_size)
        ]
        ...
        if self.rank == 0:
            logging.info(
                f"Per-node host KV free pages: {per_node_host_free} (chunk_size={chunk_size})"
            )
        ...
        for uuid in all_candidates:
            ...
            initial_capacity = max(
                seq.prompt_length + chunk_size, gpu_initial_tokens
            )
            ...
            if (
                ...
                <= per_node_effective_free[seq_node]
            ):
        ...
        if self.rank == 0:
            n_evicted = sum(
                1
                for u in prefill_batch
                if self.global_batch.get_sequence(u).status
                == SequenceStatus.EVICTED
            )
            logging.info(
    ...
    def _put_sequences_on_hold(self, uuids: List[str]) -> None:
        ...
        if self.rank == 0:
            logging.info(f"[WATERMARK] Putting {len(uuids)} sequences ON_HOLD")
        ...
        if (
            hasattr(self, "gpu_paged_kv_cache_manager")
            and self.gpu_paged_kv_cache_manager
        ):
            global_seq_ids = []
            for uuid in uuids:
                seq = self.global_batch.get_sequence(uuid)
    ...
    def _prepare_decode_batch(self) -> List[str]:
        ...
        onhold_uuids = self.global_batch.get_sequences_by_status(
            SequenceStatus.ON_HOLD
        )
        ...
        if (
            self.gpu_paged_kv_cache_manager is None
            or not self.gpu_paged_kv_cache_manager.is_initialized
        ):
            raise RuntimeError(
        ...
        total_pages = (
            self.gpu_paged_kv_cache_manager.get_stats().num_total_pages
        )
    ...
    def _check_and_extend_page_buffer(
        self, decode_uuids: List[str], batch: List[int]
        ...
        for uuid in decode_uuids:
            ...
            if (
                expected_owner == self.rank
                and uuid not in self._uuid_to_local_map
            ):
                should_be_mine_but_missing.append(
                    (uuid, seq.global_idx, seq.assigned_rank)
                )
        ...
        if should_be_mine_but_missing:
            ...
            logging.error(
                f"Rank {self.rank}: _uuid_to_local_map has {len(self._uuid_to_local_map)} entries"
            )
        ...
        logging.info(
            f"Rank {self.rank}: Per-rank reported sequences: {per_rank_reported}, total decode_uuids={len(decode_uuids)}"
        )
        ...
        if missing_uuids:
            ...
            for uuid in missing_uuids:
                ...
                missing_by_expected_owner[expected_owner].append(
                    (uuid, seq.global_idx, actual_assigned)
                )
            logging.error(
                f"Rank {self.rank}: Missing sequences by expected owner: {[(k, len(v)) for k, v in missing_by_expected_owner.items()]}"
            )
        ...
        per_rank_free = {
            r: int(gathered_free[r].item()) for r in range(self.world_size)
        }
        ...
        for uuid in decode_uuids:
            if uuid not in global_seq_info:
                logging.error(
                    f"Rank {self.rank}: MISSING uuid={uuid} from global_seq_info"
                )
                missing_from_global_info.append(uuid)
                continue
            seq = self.global_batch.get_sequence(uuid)
            info = global_seq_info[uuid]
            seqs_by_rank[seq.assigned_rank].append({"uuid": uuid, **info})
        ...
        for r in range(self.world_size):
            rank_additional = sum(
                s["additional_needed"] for s in seqs_by_rank[r]
            )
        ...
        )  # Include missing sequences in onhold
        ...
        if all_can_extend:
            ...
            my_uuids_needing_extension = [
                ...
                for uuid in decode_uuids
                if uuid in self._uuid_to_local_map
                and global_seq_info.get(uuid, {}).get("additional_needed", 0)
                > 0
            ]

            if my_uuids_needing_extension:
                success = self._extend_gpu_kv_allocation(
                    my_uuids_needing_extension
                )
                if not success:
                    logging.error(
                        f"Rank {self.rank}: Extension FAILED unexpectedly in no-eviction path"
                    )
            ...
            for r in seqs_by_rank:
                seqs_by_rank[r].sort(
                    key=lambda x: x["decoded_length"], reverse=True
                )
            ...
            my_onhold = [
                u for u in global_onhold if u in self._uuid_to_local_map
            ]

            if my_onhold:
                local_indices = self._get_local_indices_for_uuids(my_onhold)
                global_ids = self._local_indices_to_global_seq_ids(
                    local_indices
                )
            ...
            if my_remaining_needing_extension:
                ...
                if not success:
                    logging.error(
                        f"Rank {self.rank}: Extension FAILED - putting sequences ON_HOLD"
                    )
        ...
        for local_idx in active_batch:
            ...
            if seq.gpu_pages_allocated == 0:
                logging.error(
                    f"Rank {self.rank}: REMOVING uuid={uuid} - gpu_pages_allocated=0"
                )
                is_valid = False

            if uuid not in self._sequences_with_gpu_kv:
                logging.error(
                    f"Rank {self.rank}: REMOVING uuid={uuid} - not in _sequences_with_gpu_kv"
                )
                is_valid = False
        ...
        local_active_count = torch.tensor(
            [len(active_uuids)], dtype=torch.int64, device=self.torch_device
        )
        all_active_counts = [
            torch.zeros_like(local_active_count) for _ in range(self.world_size)
        ]
        ...
        if len(set(counts)) > 1:
            logging.error(
                f"Rank {self.rank}: DIVERGENCE! active_uuids counts differ across ranks: {counts}"
            )
    ...
    def _check_and_handle_completions(
        ...
        rep_flags = torch.tensor(
            [seqs[i]._rep_detected for i in range(n)], dtype=torch.bool
        )
        completed_mask = (
                ...
                if seq is not None and local_idx in self.query_book:
                    ...
                    my_completed_tokens.append(
                        (
                            seq.global_idx,
                            self.query_book[local_idx]
                            .decoded_tokens[:, : seq.decoded_length]
                            .clone(),
                            finish_reason,
                        )
                    )
            ...
            for rank_tokens in all_completed_tokens:
                if rank_tokens:
                    for global_idx, tokens, finish_reason in rank_tokens:
                        writer.submit(
                            global_idx, tokens, finish_reason=finish_reason
                        )

    def _try_load_new_sequences(
        self, current_decode_uuids: List[str], current_local_indices: List[int]
        ...
        candidates = self.global_batch.get_sequences_by_status(
            SequenceStatus.PREFILLED
        )
        ...
        if self.gpu_paged_kv_cache_manager is not None and updated_batch:
            all_global_ids = self._local_indices_to_global_seq_ids(
                updated_batch
            )
    ...
    def _allocate_and_load_gpu_kv_for_new_sequences(
        self, local_sequence_ids: List[int]
    ) -> None:
        """
        ...
        if manager is None:
            logging.warning(
                "GPU KV manager not initialized, cannot load new sequences"
            )
            return
    ...
    def _nsys_decode_profile_end_forward(
        self, forward_idx: Optional[int]
    ) -> None:
    ...
    def generate(self):
        ...
        if not dist.is_initialized():
            raise RuntimeError(
                f"Rank {self.rank}: torch.distributed not initialized (should have been verified in _reset_for_new_batch)"
            )
        if not self._check_and_reinit_pynccl():
            raise RuntimeError(
                f"Rank {self.rank}: Failed to ensure healthy PyNccl communicator"
            )
        ...
        if os.getenv("BATCHGEN_ENABLE_ALL_TO_ALL", "0") == "0":
            ...
            if self.world_size == 1:
                logging.debug(
                    "Single GPU mode: skipping PyNccl communicator initialization"
                )
            else:
                ...
                need_init_tensor = torch.tensor(
                    [need_init], dtype=torch.int32, device=self.torch_device
                )
                ...
                if any_rank_needs_init:
                    ...
                    if self.comm is not None:
                        logging.info(
                            f"Rank {self.rank}: Destroying existing comm for coordinated reinit"
                        )
                        try:
                            self.comm.destroy()
                        except Exception:
                        ...
                        self.comm = None
                        if (
                            hasattr(self, "_nccl_group")
                            and self._nccl_group is not None
                        ):
                    ...
                    if comm_master_addr is None:
                        logging.warning(
                            f"Rank {self.rank}: COMM_MASTER_ADDR not set, skipping PyNccl init"
                        )
                    elif (
                        StatelessProcessGroup is not None
                        and PyNcclCommunicator is not None
                    ):
                        ...
                        if self.rank == 0:
                            try:
                                self._nccl_port = _find_available_port(
                                    comm_master_addr, self._nccl_port
                                )
                                logging.debug(
                                    f"Rank 0: Found available port {self._nccl_port} for PyNccl"
                                )
                            except RuntimeError as e:
                                logging.error(
                                    f"Rank 0: Failed to find available port: {e}"
                                )
                                raise
                        ...
                        logging.debug(
                            f"Rank {self.rank}: Waiting for all ranks before PyNccl init..."
                        )
                        dist.barrier()

                        try:
                            logging.debug(
                                f"Rank {self.rank}: Creating PyNccl communicator on port {self._nccl_port}"
                            )
                            ...
                            self.comm = PyNcclCommunicator(
                                group=self._nccl_group, device=device
                            )
                            # Only rank 0 logs at INFO level to reduce verbosity
                            if self.rank == 0:
                                logging.info(
                                    f"PyNccl communicator initialized on port {self._nccl_port}"
                                )
                            else:
                                logging.debug(
                                    f"Rank {self.rank}: PyNccl communicator initialized on port {self._nccl_port}"
                                )
                        except Exception as e:
                            logging.error(
                                f"Rank {self.rank}: PyNccl communicator initialization failed - {e}"
                            )
                            raise RuntimeError(
                                f"Rank {self.rank}: PyNccl communicator initialization failed - {e}"
                            )
        ...
        while True:
            ...
            if self._admission_queue is not None:
                ...
                if admitted and self.rank == 0:
                    logging.info(
                        f"[POOL] Admitted new sequences, total in batch: {len(self.global_batch)}"
                    )
            ...
            if self.global_batch.all_completed():
                ...
                if not self._timing_logged and self.rank == 0:
                    ...
                    total_prompt = sum(
                        s.prompt_length for s in self.global_batch
                    )
                    total_decoded = sum(
                        s.decoded_length for s in self.global_batch
                    )
                    num_seq = len(self.global_batch)
                    pf_tp = (
                        total_prompt / prefill_time if prefill_time > 0 else 0
                    )
                    dc_tp = (
                        total_decoded / decoding_time
                        if decoding_time > 0
                        else 0
                    )
                    ov_tp = (
                        (total_prompt + total_decoded) / gen_time
                        if gen_time > 0
                        else 0
                    )
                    logging.info(
                ...
                if self.rank == 0:
                    try:
                        ...
                        if msg is None:
                            ...
                            isinstance(msg, dict) and msg.get("type") == "admit"
                        ):
                    ...
                    status = torch.tensor(
                        [0, 0, 0], dtype=torch.int32, device=self.torch_device
                    )
                    dist.broadcast(status, src=0)
                    has_new = status[0].item() == 1
                ...
                if (
                    self.global_batch.all_completed()
            ...
            # =================================================================
            if self.global_batch.has_queueing() or (
                self.enable_host_kv_eviction and self.global_batch.has_evicted()
            ):
                dist.barrier()
                ...
                on_hold_uuids = [
                    seq.uuid
                    for seq in self.global_batch
                    if seq.status == SequenceStatus.ON_HOLD
                ]
                in_decode_uuids = [
                    seq.uuid
                    for seq in self.global_batch
                ...
                if prefill_uuids:
                    if self.rank == 0:
                        logging.info(
                            f"[PREFILL] Starting for {len(prefill_uuids)} sequences"
                        )
                    for uuid in prefill_uuids:
                        seq = self.global_batch.get_sequence(uuid)
                        is_reentry = seq.evicted_token_ids is not None
                        seq.log_event(
                            SeqEvent.PREFILL_START,
                            self.rank,
                            f"evicted_reentry={is_reentry}",
                        )
                    self._update_batch_status(
                        prefill_uuids, SequenceStatus.IN_PREFILL
                    )
                    ...
                    local_prefill_indices = self._get_local_indices_for_uuids(
                        prefill_uuids
                    )
                    ...
                    if local_prefill_indices:
                        ...
                        from batchgen.models.wrappers.attention import (
                            AttnWrapperBase as _AWB,
                        )
                    ...
                    for uuid in prefill_uuids:
                        seq = self.global_batch.get_sequence(uuid)
                        seq.log_event(
                            SeqEvent.PREFILL_DONE,
                            self.rank,
                            f"decoded_len={seq.decoded_length}",
                        )
                    self._update_batch_status(
                        prefill_uuids, SequenceStatus.PREFILLED
                    )
                    dist.barrier()
            ...
            while (
                ...
                total_candidates = (
                    prefilled_count + onhold_count + in_decode_count
                )
                # Estimate max per rank (ceiling division)
                max_num_seq_estimate = (
                    total_candidates + self.world_size - 1
                ) // self.world_size
                ...
                if torch.cuda.is_available():
                    ...
                    allocated = (
                        torch.cuda.memory_allocated(self.local_rank) / 1e9
                    )
                    logging.info(
                ...
                current_decoding = self.global_batch.get_sequences_by_status(
                    SequenceStatus.IN_DECODE
                )
                ...
                if global_completed:
                    ...
                    my_completed = [
                        u
                        for u in completed_list
                        if u in self._uuid_to_local_map
                    ]
                    if my_completed:
                        ...
                        gpu_allocated = [
                            u
                            for u in my_completed
                            if u in self._sequences_with_gpu_kv
                        ]
                        if gpu_allocated:
                    ...
                    for uuid in completed_list:
                        ...
                        if (
                            seq is not None
                            and seq.status == SequenceStatus.COMPLETED
                        ):
                            self._report_completion(
                                uuid, gathered_text=gathered_texts.get(uuid)
                            )
                ...
                for uuid in decode_uuids:
                    ...
                    prev_status = (
                        "ON_HOLD"
                        if seq.had_initial_gpu_reservation
                        else "PREFILLED"
                    )
                    seq.log_event(
                        SeqEvent.DECODE_START, self.rank, f"from={prev_status}"
                    )
                ...
                local_decode_indices = self._get_local_indices_for_uuids(
                    decode_uuids
                )
                ...
                AttnWrapperBase.batchgen_debug = (
                    self._active_batchgen_debug_for_sequences(
                        global_decode_sequences
                    )
                )
                ...
                # B. Config Decode
                config_start = time.perf_counter()
                self._config_decoding_for_batch(
                    decode_uuids, local_decode_indices
                )
                ...
                self._update_batch_status(
                    decode_uuids, SequenceStatus.IN_DECODE
                )
                self._sync_sequence_metadata(decode_uuids)
                ...
                generic_cuda_graph_warmup_needed = (
                    should_warmup_cuda_graphs_before_decode(
                        graph_manager_is_initialized=self._cuda_graph_manager
                        is not None,
                ...
                if (
                    ...
                    or (self._glm5_segmented_graph_initial_capture_missing())
                ...
                with torch.inference_mode():
                    if local_decode_indices:
                        new_tokens = self._rebuild_input_tokens(
                            local_decode_indices
                        )
                    else:
                    ...
                    self.decoding_continuous(
                        new_tokens, decode_uuids, local_decode_indices
                    )
                ...
                if self._admission_queue is not None:
                    ...
                    if admitted and self.rank == 0:
                        logging.info(
                            f"[DECODE] Mid-cycle admission, total in batch: {len(self.global_batch)}"
                        )
                ...
                has_pending = self.global_batch.has_queueing() or (
                    self.enable_host_kv_eviction
                    and self.global_batch.has_evicted()
                )
                needs_prefill = (
                    has_pending and self._check_host_kv_watermark_trigger()
                )
                if needs_prefill:
                    if self.rank == 0:
                        num_queued = len(
                        ...
                        logging.info(
                            f"[DECODE] Breaking for prefill (watermark) - {num_queued} queued, {num_evicted} evicted"
                        )
                    in_decode_uuids = [
                        u
                        for u in decode_uuids
                        if self.global_batch.get_sequence(u).status
                        == SequenceStatus.IN_DECODE
                    ]
                    ...
                    if (
                        ...
                        logging.info(
                            f"[MULTI_DIAG] Putting {len(in_decode_uuids)} seqs ON_HOLD (decode_group={self._decode_group_idx})"
                        )
                    if in_decode_uuids:
        ...
        decode_throughput = (
            total_decoded_tokens / decoding_time if decoding_time > 0 else 0
        )
        ...
        local_results = []
        # Gather from global_batch ownership rather than _local_to_uuid_map /
        # query_book: the continuous-decode path pops completed sequences out of
        # those local maps before this finalize runs, so they are empty here.
        # global_batch retains the owned sequences and each carries its own
        # decoded_tokens buffer-pool view + decoded_length.
        my_uuids = (
            self.global_batch.get_sequences_for_rank(self.rank)
        ...
        for uuid in my_uuids:
            ...
            if seq is None:
                continue
            decoded_view = getattr(seq, "decoded_tokens", None)
            if decoded_view is None or seq.decoded_length <= 0:
                logging.warning(
                    f"Rank {self.rank}: no decoded tokens for uuid={uuid[:8]}... "
                    f"(decoded_length={getattr(seq, 'decoded_length', None)})"
                )
                continue
            decoded_tokens = decoded_view[:, : seq.decoded_length]
            decoded_str = self._decode_tokens_to_string(decoded_tokens)
            local_results.append((seq.global_idx, decoded_str))

        all_results = [None] * self.world_size
        dist.all_gather_object(all_results, local_results)
        all_results = [item for sublist in all_results for item in sublist]
        result_dict = {
            global_idx: decoded_str for global_idx, decoded_str in all_results
        }

        if self.rank == 0:
            logging.info(
                f"Detokenization complete: {len(result_dict)} sequences (distributed across {self.world_size} ranks)"
            )
    ...
    def _decode_tokens_to_string(
        self, tokens: torch.Tensor, min_tokens: int = 1
    ) -> str:
        ...
        eos_positions = [
            ...
            if t in self.eos_token_ids and i >= min_tokens
        ]
        ...
        if eos_positions:
            ...
            non_pad = [
                i for i, t in enumerate(tokens_list) if t != self.pad_token_id
            ]
        ...
        return self.tokenizer.decode(
            tokens_list[:end_pos],
            skip_special_tokens=(
                not self.detokenization_include_special_tokens
            ),
        )
    ...
    def _config_prefill_for_batch(self, prefill_uuids: List[str]) -> None:
        ...
        in_decode = self.global_batch.get_sequences_by_status(
            SequenceStatus.IN_DECODE
        )
        on_hold = self.global_batch.get_sequences_by_status(
            SequenceStatus.ON_HOLD
        )
        prefilling = self.global_batch.get_sequences_by_status(
            SequenceStatus.PREFILLED
        )
        ...
        if (
            hasattr(self, "_pending_kv_append_tasks")
            and self._pending_kv_append_tasks
        ):
            logging.info(
        ...
        for uuid in prefill_uuids:
            ...
            self._buffer_pool.input_ids_buffer[slot, :new_prompt_len] = (
                evicted_ids
            )
            seq.input_ids = self._buffer_pool.get_input_ids_view(
                slot, seq_extended_size
            )

            # Pre-fill decoded_tokens with previously decoded tokens (Q1/Q2)
            # so the final decoded_tokens contains the COMPLETE response.
            self._buffer_pool.decoded_tokens_buffer[slot, :] = (
                self._buffer_pool.pad_token_id
            )
            seq.decoded_tokens = self._buffer_pool.get_decoded_tokens_view(slot)
            if prev_decoded > 0:
            ...
            if (
                seq.assigned_rank == self.rank
                and uuid in self._uuid_to_local_map
            ):
                local_idx = self._uuid_to_local_map[uuid]
                self.query_book[local_idx] = make_query_book_entry(seq)
        ...
        for uuid in prefill_uuids:
            ...
            if seq.assigned_rank == self.rank:
                ...
                if uuid not in self._uuid_to_local_map:
                    new_local_idx = self._bind_local_sequence_to_query_book(
                        uuid
                    )
                    logging.debug(
        ...
        if my_prefill_uuids:
            ...
            for uuid in my_prefill_uuids:
                ...
                post_prefill_length = (
                    seq.prompt_length + 1
                )  # prefill produces 1 decode token
                gpu_initial_pages = (
                    math.ceil(post_prefill_length / seq.PAGE_SIZE)
                    + INITIAL_GPU_PAGE_BUFFER
                )
                ...
                initial_capacity = min(initial_capacity, seq.kv_token_budget)
                seq.host_pages_allocated = math.ceil(
                    initial_capacity / seq.PAGE_SIZE
                )
                seq.host_token_capacity = (
                    seq.host_pages_allocated * seq.PAGE_SIZE
                )
            ...
            total_pages_needed = sum(
                math.ceil(t / seq.PAGE_SIZE) for t in sequence_tokens
            )
            if total_pages_needed > kv_stats.num_free_pages:
                # Log per-sequence breakdown to help diagnose the selection bug.
                seq_details = []
                for gid, tokens in list(
                    zip(global_sequence_ids, sequence_tokens)
                )[:10]:
                    s = self.global_batch.get_sequence(
                        next(
                            u
                            for u in my_prefill_uuids
                            if self.global_batch.get_sequence(u).global_idx
                            == gid
                        )
                    )
                    seq_details.append(
            ...
            if self.rank == 0:
                logging.info(
                    f"[PREFILL] Host KV allocated: {kv_stats.num_used_pages}/{kv_stats.num_total_pages} pages"
                )

        if self.rank == 0:
            logging.info(
                f"[PREFILL] Config completed: {(time.perf_counter() - start_time) * 1000:.1f}ms"
            )
    ...
    def _get_deepseek_v4_dense_rope_cache(self):
        cache = getattr(self, "_deepseek_v4_dense_rope_cache", None)
        ...
        rope_head_dim = int(getattr(self.model_config, "qk_rope_head_dim", 64))
        theta = float(getattr(self.model_config, "rope_theta", 10000.0))
        max_pos = int(getattr(self, "model_context_length", 0) or 0)
        if max_pos <= 0:
            max_pos = int(
        ...
        self._deepseek_v4_dense_rope_cache = cache
        return cache

    def _deepseek_v4_compress_rope_params(self):
        rope_head_dim = int(getattr(self.model_config, "qk_rope_head_dim", 64))
        theta = float(
            getattr(self.model_config, "compress_rope_theta", 160000.0)
        )
        max_pos = int(getattr(self, "model_context_length", 0) or 0)
        if max_pos <= 0:
            max_pos = int(
        ...
        scaling = getattr(self.model_config, "rope_scaling", None) or {}
    ...
    def _get_deepseek_v4_compressed_rope_cache(self):
        cache = getattr(self, "_deepseek_v4_compressed_rope_cache", None)
        if cache is not None:
            return cache
        from batchgen.attention.dsa.v4_flashmla_adapter import (
            build_v4_rope_cache,
        ...
        cache = build_v4_rope_cache(device=self.torch_device, **params)
        self._deepseek_v4_compressed_rope_cache = cache
        return cache

    def _get_deepseek_v4_compressed_rope_tables(self):
        tables = getattr(self, "_deepseek_v4_compressed_rope_tables", None)
        ...
        params = self._deepseek_v4_compress_rope_params()
        tables = build_v4_rope_tables(device=self.torch_device, **params)
        self._deepseek_v4_compressed_rope_tables = tables
        return tables

    def _prepare_deepseek_v4_decode_metadata_for_forward(
        self, gpu_manager
    ) -> None:
        if not self._is_deepseek_v4_kv_manager(gpu_manager):
            return
        backend = getattr(self, "_deepseek_v4_decode_backend", None)
        if backend is None:
            return
        from batchgen.models.wrappers import AttnWrapperBase

        sequence_ids = list(AttnWrapperBase.cur_batch or [])
        if not sequence_ids:
        ...
        page_tables = gpu_manager.rebuild_page_table(sequence_ids)
        positions = AttnWrapperBase.position_ids
        if positions is not None:
            positions = positions.view(-1).to(torch.int32)
        cache_seqlens = AttnWrapperBase.cache_seqlens.to(torch.int32)
        rope_cache = self._get_deepseek_v4_dense_rope_cache()
        metadata = build_v4_decode_attn_metadata(
            coordinator=gpu_manager,
            sequence_ids=sequence_ids,
    ...
    def _install_deepseek_v4_decode_backend(self) -> None:
        coordinator = self.gpu_paged_kv_cache_manager
        if not self._is_deepseek_v4_kv_manager(coordinator):
            return
        ...
        layer_configs = build_layer_configs_from_compress_ratios(
            compress_ratios,
            n_heads=int(getattr(self.model_config, "num_attention_heads", 64)),
            head_dim=int(getattr(self.model_config, "head_dim", 512)),
            rope_head_dim=int(
        ...
        self._deepseek_v4_decode_backend = backend
        for layer_idx, layer in enumerate(model.model.layers):
            wrapper = layer.self_attn
            if isinstance(wrapper, DeepSeekV4FlashAttnWrapper):
                wrapper.set_v4_backend(backend)
        if self.rank == 0:
            logging.info(
    ...
    def _init_gpu_kv_with_actual_size(self) -> None:
        ...
        if (
            self.gpu_paged_kv_cache_manager is not None
            and self.gpu_paged_kv_cache_manager.is_initialized
        ):
            self._install_deepseek_v4_decode_backend()
            return
        ...
        new_gpu_kv_cache_size = (
            total_mem_gb * self.gpu_memory_frac - used_mem_gb
        )
        ...
        if self.rank == 0:
            ...
            logging.info(
                f"[GPU-KV] Initialized: {self.gpu_kv_cache_size_gb:.2f} GB, {stats.num_total_pages} pages"
            )

    def _config_decoding_for_batch(
        self, decode_uuids: List[str], local_decode_indices: List[int]
    ) -> None:
        """
        ...
        for uuid in decode_uuids:
            ...
            seq.validate_metadata(
                f"rank {self.rank} _config_decoding_for_batch/entry"
            )

        # VALIDATION: Verify decode_uuids consistency across all ranks
        local_uuid_count = torch.tensor(
            [len(decode_uuids)], dtype=torch.int64, device=self.torch_device
        )
        all_uuid_counts = [
            torch.zeros_like(local_uuid_count) for _ in range(self.world_size)
        ]
        ...
        if resuming_seqs and BATCHGEN_CB_DEBUG:
            ...
            problematic = [
                s
                for s in resuming_seqs
                if s["gpu_pages_allocated"] == 0
                and s["had_initial_gpu_reservation"]
            ]
            if problematic:
                logging.error(
        ...
        if local_decode_indices:
            alloc_ok = self._allocate_gpu_kv_two_page_buffer(
                local_decode_indices, load_from_host=True
            )
            if alloc_ok:
                ...
                for local_idx in local_decode_indices:
                    ...
                    seq.gpu_pages_allocated = (
                        seq.get_gpu_pages_for_two_page_buffer()
                    )
        ...
        if self.rank == 0:
            logging.info(
                f"[DECODE] Config completed: {(time.perf_counter() - start_time) * 1000:.1f}ms, {len(decode_uuids)} sequences"
            )
    ...
    def _prepare_decode_batch_two_page_buffer(self) -> List[str]:
        ...
        prefilled = self.global_batch.get_sequences_by_status(
            SequenceStatus.PREFILLED
        )
        onhold = self.global_batch.get_sequences_by_status(
            SequenceStatus.ON_HOLD
        )
    ...
    def _try_load_new_sequences_at_boundary_v2(
        self, current_decode_uuids: List[str], current_batch: List[int]
    ) -> Tuple[List[str], List[int]]:
        """
        ...
        if new_local_indices:
            self._allocate_gpu_kv_two_page_buffer(
                new_local_indices, load_from_host=True
            )
        ...
        logging.info(f"Rank {self.rank}: Loaded {len(new_uuids)} new sequences")
    ...
    def _release_host_kv_pages_for_batch(self, uuids: List[str]) -> None:
        ...
        if my_uuids:
            ...
            manager = self.gpu_paged_kv_cache_manager
            if manager is not None and manager.is_initialized:
                remaining_in_decode = self.global_batch.get_sequences_by_status(
                    SequenceStatus.IN_DECODE
                )
    ...
    def prefill(self, batch: list[int]):
        ...
        AttnWrapperBase.host_paged_kv_worker_view = getattr(
            self.core_engine, "host_paged_kv_worker_view", None
        )
        AttnWrapperBase.host_paged_kv_worker_view_aux = getattr(
            ...
            if seq_len < batch_max_len:
                ...
                seq_attention_mask = torch.cat(
                    [
                        seq_attention_mask,
                        torch.zeros(
                            (1, pad_len), dtype=seq_attention_mask.dtype
                        ),
                    ],
                    dim=1,
                )
        ...
        for micro_batch_idx in tqdm(
            ...
            with torch.inference_mode():
                ...
                new_tokens = self._select_tokens(
                    outputs.logits[:, -1, :], cur_batch_sequences
                )
        ...
        for i, local_idx in enumerate(batch):
            ...
            token_pos = (
                seq.decoded_length
            )  # 0 for fresh, prev_decoded for re-entry
            self.query_book[local_idx].decoded_tokens[:, token_pos] = (
                new_tokens_cpu[i]
            )
    ...
    def prefill_prepacked(self, batch: list[int]):
        ...
        for query_idx in batch:
            ...
            if (
                query_entry.decoded_tokens.data_ptr()
        ...
        row_capacity = (
            self.engine_config.Module_Batching_Config.prepack_row_capacity
        )
        ...
        for seq_idx in range(prepack_meta.num_original_sequences):
            ...
            seq_input_ids = prepack_meta.packed_input_ids[
                row_idx, start_pos : start_pos + seq_len
            ]
        ...
        packed_input_ids_flat = torch.cat(
            packed_input_ids_flat, dim=0
        )  # [total_tokens]
        packed_position_ids_flat = torch.cat(
            packed_position_ids_flat, dim=0
        )  # [total_tokens]
        ...
        with torch.inference_mode():
            for batch_idx, (seq_start, seq_end) in tqdm(
                ...
                batch_position_ids = []
                token_offset = sum(
                    seq_lengths_list[:seq_start]
                )  # Offset into flat tensors
                ...
                for seq_idx in range(seq_start, seq_end):
                    ...
                    batch_input_ids.append(
                        packed_input_ids_flat[seq_token_start:seq_token_end]
                    )
                    batch_position_ids.append(
                        packed_position_ids_flat[seq_token_start:seq_token_end]
                    )
                ...
                Attn_Wrapper.cur_batch = (
                    prefill_sequence_spans_to_global_seq_ids(batch_spans)
                )
    ...
    def _compute_boundary_decisions(
        ...
        growth_pages_by_uuid = dict(
            zip(host_growth_uuids, host_growth_pages_list)
        )
        ...
        if per_node_host_stats:
            ...
            for node in sorted(
                active_nodes | completed_nodes | set(host_stats_by_node.keys())
            ):
                ...
                node_active_uuids = [
                    ...
                    if uuid in global_seq_state
                    and global_seq_state[uuid].get("assigned_rank") is not None
                    and self._get_node_for_rank(
                        global_seq_state[uuid]["assigned_rank"]
                    )
                    == node
                ]
                ...
                if (
                    not node_active_uuids
                    and not node_completed_uuids
                    and not node_growth_uuids
                ):
                    continue
                if (
                    node_stats is None
                    or int(node_stats.get("num_total_pages", 0) or 0) <= 0
                ):
                    if node_growth_uuids:
                        scheduler_error = f"[HOST_KV_GROWTH_PLAN] node {node} has growth requests but no host KV stats"
                        ...
                        if state and uuid not in completed_set:
                            ...
                            eviction_candidates.append(
                                (
                                    uuid,
                                    {
                                        "decoded_length": state[
                                            "decoded_length"
                                        ],
                                        "host_pages_allocated": state.get(
                                            "host_pages_allocated", 0
                                        ),
                                        "global_idx": seq.global_idx
                                        if seq is not None
                ...
                growth_plan = plan_host_kv_growth_evictions(
                    ...
                    host_growth_pages=[
                        growth_pages_by_uuid[uuid] for uuid in node_growth_uuids
                    ],
                    eviction_candidates=eviction_candidates,
                    free_pages=free_pages,
                    total_pages=total_pages,
                    completed_pages=completed_host_pages,
                    watermark_percent=self.host_kv_eviction_watermark
                    if self.enable_host_kv_eviction
                    else 0,
                ...
                if (
                    growth_plan.remaining_growth_needed > 0
                    or growth_plan.evicted_uuids
                ):
                    growth_eviction_overlap = len(
                        set(growth_plan.evicted_uuids) & set(node_growth_uuids)
                    )
                    logging.info(
                    ...
                    if node_growth_uuids and (
                        growth_plan.evicted_uuids
                        or not growth_plan.growth_feasible_after_eviction
                    ):
                        detail_rows = []
                        for uuid in node_growth_uuids:
                            state = global_seq_state.get(uuid, {})
                            ...
                            context_len = int(
                                state.get(
                                    "current_context_length",
                                    getattr(seq, "current_context_length", 0),
                                )
                                or 0
                            )
                            capacity = int(
                                state.get(
                                    "host_token_capacity",
                                    getattr(seq, "host_token_capacity", 0),
                                )
                                or 0
                            )
                            detail_rows.append(
                        ...
                        logging.warning(
                            ...
                            + "; ".join(
                                ...
                                for runway, uuid, gid, rank, ctx, cap, host_pages, growth_pages in detail_rows[
                                    :8
                                ]
                            )
                        )
            ...
            scheduler_error = "[HOST_KV_GROWTH_PLAN] host growth requested but per-node host KV stats are missing"
            logging.error(scheduler_error)

        evicted_set = set(host_evicted_uuids)
        host_evicted_uuids = [
            uuid for uuid in active_uuids if uuid in evicted_set
        ]
        ...
        for uuid in decode_after_eviction:
            ...
            if state and state["additional_pages_needed"] > 0:
                ...
                total_additional_by_rank[assigned_rank] += state[
        ...
        if all_can_extend:
            ...
            for r in range(self.world_size):
                if total_additional_by_rank[r] > per_rank_free[r]:
                    rank_seqs = [
                        ...
                        for uuid in decode_after_eviction
                        if uuid in global_seq_state
                        and global_seq_state[uuid]["assigned_rank"] == r
                    ]
                    # Priority-aware: NORMAL (0) evicted before HIGH (1)
                    rank_seqs.sort(
                        key=lambda x: (
                            getattr(
                                self.global_batch.get_sequence(x[0]),
                                "priority",
                                0,
                            ),
                            x[1]["decoded_length"],
            ...
            for uuid in seqs_needing_extension:
                if uuid not in onhold_set:
                    ...
                    if r is not None:
                        actual_extension_by_rank[r] += state.get(
        ...
        host_growth_uuids = [
            uuid
            for uuid in host_growth_uuids
            if uuid in remaining_growth_by_uuid
        ]
        host_growth_pages_list = [
            remaining_growth_by_uuid[uuid] for uuid in host_growth_uuids
        ]
        ...
        if total_growth_needed > 0:
            ...
            for node, node_growth_pages in sorted(
                remaining_growth_by_node.items()
            ):
                plan_info = per_node_growth_plans.get(node)
                if plan_info is None:
                    scheduler_error = f"[HOST_KV_GROWTH_PLAN] node {node} has remaining growth but no host KV plan"
                    logging.error(scheduler_error)
                    break
                required_free = node_growth_pages + int(
                    plan_info["safety_margin"]
                )
        ...
        if global_candidate_info:
            ...
            new_load_uuids, _ = select_sequences_for_loading(
                ...
                get_global_idx_fn=lambda u: (
                    self.global_batch.get_sequence(u).global_idx
                    if self.global_batch.get_sequence(u)
                    else float("inf")
                ),
            )
    ...
    def _page_boundary_fast(
            ...
            if not all_sets_equal:
                for r, s in enumerate(all_decode_sets):
                    if s != local_decode_set:
                        ...
                        diff_in_local = (
                            local_decode_set - s if s else local_decode_set
                        )
                        logging.error(
                ...
                rank0_set = (
                    all_decode_sets[0]
                    if all_decode_sets[0] is not None
                    else set()
                )
                decode_uuids = sorted(
                    rank0_set,
                ...
                logging.warning(
                    f"Rank {self.rank}: Using rank-0 authoritative set at boundary start, decode_uuids now {len(decode_uuids)}"
                )
        ...
        if pending_load_uuids:  # ALL ranks have identical pending_load_uuids
            ...
            if batch and gpu_manager is not None and gpu_manager.is_initialized:
                ...
                if gpu_manager._gpu_page_table_manager:
                    post_finalize_slot_order = (
                        list(gpu_manager._gpu_page_table_manager.slot_to_seq_id)
                        if gpu_manager._gpu_page_table_manager.slot_to_seq_id
                        else []
                    )
                    post_finalize_batch_global_ids = (
                        self._local_indices_to_global_seq_ids(batch)
                    )
                    if (
                        post_finalize_slot_order
                        != post_finalize_batch_global_ids
        ...
        for uuid in self._uuid_to_local_map.keys():
            ...
            seq.validate_metadata(
                f"rank {self.rank} _page_boundary_fast/load_candidate"
            )
        ...
        if missing_uuids:
            ...
            for missing_uuid in missing_uuids[:10]:
                ...
                rank_reported = [
                    r
                    for r, p in enumerate(all_payloads)
        ...
        if completed_uuids:
            ...
            my_completed = [
                u for u in completed_uuids if u in self._uuid_to_local_map
            ]
            if my_completed:
                # Only release GPU pages for seqs that were actually GPU-allocated.
                ...
                gpu_allocated = [
                    u for u in my_completed if u in self._sequences_with_gpu_kv
                ]
                if gpu_allocated:
                    self._release_gpu_kv_pages(
                        self._get_local_indices_for_uuids(gpu_allocated)
            ...
            if self.adaptive_chunk_sizer is not None:
                for uuid in completed_uuids:
                    state = global_seq_state.get(uuid)
                    if state:
                        self.adaptive_chunk_sizer.report_completion(
                            state["decoded_length"]
                        )
            ...
            if self.rank == 0 and BATCHGEN_CB_DEBUG:
                for uuid in completed_uuids:
                    ...
                    state = global_seq_state.get(uuid, {})
                    was_evicted = (
                        getattr(seq, "total_decoded_before_eviction", 0) > 0
                    )
                    logging.debug(
        ...
        if host_evicted_uuids:
            ...
            my_evicted = [
                u for u in host_evicted_uuids if u in self._uuid_to_local_map
            ]
            if my_evicted:
                ...
                gpu_allocated = [
                    u for u in my_evicted if u in self._sequences_with_gpu_kv
                ]
                if gpu_allocated:
                    self._release_gpu_kv_pages(
                        self._get_local_indices_for_uuids(gpu_allocated)
                ...
                for uuid in my_evicted:
                    ...
                    if (
                        ...
                        new_decoded = seq.decoded_tokens[
                            0, baseline : seq.decoded_length
                        ]
                        seq.evicted_token_ids = torch.cat(
                            [prompt_tokens, new_decoded]
                        )
                    else:
        ...
        if decisions.growth_feasible and decisions.host_growth_uuids:
            host_grow_requests = []
            for uuid, growth_pages in zip(
                decisions.host_growth_uuids, decisions.host_growth_pages
            ):
        ...
        timing.total_completed_cumulative = len(
            self.global_batch.get_sequences_by_status(SequenceStatus.COMPLETED)
        )
        ...
        if onhold_uuids:
            ...
            my_onhold = [
                u for u in onhold_uuids if u in self._uuid_to_local_map
            ]
            if my_onhold:
                local_indices = self._get_local_indices_for_uuids(my_onhold)
                global_ids = self._local_indices_to_global_seq_ids(
        ...
        remaining_needing_ext = [
            u for u in seqs_needing_extension if u not in onhold_set
        ]
        my_remaining_ext = [
            u for u in remaining_needing_ext if u in self._uuid_to_local_map
        ]
        if my_remaining_ext:
            ...
            if not success:
                ...
                ext_failed_local = self._get_local_indices_for_uuids(
                    my_remaining_ext
                )
                ext_failed_global = self._local_indices_to_global_seq_ids(
                    ext_failed_local
                )
                ...
                for uuid in remaining_needing_ext:
                    ...
                    seq.gpu_pages_allocated = 0
                    seq.log_event(
                        SeqEvent.ON_HOLD, self.rank, "trigger=extension_failed"
                    )
                    self.global_batch.update_status(
                        uuid, SequenceStatus.ON_HOLD
                    )
                ...
                decode_uuids = [
                    u for u in decode_uuids if u not in ext_failed_set
                ]
                batch = self._get_local_indices_for_uuids(decode_uuids)
        ...
        if new_load_uuids:
            my_new_uuids = [
                u
                for u in new_load_uuids
                if global_candidate_info.get(u, {}).get("assigned_rank")
                == self.rank
            ]
            ...
            if new_load_local:
                ...
                if filtered_local:
                    ...
                    if worker_view is not None:
                        ...
                        if self._is_deepseek_v4_kv_manager(gpu_manager):
                            all_load_global = list(new_load_global) + [
                                gid
                                for gid in existing_global_ids
                                if gid not in set(new_load_global)
                            ]
                            if all_load_global:
                                gpu_manager.rebuild_page_table(all_load_global)
                            new_async_task = None
                            self._async_load_tensors = None
                        elif isinstance(gpu_manager, DualKVCacheCoordinator):
                            pointers = self._prepare_dual_kv_load_pointers(
                                gpu_manager,
                                new_load_global,
                                existing_global_ids,
                            )
                            new_async_task = self._launch_dual_host_kv_load(
                                pointers
                            )
                            ...
                            k_ptrs, v_ptrs = (
                                gpu_manager.get_padded_3d_page_pointers()
                            )
                            active_page_counts = (
                                gpu_manager.export_active_sequence_page_counts()
                            )
                            sequence_tensor = torch.tensor(
                                new_load_global, dtype=torch.int64, device="cpu"
                            )
                            new_async_task = (
                                worker_view.async_load_layer_paged_kv_to_device(
                                    sequence_ids=sequence_tensor,
        ...
        t0 = time.perf_counter()
        if BATCHGEN_CB_DEBUG:
            global_ids_for_rebuild = (
                self._local_indices_to_global_seq_ids(batch) if batch else []
            )
            logging.debug(
        ...
        if list(batch) != expected_local:
            actual_uuids = local_indices_to_uuid_order(
                batch, self._local_to_uuid_map
            )
            ...
            self._rebuild_page_table_for_batch(batch, gpu_manager)
            logging.info(
                f"Rank {self.rank}: Page table rebuilt after batch correction"
            )
        ...
        if (
            ...
            if worker_view is not None:
                ...
                pct = (
                    (used / hs.num_total_pages * 100)
                    if hs.num_total_pages > 0
                    else 0
                )
                ...
                hp_avg = (
                    sum(host_pages_list) / len(host_pages_list)
                    if host_pages_list
                    else 0
                )
                logging.info(
    ...
    def _finalize_async_load_minimal(
        ...
        if pending_local_indices and isinstance(
            gpu_manager, DualKVCacheCoordinator
        ):
        ...
        for uuid in pending_uuids:
            ...
            if seq.assigned_rank is None:
                invalid_pending.append(
                    f"{uuid[:8]} gid={seq.global_idx} has no assigned_rank"
                )
                continue
            if (
                seq.assigned_rank == self.rank
                and uuid not in pending_local_uuid_set
            ):
                invalid_pending.append(
        ...
        for local_idx in pending_local_indices:
            ...
            self._sequences_with_gpu_kv.add(uuid)
            seq.validate_metadata(
                f"rank {self.rank} _finalize_async_load_minimal"
            )
            seq.log_event(
                SeqEvent.KV_LOAD_DONE,
                self.rank,
        ...
        updated_batch = [
            uuid_to_local[u] for u in updated_uuids if u in uuid_to_local
        ]

        return updated_uuids, updated_batch

    def _sync_decode_moe_rank_counts(
        self, batch: List[int], *, reason: str
    ) -> int:
        """Synchronize per-rank decode row counts for 3D MoE padding masks."""
        local_count = int(len(batch))
        local_count_tensor = torch.tensor(
            [local_count], dtype=torch.int64, device=self.torch_device
        )
        ...
        if (
            max_batch_size > 0
            and hasattr(self, "parallel_manager")
            and self.parallel_manager is not None
        ):
    ...
    def _warmup_cuda_graphs(self):
        ...
        if not self.engine_config.Basic_Config.enable_cuda_graphs:
            if (
                glm5_dsa_graph_enabled
                or glm5_moe_graph_enabled
                or glm5_whole_graph_enabled
            ):
                self.engine_config.Basic_Config.enable_cuda_graphs = True
            else:
                return
        if (
            ...
            logging.info(
                f"Rank {self.rank}: CUDA graphs not supported for '{model_name}', skipping"
            )
            return
        ...
        if gpu_manager is None:
            logging.warning(
                f"Rank {self.rank}: No GPU KV manager, skipping CUDA graph warmup"
            )
            return
    ...
    def _setup_glm5_moe_cuda_graphs(self, bucket_sizes):
        ...
        if (
            "glm" not in model_name_l
            or not self._glm5_moe_graph_requested_for_current_batch()
        ):
            return
        if self._glm5_moe_cuda_graph_manager is None and getattr(
            self, "_glm5_moe_graph_capture_attempted_for_batch", False
        ):
            logging.info(
        ...
        if max_bsz <= 0:
            logging.info(
                f"Rank {self.rank}: no GLM-5 MoE decode rows globally; skipping MoE graph capture"
            )
            return
        ...
        capture_buckets = [
            ...
            if int(bucket)
            not in getattr(self, "_glm5_moe_graph_failed_buckets", set())
        ]
        ...
        if self._glm5_moe_cuda_graph_manager is not None:
            missing_buckets = [
                ...
                if not self._glm5_moe_cuda_graph_manager.has_bucket_for_all_segments(
                    bucket
                )
            ]
            ...
            self._glm5_moe_graph_capture_attempted_for_batch = True
            try:
                self._glm5_moe_cuda_graph_manager.warmup_and_capture_buckets(
                    missing_buckets
                )
        ...
        graph_output_required = (
            self._glm5_moe_graph_output_required_for_current_batch()
        )
        ...
        for layer_idx, decoder_layer in enumerate(self.model.model.layers):
            ...
            if not getattr(moe, "_fp8_blockwise_ready", False):
                raise RuntimeError(
                    f"Layer {layer_idx}: GLM-5 MoE graph requires FP8 blockwise weights"
                )
    ...
    def _glm5_segmented_graph_initial_capture_missing(self) -> bool:
        ...
        dsa_missing = (
            self._glm5_dsa_graph_requested_for_current_batch()
            and int(getattr(self, "_current_decode_local_batch_size", 0) or 0)
            > 0
            and self._cuda_graph_manager is None
            and not getattr(
                self,
    ...
    def _glm5_segmented_graph_capture_already_attempted_for_requested_paths(
        self,
    ) -> bool:
        model_name_l = (getattr(self, "model_name", "") or "").lower()
        if "glm" not in model_name_l:
        ...
        dsa_done = (
            not dsa_requested
            or int(getattr(self, "_current_decode_local_batch_size", 0) or 0)
            <= 0
            or bool(
                getattr(
                    self, "_glm5_dsa_graph_capture_attempted_for_batch", False
        ...
        moe_done = not moe_requested or bool(
            getattr(self, "_glm5_moe_graph_capture_attempted_for_batch", False)
        )
    ...
    def _glm5_dsa_graph_score_capacity_tokens(
        ...
        *,
        ...
        primary_capacity = int(primary_page_table.shape[1]) * int(
            primary_page_size
        )
        aux_capacity = int(aux_page_table.shape[1]) * int(aux_page_size)
        capacities = [primary_capacity, aux_capacity]
        if (
            model_max_position_embeddings is not None
            and int(model_max_position_embeddings) > 0
        ):
    ...
    def _glm5_dsa_graph_page_table_storage_changed(self) -> bool:
        ...
        try:
            ...
            return False
        expected_primary = getattr(
            wrapper, "_dsa_cuda_graph_primary_page_table_signature", None
        )
        expected_aux = getattr(
            wrapper, "_dsa_cuda_graph_aux_page_table_signature", None
        )
        ...
        def _sig(manager):
            get_storage = getattr(
                manager, "get_cuda_graph_page_table_storage", None
            )
            try:
                if get_storage is not None:
                    table = get_storage()
                else:
                    get_graph_table = getattr(
                        manager, "get_cuda_graph_page_table", None
                    )
                    table = (
                        get_graph_table()
                        if get_graph_table is not None
                        else None
                    )
            except RuntimeError:
                return None
        ...
        if (
            _sig(primary_manager) == expected_primary
            and _sig(aux_manager) == expected_aux
        ):
            return False
        logging.warning(
    ...
    def _glm5_dsa_graph_requested_for_current_batch(self) -> bool:
        ...
        debug = (
            self._batchgen_debug
            or getattr(AttnWrapperBase, "batchgen_debug", None)
            or {}
        )
        if not isinstance(debug, dict):
            return False
    ...
    def _glm5_whole_model_graph_requested_for_current_batch(self) -> bool:
        if (
            ...
            or os.environ.get("BATCHGEN_GLM5_WHOLE_MODEL_GRAPH_COMPARE", "0")
            == "1"
        ):
            return True
        debug = (
        ...
        if not isinstance(debug, dict):
            return False
        return self._debug_flag_enabled(
            debug.get("glm5_whole_model_graph")
        ) or self._debug_flag_enabled(
            debug.get("glm5_whole_model_graph_compare")
        )

    def _glm5_whole_model_graph_compare_requested_for_current_batch(
        self,
    ) -> bool:
        if (
            os.environ.get("BATCHGEN_GLM5_WHOLE_MODEL_GRAPH_COMPARE", "0")
            == "1"
        ):
            return True
        debug = (
    ...
    def _glm5_whole_model_graph_timing_requested_for_current_batch(
        self,
    ) -> bool:
        ...
        if isinstance(debug, dict) and self._debug_flag_enabled(
            debug.get("glm5_whole_model_graph_timing")
        ):
            return True
        return (
            self._glm5_whole_model_graph_compare_requested_for_current_batch()
        )

    def _glm5_whole_model_graph_compare_fail_on_mismatch(self) -> bool:
        if (
            os.environ.get("BATCHGEN_GLM5_WHOLE_MODEL_GRAPH_COMPARE_FAIL", "0")
            == "1"
        ):
            return True
        debug = (
        ...
        if not isinstance(debug, dict):
            return False
        return self._debug_flag_enabled(
            debug.get("glm5_whole_model_graph_compare_fail")
        )
    ...
    def _glm5_whole_model_graph_capture_signature(self, bucket_size: int):
        ...
        def _table_sig(manager):
            ...
            try:
                table = (
                    get_graph_table() if get_graph_table is not None else None
                )
            except RuntimeError:
                return None
            if table is None:
    ...
    def _glm5_whole_model_graph_current_bucket_missing(self) -> bool:
        ...
        if max_bsz <= 0:
            return False
        if self._cuda_graph_manager is None or not getattr(
            self, "_glm5_whole_model_graph", False
        ):
            return True
        try:
    ...
    def _glm5_moe_graph_current_bucket_missing(self) -> bool:
        ...
        try:
            bucket = (
                self._glm5_moe_cuda_graph_manager.bucketing.get_padded_size(
                    max_bsz
                )
            )
        except AttributeError:
            missing = not self._glm5_moe_cuda_graph_manager.has_bucket_for_all_segments(
                max_bsz
            )
        except ValueError:
            return False
    ...
    def _glm5_dsa_graph_path_state(self, local_bsz: int, gpu_manager):
        ...
        index_topk = getattr(
            getattr(wrapper.module, "indexer", None), "index_topk", 2048
        )
        ...
        if not _glm5_dsa_cuda_graph_can_replay(
            ...
            max_seqlen,
            index_topk,
            captured_max_seqlen=getattr(
                wrapper, "_dsa_cuda_graph_max_seqlen", None
            ),
        ):
        ...
        active_sequence_ids = list(
            getattr(AttnWrapperBase, "cur_batch", None) or []
        )
        for manager_obj, label in (
            (primary_manager, "primary"),
            (aux_manager, "aux"),
        ):
            ensure_graph_table = getattr(
                manager_obj, "ensure_cuda_graph_page_table", None
            )
        ...
        if not wrapper._dsa_cuda_graph_page_tables_match(
            primary_manager, aux_manager
        ):
    ...
    def _prepare_glm5_dsa_graph_flashmla_metadata_for_forward(
        ...
        path, bucket, reason = self._glm5_dsa_graph_path_state(
            local_bsz, gpu_manager
        )
        ...
        if not layers:
            raise RuntimeError(
                "GLM-5 DSA CUDA graph replay requires attention wrappers"
            )
        wrapper = layers[0].self_attn
        index_topk = int(
            getattr(
        ...
        if cache_seqlens is None:
            raise RuntimeError(
                "GLM-5 DSA CUDA graph replay requires cache_seqlens metadata"
            )
        bucket = int(bucket)
        ...
        if aux_manager is None:
            raise RuntimeError(
                "GLM-5 DSA CUDA graph replay requires auxiliary GPU KV manager"
            )
        primary_state = primary_manager.get_cuda_graph_page_table_state()
        aux_state = aux_manager.get_cuda_graph_page_table_state()
        AttnWrapperBase.glm5_decode_primary_slot_indices = (
            primary_state.slot_indices[:local_bsz].to(
                dtype=torch.int32,
                device=self.torch_device,
            )
        )
        AttnWrapperBase.glm5_decode_aux_slot_indices = aux_state.slot_indices[
            :local_bsz
        ].to(
            dtype=torch.int32,
            device=self.torch_device,
        ...
        if local_bsz < bucket:
            captured_max_seqlen = int(
                getattr(wrapper, "_dsa_cuda_graph_max_seqlen", index_topk)
            )
            selected_lengths[local_bsz:].fill_(
                min(captured_max_seqlen, index_topk)
            )
    ...
    def _log_glm5_graph_path_for_forward(
        ...
        *,
        ...
        if (
            "glm" not in model_name_l
            or not self._glm5_graph_path_log_requested_for_current_batch()
        ):
            return
        ...
        moe_path, moe_bucket, moe_reason = self._glm5_moe_graph_path_state(
            max_rank_bsz
        )
    ...
    def _generate_bucket_sizes(max_bucket: int, num_buckets: int) -> list:
        ...
        while len(sizes) < num_buckets:
            ...
            if (
                mid in sizes
                or mid <= sizes[best_idx]
                or mid >= sizes[best_idx + 1]
            ):
                break
    ...
    def _setup_cuda_graphs(self, gpu_manager):
        ...
        logging.info(
            f"CUDA graph bucket sizes: {bucket_sizes} (max={max_bucket}, num_buckets={num_buckets})"
        )
        bucketing = BatchSizeBucketing(bucket_sizes)
        manager = CUDAGraphManager(bucketing, device=self.torch_device)
        ...
        if glm5_whole_graph_enabled:
            local_bsz = int(
                getattr(self, "_current_decode_local_batch_size", 0) or 0
            )
            max_bsz = int(
                getattr(self, "_current_decode_max_rank_batch_size", 0) or 0
            )
            ...
            if aux_manager is None:
                raise RuntimeError(
                    "GLM-5 whole-model CUDA graph requested but auxiliary GPU KV manager is missing"
                )
            active_sequence_ids = list(cur_batch)
            primary_page_table = primary_manager.ensure_cuda_graph_page_table(
                active_sequence_ids
            )
            aux_page_table = aux_manager.ensure_cuda_graph_page_table(
                active_sequence_ids
            )
            ...
            graph_max_seqlen = int(
                os.environ.get(
                    "BATCHGEN_GLM5_WHOLE_MODEL_CUDA_GRAPH_MAX_SEQLEN", "8192"
                )
            )
            if graph_max_seqlen <= 0:
                raise RuntimeError(
                    "BATCHGEN_GLM5_WHOLE_MODEL_CUDA_GRAPH_MAX_SEQLEN must be positive"
                )
            if (
            ...
            for layer_idx, decoder_layer in enumerate(self.model.model.layers):
                ...
                if getattr(wrapper, "_indexer_cuda_module", None) is None:
                    raise RuntimeError(
                        f"Layer {layer_idx}: GLM-5 whole-model graph requires fused indexer CUDA module"
                    )
            moe_not_ready = []
            for layer_idx, decoder_layer in enumerate(self.model.model.layers):
                mlp = getattr(decoder_layer, "mlp", None)
                if hasattr(mlp, "experts_per_rank") and not getattr(
                    mlp, "_fp8_blockwise_ready", False
                ):
            ...
            if moe_not_ready:
                ...
                if (
                    os.environ.get("BATCHGEN_GLM5_WHOLE_MODEL_CUDA_GRAPH", "0")
                    == "1"
                ):
                    raise RuntimeError(reason)
                logging.warning(
                    "%s Using eager decode without whole-model graph compare.",
                    reason,
                )
                return
            ...
            vocab_size = (
                ...
                or self.model.config.vocab_size
            )
            hidden_size = self.model.config.hidden_size
            probe_layers_env = os.environ.get(
                "BATCHGEN_GLM5_WHOLE_MODEL_GRAPH_PROBE_LAYERS", ""
            )
            ...
            capture_input_ids = getattr(
                self, "_glm5_whole_model_capture_input_ids", None
            )
            if (
                capture_input_ids is None
                or capture_input_ids.shape[0] < local_bsz
            ):
                logging.info(
                    f"Rank {self.rank}: GLM-5 whole-model graph capture deferred until "
            ...
            def _capture_slots(manager):
                ensure_graph_table = getattr(
                    manager, "ensure_cuda_graph_page_table", None
                )
                if ensure_graph_table is not None:
                    ensure_graph_table(list(cur_batch))
                slot_indices = (
                    manager._gpu_page_table_manager._slot_index_tensor
                )
            ...
            rank_counts = getattr(
                self, "_current_decode_rank_token_counts", None
            )
            if rank_counts is None:
            ...
            capture_cache_seqlens = AttnWrapperBase.cache_seqlens[
                :local_bsz
            ].to(dtype=torch.int32)
            capture_position_ids = AttnWrapperBase.position_ids[:local_bsz].to(
                dtype=torch.int64
            )
            ...
            try:
                ...
                return
            self._glm5_whole_model_graph_signature = (
                self._glm5_whole_model_graph_capture_signature(capture_bucket)
            )
        ...
        if glm5_dsa_graph_enabled:
            ...
            if local_bsz <= 0:
                ...
                return
            if self._cuda_graph_manager is None and getattr(
                self, "_glm5_dsa_graph_capture_attempted_for_batch", False
            ):
                logging.info(
            ...
            if self._cuda_graph_manager is not None:
                capture_buckets = [
                    ...
                    if int(bucket)
                    not in getattr(
                        self, "_glm5_dsa_graph_failed_buckets", set()
                    )
                ]
                missing_buckets = [
                    bucket
                    for bucket in capture_buckets
                    if not self._cuda_graph_manager.has_bucket_for_all_segments(
                        bucket
                    )
                ]
                if missing_buckets:
                    ...
                    self._glm5_dsa_graph_capture_attempted_for_batch = True
                    try:
                        self._cuda_graph_manager.warmup_and_capture_buckets(
                            missing_buckets
                        )
                    except torch.OutOfMemoryError as exc:
                        for bucket in missing_buckets:
            ...
            if aux_manager is None:
                raise RuntimeError(
                    "GLM-5 DSA CUDA graph requested but auxiliary GPU KV manager is missing"
                )

            primary_page_size = int(primary_manager.config.page_size_tokens)
            aux_page_size = int(aux_manager.config.page_size_tokens)
            primary_page_table = (
                primary_manager.get_cuda_graph_page_table_storage()
            )
            ...
            graph_max_seqlen = self._glm5_dsa_graph_score_capacity_tokens(
                ...
                model_max_position_embeddings=getattr(
                    self.model_config, "max_position_embeddings", None
                ),
            )
            legacy_graph_cap = os.environ.get(
                "BATCHGEN_GLM5_DSA_CUDA_GRAPH_MAX_SEQLEN"
            )
            ...
            capture_buckets = [
                ...
                for bucket in bucketing.bucket_sizes
                if int(bucket)
                not in getattr(self, "_glm5_dsa_graph_failed_buckets", set())
            ]
            if not capture_buckets:
                self._glm5_dsa_graph_capture_attempted_for_batch = True
                if glm5_moe_graph_enabled:
            ...
            for layer_idx, decoder_layer in enumerate(self.model.model.layers):
                ...
                if indexer is None:
                    raise RuntimeError(
                        f"GLM-5 DSA CUDA graph requested but layer {layer_idx} has no indexer"
                    )
                if getattr(wrapper, "_fp8_absorb_weights", None) is None:
                    wrapper.initialize_decode_absorb()
                if (
                ...
                if getattr(wrapper, "_fp8_absorb_weights", None) is None:
                    raise RuntimeError(
                        f"Layer {layer_idx}: GLM-5 DSA CUDA graph requires FP8 absorb weights"
                    )
                if getattr(wrapper, "_fused_wqb_weights", None) is None:
                    raise RuntimeError(
                        f"Layer {layer_idx}: GLM-5 DSA CUDA graph requires fused WQB weights"
                    )
                if getattr(wrapper, "_indexer_cuda_module", None) is None:
                    raise RuntimeError(
                        f"Layer {layer_idx}: GLM-5 DSA CUDA graph requires fused indexer CUDA module"
                    )
                ...
                cos_table, sin_table = indexer.rotary_emb(
                    dummy, seq_len=graph_max_seqlen
                )
                if glm5_dsa_full_graph_enabled:
                    ...
                    segment_name = make_glm5_full_dsa_graph_segment_name(
                        layer_idx
                    )
                else:
        ...
        if not _is_k25:
            ...
            for layer_idx, decoder_layer in enumerate(self.model.model.layers):
                ...
                if (
                    wrapper.sinks is None
                    and wrapper.persistent
                    and hasattr(wrapper.module, "sinks")
                ):
                    wrapper.sinks = wrapper.module.sinks.data.to(
                        self.torch_device
                    )
            ...
            for layer_idx, decoder_layer in enumerate(self.model.model.layers):
                ...
                if (
                    hasattr(moe_decode, "persistent_expert_indices")
                    and len(moe_decode.persistent_expert_indices) > 0
                    and hasattr(moe_decode, "comm")
                    and moe_decode.comm is not None
                ):
                    ...
                    if moe_pool is None:
                        moe_pool = SharedMoEBufferPool(
                            ...
                            num_local_experts=len(
                                moe_decode.persistent_expert_indices
                            ),
                    ...
                    if not os.environ.get("BATCHGEN_MOE_EAGER"):
                        ...
                        manager.register_segment(
                            f"layer_{layer_idx}_moe", moe_compute_seg
                        )
                        has_moe_graph = True
        else:
            ...
            dummy = torch.zeros(
                1, 1, 1, rotary_emb.dim, device=self.torch_device
            )
        ...
        if _is_k25:
            ...
            use_whole_model = (
                os.environ.get("BATCHGEN_SEGMENTED_GRAPH", "0") != "1"
            )
        ...
        if use_whole_model:
            ...
            if _is_k25:
                ...
                for layer_idx, decoder_layer in enumerate(
                    ...
                    moe_name = (
                        f"layer_{layer_idx}_moe" if has_moe_graph else None
                    )
                    decoder_layer.enable_cuda_graph(
                        manager,
    ...
    def decoding_continuous(
        ...
        logging.info(
            f"[RELOAD-TEST-V4] DEADLOCK-FIXED hot-reload on rank {getattr(self, 'global_rank', '?')}"
        )
        if "deepseek" in self.model_config.model_type:
        ...
        if gpu_manager and gpu_manager._gpu_page_table_manager:
            ...
            entry_cur_batch = (
                list(Attn_Wrapper.cur_batch) if Attn_Wrapper.cur_batch else []
            )
            if entry_slot_order != entry_cur_batch:
                ...
                if entry_cur_batch:
                    ...
                    logging.info(
                        f"Rank {self.rank}: Page table rebuilt to match cur_batch order"
                    )
            else:
                if BATCHGEN_CB_DEBUG:
                    logging.debug(
        ...
        _new_tokens_pinned = torch.empty(
            max(max_batch_size, 1), 1, dtype=torch.long, pin_memory=True
        )
        ...
        while decode_uuids:
            ...
            if local_iteration - last_boundary >= self.DECISION_INTERVAL:
                ...
                if (
                    batch
                    and gpu_manager
                    and gpu_manager.is_initialized
                    and gpu_manager._gpu_page_table_manager
                ):
                    post_boundary_slot_order = (
                        list(gpu_manager._gpu_page_table_manager.slot_to_seq_id)
                        if gpu_manager._gpu_page_table_manager.slot_to_seq_id
                        else []
                    )
                    post_boundary_batch_global_ids = (
                        self._local_indices_to_global_seq_ids(batch)
                    )
                ...
                if self._admission_queue is not None:
                    ...
                    if has_q and watermark_triggered:
                        if self.rank == 0:
                            logging.info(
                                f"[DECODE] Breaking for new batch prefill (watermark triggered)"
                            )
                        break
                ...
                if self.rank == 0:
                    ...
                    num_host_kv_total = (
                        num_prefilled + num_onhold + num_in_decode
                    )

                    # Get page stats if available
                    page_info = ""
                    if (
                        hasattr(self, "_host_kv_page_stats")
                        and self._host_kv_page_stats
                    ):
            ...
            global_decode_sequences = self._debug_sequences_for_decode_uuids(
                decode_uuids
            )
            AttnWrapperBase.batchgen_debug = (
                self._active_batchgen_debug_for_sequences(
            ...
            if BATCHGEN_DECODE_ASSERT and batch:
                for seq in batch_sequences:
                    max_tokens = (
                        seq.gpu_pages_allocated * SequenceEntry.PAGE_SIZE
                    )
            ...
            with torch.inference_mode():
                if batch:
                    ...
                    if (
                        BATCHGEN_MULTI_BATCH_DIAG
                        and self.rank == 0
                        and local_iteration <= 1
                    ):
                        fresh = [
                            (s.uuid[:8], s.decoded_length, ctx)
                            for s, ctx in zip(batch_sequences, cache_seqlens)
                            if s.decoded_length <= 1
                        ]
                        resumed = [
                            (
                                s.uuid[:8],
                        ...
                        for uid, dl, ctx in fresh[:5]:
                            logging.info(
                                f"[MULTI_DIAG]   FRESH: {uid} decoded={dl} cache_seqlen={ctx}"
                            )
                        for uid, dl, ctx, pg in resumed[:5]:
                            logging.info(
                                f"[MULTI_DIAG]   RESUMED: {uid} decoded={dl} cache_seqlen={ctx} gpu_pages={pg}"
                            )
                    ...
                    Attn_Wrapper.attention_mask = (
                        None  # Removed: no longer used in decode
                    )
                    Attn_Wrapper.cache_seqlens = seqlens_tensor.to(torch.int32)
                    Attn_Wrapper.position_ids = (
                        (Attn_Wrapper.cache_seqlens - 1)
                        .unsqueeze(-1)
                        .to(torch.int64)
                    )
                    ...
                    _dsa_index_topk = getattr(
                        self.model_config, "index_topk", None
                    )
                    if _dsa_index_topk is not None:
                        AttnWrapperBase._dsa_short_count = int(
                            (Attn_Wrapper.cache_seqlens <= _dsa_index_topk)
                            .sum()
                            .item()
                        )
                    else:
                ...
                if batch:
                    ...
                    if not _page_table_verified_this_batch:
                        ...
                        if gpu_manager and gpu_manager._gpu_page_table_manager:
                            slot_order = (
                                list(
                                    gpu_manager._gpu_page_table_manager.slot_to_seq_id
                                )
                                if gpu_manager._gpu_page_table_manager.slot_to_seq_id
                                else []
                            )
                ...
                if (
                    ...
                    _all_rank_counts = getattr(
                        self, "_current_decode_rank_token_counts", None
                    )
                    _cached_local_bsz = int(
                        getattr(self, "_current_decode_local_batch_size", -1)
                    )
                    _max_bs = int(
                        getattr(self, "_current_decode_max_rank_batch_size", 0)
                        or 0
                    )
                    if (
                        _all_rank_counts is None
                        or _max_bs <= 0
                        or _cached_local_bsz != len(batch)
                    ):
                ...
                if _kv_worker_view is not None:
                    ...
                    self._deferred_kv_worker_view_aux = getattr(
                        self, "host_paged_kv_worker_view_aux", None
                    )
                ...
                if BATCHGEN_SYNC_KV and _kv_worker_view is not None:
                    ...
                    def kv_append_callback(
                        ...
                        torch.cuda.synchronize(self.torch_device)
                        task = (
                            _sync_kv_worker_view.async_append_decode_kv_to_host(
                                layer_idx=layer_idx,
                    ...
                    def kv_append_callback(
                        ...
                        self._deferred_kv_entries.append(
                ...
                if aux_view is not None:
                    if BATCHGEN_SYNC_KV:
                        ...
                        def kv_append_callback_aux(
                            layer_idx: int,
                            k_tensor: torch.Tensor,
                            v_tensor: torch.Tensor = None,
                        ):
                            self._append_decode_kv_to_host_aux_async(
                                layer_idx, current_batch, k_tensor, v_tensor
                            )
                    else:
                        ...
                        def kv_append_callback_aux(
                            ...
                            self._deferred_kv_entries_aux.append(
                                (layer_idx, k_tensor, v_tensor)
                            )

                    AttnWrapperBase.kv_append_callback_aux = (
                        kv_append_callback_aux
                    )
                else:
                ...
                if self._glm5_whole_model_graph_current_bucket_missing():
                    ...
                    self._glm5_whole_model_capture_input_ids = new_tokens[
                        : len(batch)
                    ]
                    self._warmup_cuda_graphs()
                ...
                if _glm5_whole_graph_active:
                    try:
                        _glm5_whole_bucket = (
                            self._whole_model_bucketing.get_padded_size(_max_bs)
                        )
                    except ValueError:
                        ...
                        _glm5_whole_graph_active = (
                            ...
                            not in getattr(
                                self,
                                "_glm5_whole_model_graph_failed_buckets",
                                set(),
                            )
                            and self._cuda_graph_manager.has_bucket_for_all_segments(
                                _max_bs
                            )
                            and int(
                                getattr(AttnWrapperBase, "max_seqlen", 0) or 0
                            )
                            <= int(
                                getattr(
                                    self._whole_model_segment, "max_seqlen", 0
                                )
                            )
                            and self._glm5_whole_model_graph_capture_signature(
                                _glm5_whole_bucket
                            )
                            == getattr(
                                self, "_glm5_whole_model_graph_signature", None
                ...
                if _use_graph:
                    ...
                    bucket = self._whole_model_bucketing.get_padded_size(
                        _max_bs
                    )
                    if getattr(self, "_glm5_whole_model_graph", False):
                        primary_manager = getattr(
                            gpu_manager, "primary", gpu_manager
                        )
                        aux_manager = getattr(
                            gpu_manager,
                            "auxiliary",
                        ...
                        if aux_manager is None:
                            raise RuntimeError(
                                "GLM-5 whole-model graph replay requires auxiliary GPU KV manager"
                            )
                        ...
                        def _graph_slots(manager):
                            ...
                            real_slots = slot_indices[:batch_size].to(
                                dtype=torch.int32
                            )
                        ...
                        graph_input_ids = _pad_graph_input(
                            new_tokens[:batch_size], bucket, 0
                        )
                        graph_cache_seqlens = _pad_graph_input(
                            AttnWrapperBase.cache_seqlens[:batch_size].to(
                                dtype=torch.int32
                            ),
                            bucket,
                            1,
                        ...
                        graph_position_ids = _pad_graph_input(
                            AttnWrapperBase.position_ids[:batch_size].to(
                                dtype=torch.int64
                            ),
                            bucket,
                            0,
                        ...
                        page_table_tensor = (
                            gpu_manager._gpu_page_table_manager.gpu_table
                        )
                        ...
                        if pt_slice.shape[1] < wm_max_pages:
                            pt_slice = torch.nn.functional.pad(
                                pt_slice,
                                (0, wm_max_pages - pt_slice.shape[1]),
                                value=0,
                            )
                    ...
                    if _glm5_whole_compare:
                        ...
                        graph_tokens_for_compare = torch.argmax(
                        ...
                        if getattr(
                            self._whole_model_segment,
                            "compare_probe_layers",
                            (),
                        ):
                            eager_probe_outputs = (
                                self._whole_model_segment.run_model_with_probes(
                                    input_ids=new_tokens,
                                    attention_mask=Attn_Wrapper.attention_mask,
                                    position_ids=Attn_Wrapper.position_ids,
                                )
                            )
                            eager_hidden_states = eager_probe_outputs[
                                "hidden_states"
                            ]
                            ...
                            eager_hidden_states = eager_model_outputs[0][
                                :, -1, :
                            ]
                            eager_logits = self.model.lm_head(
                                eager_model_outputs[0]
                            )[:, -1, :]
                            eager_probe_hidden_states = {}
                        if _glm5_whole_timing:
                            torch.cuda.synchronize(self.torch_device)
                        ...
                        eager_tokens_for_compare = torch.argmax(
                            eager_logits, dim=-1, keepdim=True
                        )
                        new_tokens_out = self._select_tokens(
                            eager_logits, batch_sequences
                        )
                        from batchgen.models.glm.glm5.whole_model_cuda_graph_segments import (
                        ...
                        compare = compare_glm5_whole_model_graph_logits(
                            ...
                            atol=float(
                                os.environ.get(
                                    "BATCHGEN_GLM5_WHOLE_MODEL_GRAPH_COMPARE_ATOL",
                                    "1e-2",
                                )
                            ),
                            rtol=float(
                                os.environ.get(
                                    "BATCHGEN_GLM5_WHOLE_MODEL_GRAPH_COMPARE_RTOL",
                                    "1e-2",
                                )
                            ),
                        )
                        ...
                        if (
                            not compare["ok"]
                            and self._glm5_whole_model_graph_compare_fail_on_mismatch()
                        ):
                            raise RuntimeError(
                                f"GLM-5 whole-model CUDA graph compare mismatch: {compare}"
                            )
                        _glm5_skip_graph_kv_offload = True
                    else:
                        new_tokens_out = self._select_tokens(
                            logits, batch_sequences
                        )
                    ...
                    if not _glm5_skip_graph_kv_offload:
                        ...
                        if (
                            ...
                            primary_stage = None
                            primary_key_buffer = getattr(
                                wm_seg, "_kv_key_buffer", None
                            )
                            if primary_key_buffer is not None:
                                primary_stage = primary_key_buffer[
                                    :, :batch_size
                                ].clone()
                            for layer_idx in range(wm_seg.num_layers):
                                ...
                                v_clone = (
                                    v_buf[:batch_size].clone()
                                    if v_buf is not None
                                    and v_buf.numel() > 0
                                    and not getattr(
                                        wm_seg, "_no_v_cache", False
                                    )
                                    else None
                                )
                                k_tensor = (
                        ...
                        aux_buffers = (
                            getattr(wm_seg, "_aux_kv_buffers", None)
                            if wm_seg is not None
                            else None
                        )
                        if (
                            batch_size > 0
                            and aux_cb is not None
                            and aux_buffers is not None
                        ):
                            aux_stage = None
                            aux_key_buffer = getattr(
                                wm_seg, "_aux_kv_key_buffer", None
                            )
                            if aux_key_buffer is not None:
                                aux_stage = aux_key_buffer[
                                    :, :batch_size
                                ].clone()
                            for layer_idx in range(wm_seg.num_layers):
                                aux_cb(
                                    ...
                                    else aux_buffers[layer_idx]["key"][
                                        :batch_size
                                    ].clone(),
                                    None,
                                )
                    ...
                    if _glm5_whole_timing:
                        logging.info(
                            ...
                            _glm5_whole_timing_items.get(
                                "offload_callback_ms", -1.0
                            ),
                            _glm5_whole_compare,
                        )
                else:
            ...
            if bs > _new_tokens_pinned.shape[0]:
                _new_tokens_pinned = torch.empty(
                    bs, 1, dtype=torch.long, pin_memory=True
                )
            ...
            for i, (local_idx, seq) in enumerate(zip(batch, batch_sequences)):
                ...
                if BATCHGEN_CB_DEBUG:
                    qb_ptr = self.query_book[
                        local_idx
                    ].decoded_tokens.data_ptr()
                ...
                self.query_book[local_idx].decoded_tokens[:, decode_pos] = (
                    new_tokens_cpu[i]
                )
        ...
        if (
            ...
            avg_forward = (
                self._cumulative_forward_ms / self._cumulative_decode_iterations
                if self._cumulative_decode_iterations > 0
                else 0
            )
            avg_round = (
                self._cumulative_boundary_ms
                / self._cumulative_decode_boundaries
            )
            logging.debug(
                f"\n{'=' * 50}\n"
    ...
    def _rebuild_page_table_for_batch(
        self, batch: List[int], gpu_manager: GPUPagedKVCacheManager
    ) -> None:
        ...
        if manager_sequences is not None:
            allocated_ids = [
                gid for gid in global_ids if gid in manager_sequences
            ]
            if len(allocated_ids) < len(global_ids):
                missing = [
                    gid for gid in global_ids if gid not in manager_sequences
                ]
                logging.error(
    ...
    def _launch_async_load_new_sequences(
        ...
        new_global_ids = self._local_indices_to_global_seq_ids(
            new_local_indices
        )
        ...
        existing_global_ids = self._local_indices_to_global_seq_ids(
            current_batch
        )
        ...
        if self._is_deepseek_v4_kv_manager(gpu_manager):
            all_global_ids = list(new_global_ids) + [
                gid
                for gid in existing_global_ids
                if gid not in set(new_global_ids)
            ]
            if all_global_ids:
    ...
    def _launch_async_load_new_sequences_timed(
        ...
        Optional[object], List[str], List[int], List[int], Dict[str, float]
    ]:
        """
        ...
        for uuid in candidates:
            ...
            if req_pages is None:
                logging.warning(
                    f"Rank {self.rank}: No page count for {uuid}, skipping"
                )
                continue
        ...
        can_allocate_tensor = torch.tensor(
        ...
        if worker_view is None:
            logging.warning(
                f"Rank {self.rank}: worker_view is None, cannot launch async load"
            )
    ...
    def _rebuild_input_tokens(self, batch: List[int]) -> torch.Tensor:
        ...
        result = (
            torch.cat(tokens, dim=0).to(self.torch_device)
            if tokens
            else torch.empty(
                (0, 1), dtype=torch.int64, device=self.torch_device
            )
    ...
    def _sync_completion_status(
        ...
        completion_mask = torch.zeros(
            len(decode_uuids), dtype=torch.int32, device=self.torch_device
        )
    ...
    def _decoding_legacy_modes(
        ...
        while new_token_idx < self.max_decoding_length and (
            decode_uuids or batch
        ):
            if self.rank == 0:
            ...
            if (
                new_token_idx > 0
                and new_token_idx % self.DECISION_INTERVAL == 0
            ):
                dist.barrier()

                # FIXED: Use updated _check_and_handle_completions
                decode_uuids, batch, completed_uuids = (
                    self._check_and_handle_completions(
                ...
                if decode_uuids:
                    decode_uuids, batch = self._try_load_new_sequences(
                        decode_uuids, batch
                    )
            ...
            if RUNTIME_ATTN_MODE == 0:
                ...
                with torch.inference_mode():
                    ...
                    seqlens_tensor = torch.tensor(
                        cache_seqlens, dtype=torch.int64
                    )
                    positions = torch.arange(max_len)
                    attention_mask = (
                        positions.unsqueeze(0) < seqlens_tensor.unsqueeze(1)
                    ).to(torch.int64)
                    ...
                    for i, local_idx in enumerate(batch):
                        ...
                        seq.current_context_length = (
                            seq.prompt_length + new_token_idx + 1
                        )
    ...
    def init_nvshmem(self):
        ...
        if BATCHGEN_ENABLE_ALL_TO_ALL != "1" or nvshmem_init is None:
            if self.rank == 0:
                logging.debug(
                    "Skipping NVSHMEM initialization; BATCHGEN_ENABLE_ALL_TO_ALL is disabled"
                )
            return
        ...
        if getattr(self, "_nvshmem_initialized_this_run", False):
            logging.debug(
                f"Rank {self.rank}: NVSHMEM already initialized this run, skipping"
            )
            return
    ...
    def _finalize_nvshmem(self) -> None:
        ...
        try:
            ...
            logging.warning(
                f"Rank {self.rank}: Failed to finalize NVSHMEM: {e}"
            )
    ...
    def _init_torch_dist(self):
        ...
        try:
            ...
            logging.info(
                f"Rank {self.rank}: torch.distributed initialized with timeout={timeout}"
            )
        except RuntimeError as e:
    ...
    def _ensure_dist_healthy(self) -> bool:
        ...
        if not dist.is_initialized():
            logging.warning(
                f"Rank {self.rank}: torch.distributed not initialized, attempting init..."
            )
            return self._coordinated_dist_reinit(
                MAX_REINIT_RETRIES, INITIAL_RETRY_DELAY
            )
        ...
        try:
            ...
            work = dist.all_reduce(
                health_tensor, op=dist.ReduceOp.SUM, async_op=True
            )
            ...
            if abs(health_tensor.item() - expected) > 1e-6:
                raise RuntimeError(
                    f"Health check mismatch: got {health_tensor.item()}, expected {expected}"
                )

            logging.debug(
                f"Rank {self.rank}: torch.distributed health check passed"
            )
            return True

        except Exception as e:
            logging.warning(
                f"Rank {self.rank}: torch.distributed health check failed: {e}"
            )
    ...
    def _coordinated_dist_reinit(
        self, max_retries: int, initial_delay: float
    ) -> bool:
        """
        ...
        for attempt in range(max_retries):
            logging.info(
                f"Rank {self.rank}: Reinit attempt {attempt + 1}/{max_retries}"
            )
            ...
            if dist.is_initialized():
                try:
                    ...
                    logging.debug(
                        f"Rank {self.rank}: Destroyed existing process group"
                    )
                except Exception as e:
                    logging.warning(
                        f"Rank {self.rank}: Error destroying process group: {e}"
                    )
            ...
            if hasattr(self, "comm") and self.comm is not None:
                try:
                    self.comm.destroy()
                    logging.debug(
                        f"Rank {self.rank}: Destroyed PyNccl communicator"
                    )
                except Exception as e:
                    logging.warning(
                        f"Rank {self.rank}: Error destroying PyNccl communicator: {e}"
                    )
                self.comm = None
            ...
            logging.debug(
                f"Rank {self.rank}: Waiting {rank_delay:.1f}s before reinit..."
            )
            ...
            try:
                ...
                logging.info(
                    f"Rank {self.rank}: torch.distributed reinitialized successfully on attempt {attempt + 1}"
                )
                return True
            except Exception as e:
                logging.warning(
                    f"Rank {self.rank}: Reinit attempt {attempt + 1} failed: {e}"
                )
                delay *= 2  # Exponential backoff

        logging.error(
            f"Rank {self.rank}: Failed to reinitialize torch.distributed after {max_retries} attempts"
        )
        return False
    ...
    def _check_and_reinit_distributed(self) -> bool:
        ...
        if not dist.is_initialized():
            logging.warning(
                f"Rank {self.rank}: torch.distributed not initialized, attempting to initialize..."
            )
            try:
                self._init_torch_dist()
                return True
                ...
                logging.error(
                    f"Rank {self.rank}: Failed to initialize torch.distributed: {e}"
                )
                return False
        ...
        try:
            ...
            if abs(health_tensor.item() - expected) > 1e-6:
                raise RuntimeError(
                    f"Health check result mismatch: got {health_tensor.item()}, expected {expected}"
                )
            ...
            logging.warning(
                f"Rank {self.rank}: Distributed health check failed: {e}"
            )
            logging.info(
                f"Rank {self.rank}: Attempting to reinitialize torch.distributed..."
            )
            ...
            try:
                ...
                logging.warning(
                    f"Rank {self.rank}: Error destroying process group: {destroy_e}"
                )

            try:
                self._init_torch_dist()
                logging.info(
                    f"Rank {self.rank}: Successfully reinitialized torch.distributed"
                )
                return True
            except Exception as reinit_e:
                logging.error(
                    f"Rank {self.rank}: Failed to reinitialize torch.distributed: {reinit_e}"
                )
                return False
    ...
    def _proactive_dist_reinit(self) -> None:
        ...
        """
        logging.info(
            f"Rank {self.rank}: Proactively reinitializing torch.distributed for new batch"
        )
        ...
        if hasattr(self, "_nccl_group") and self._nccl_group is not None:
            try:
                ...
                logging.warning(
                    f"Rank {self.rank}: Error destroying PyNccl group: {e}"
                )
            self._nccl_group = None
        ...
        if hasattr(self, "_nccl_port"):
            self._nccl_port += 1
            logging.debug(
                f"Rank {self.rank}: Incremented PyNccl port to {self._nccl_port}"
            )
        ...
        try:
            self._init_torch_dist()
            logging.info(
                f"Rank {self.rank}: torch.distributed reinitialized successfully"
            )
        except Exception as e:
            logging.error(
                f"Rank {self.rank}: Failed to reinitialize torch.distributed: {e}"
            )
            raise RuntimeError(
                f"Rank {self.rank}: Failed to reinitialize torch.distributed: {e}"
            )
    ...
    def _check_and_reinit_pynccl(self) -> bool:
        ...
        if not self.comm.available:
            logging.debug(
                f"Rank {self.rank}: PyNccl communicator not available, skipping health check"
            )
            return True
        ...
        try:
            ...
            health_tensor = torch.ones(
                1, device=self.torch_device, dtype=torch.float32
            )
            ...
            if abs(health_tensor.item() - expected) > 1e-6:
                raise RuntimeError(
                    f"PyNccl health check mismatch: got {health_tensor.item()}, expected {expected}"
                )
            ...
            logging.warning(
                f"Rank {self.rank}: PyNccl health check failed: {e}"
            )
            logging.info(
                f"Rank {self.rank}: Attempting to reinitialize PyNccl communicator..."
            )
            ...
            try:
                if self.comm is not None:
                    ...
                    self.comm = None
                    logging.info(
                        f"Rank {self.rank}: NCCL communicator destroyed successfully"
                    )
            except Exception as destroy_e:
                logging.warning(
                    f"Rank {self.rank}: Error destroying PyNccl communicator: {destroy_e}"
                )
                self.comm = None
            ...
            try:
                if self._nccl_group is not None:
                    ...
                    gc.collect()
                    logging.info(
                        f"Rank {self.rank}: NCCL group destroyed successfully"
                    )
            except Exception as group_e:
                logging.warning(
                    f"Rank {self.rank}: Error destroying NCCL group: {group_e}"
                )
                self._nccl_group = None
            ...
            try:
                if dist.is_initialized():
                    dist.barrier()
                    logging.debug(
                        f"Rank {self.rank}: Barrier after NCCL cleanup passed"
                    )
            except Exception as barrier_e:
                logging.warning(
                    f"Rank {self.rank}: Barrier after NCCL cleanup failed: {barrier_e}"
                )
            ...
            if self.rank == 0:
                try:
                    self._nccl_port = _find_available_port(
                        comm_master_addr, self._nccl_port + 1
                    )
                    logging.debug(
                        f"Rank 0: Found available port {self._nccl_port} for PyNccl reinit"
                    )
                except RuntimeError as e:
            ...
            port_tensor = torch.tensor(
                [self._nccl_port], dtype=torch.int32, device=self.torch_device
            )
            dist.broadcast(port_tensor, src=0)
            self._nccl_port = port_tensor.item()
            logging.debug(
                f"Rank {self.rank}: Next PyNccl port will be {self._nccl_port}"
            )
        ...
        for layer_idx in range(len(self.model.model.layers)):
            ...
            if layer_idx >= self.loaded_model_config.first_k_dense_replace:
                shared_experts = getattr(
                    self.model.model.layers[layer_idx].mlp,
                    "shared_experts",
                    None,
                )
                if shared_experts is not None and hasattr(
                    ...
                    shared_experts._unregister_fp8_weights()
                for routed_expert_idx in range(
                    self.model_config.num_local_experts
                ):
                    if hasattr(
                        self.model.model.layers[layer_idx].mlp.experts[
                            routed_expert_idx
                        ],
    ...
    def _handle_hot_reload(self, msg: dict) -> dict:
        ...
        try:
            reload_deps = (
                msg.get("reload_deps", True) if isinstance(msg, dict) else True
            )
            ...
            if reload_deps:
                ...
                for mod_name in dep_modules:
                    if mod_name in sys.modules:
                        ...
                        _log.info(
                            f"Rank {self.rank}: Reloaded dependency {mod_name}"
                        )
            ...
            try:
                ...
                if new_init_src != old_init_src:
                    new_attrs = set(
                        re.findall(r"self\.(\w+)\s*=", new_init_src)
                    )
                    missing = sorted(
                        [a for a in new_attrs if not hasattr(self, a)]
                    )
                    if missing:
                        _log.warning(
                ...
                try:
                    if isinstance(descriptor, staticmethod):
                        ...
                        setattr(
                            self,
                            name,
                            descriptor.__func__.__get__(type(self), type(self)),
                        )
                    elif inspect.isfunction(descriptor):
                        setattr(
                            self, name, descriptor.__get__(self, type(self))
                        )
                    else:
                        continue
                    rebound += 1
            ...
            _log.error(
                f"Rank {self.rank}: Hot reload FAILED: {e}", exc_info=True
            )
            result = {
    ...
    def _write_reload_status(self, result: dict) -> None:
        ...
        try:
            ...
            _log.warning(
                f"Rank {self.rank}: Failed to write reload status: {e}"
            )
    ...
    def _reset_for_new_batch(self) -> None:
        ...
        if not self._ensure_dist_healthy():
            raise RuntimeError(
                f"Rank {self.rank}: Failed to ensure healthy torch.distributed connection"
            )
        ...
        if hasattr(self, "global_batch") and self.global_batch is not None:
            try:
                ...
                if (
                    worker_view is not None
                    and hasattr(self, "_uuid_to_local_map")
                    and self._uuid_to_local_map
                ):
                        ...
                        for seq_id in global_ids_to_release:
                            try:
                                ...
                                if aux_view_shutdown is not None:
                                    aux_view_shutdown.release_sequence_pages(
                                        [seq_id]
                                    )
                                released_count += 1
                            except Exception:
                                ...
                                pass
                        logging.info(
                            f"Rank {self.rank}: Released {released_count}/{len(global_ids_to_release)} sequences (others already released)"
                        )
            except Exception as e:
                logging.warning(
                    f"Rank {self.rank}: Failed to cleanup host KV: {e}"
                )
        ...
        if hasattr(self, "model") and self.model is not None:
            try:
                ...
                logging.warning(
                    f"Rank {self.rank}: Failed to cleanup model: {e}"
                )
        self.model = None
        self._cuda_graph_manager = None
        self._glm5_moe_cuda_graph_manager = None

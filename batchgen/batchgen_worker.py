...
class BatchGenWorker:
	...
	def _finish_glm5_prefill_offload_audit(self, local_prefill_indices) -> None:
		...
		# The current packed path enqueues one batched task per offload call,
		# rather than one task per sequence. Validate the task lifecycle against
		# the recorded calls so this proof follows the production batching seam.
		expected_tasks = Counter({
			layer_idx: primary[layer_idx]["calls"]
			+ (aux[layer_idx]["calls"] if layer_idx in aux else 0)

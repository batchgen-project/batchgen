...
class BatchGenWorker:
	...
	def __init__(self, args: BatchGenWorkerArgs):
		...
		self._prefill = PrefillScheduler(self._state, self._index, self._kv, worker=self)
		self._boundary = BoundaryHandler(self._state, self._index, self._kv, worker=self)
		self._decode = DecodeScheduler(self._state, self._index, self._kv, self._boundary, worker=self)
		self._rebalancer = HostKVRebalancer(self._state, self._index, self._kv, self._sync, worker=self)
		self._completion = CompletionHandler(self._state, worker=self)
	...
	def _should_stop_at_eos(self, token_id: int) -> bool:
		return self._completion.should_stop_at_eos(token_id)

	def _is_sequence_completed(self, seq) -> bool:
		return self._completion.is_completed(seq)

	def _get_finish_reason(self, seq) -> str:
		return self._completion.get_finish_reason(seq)
	...
	def _initialize_core_components(self, num_queries: int) -> None:
		...
		self._state.parallel_manager = self.parallel_manager
	...
	def generate(self):
		...
		while not self.global_batch.all_completed():
			...
			if self.global_batch.has_queueing() or (self.enable_host_kv_eviction and self.global_batch.has_evicted()):
				...
				prefill_uuids = self._prefill.prepare_batch()
				...
				if prefill_uuids:
					...
					self._prefill.config_for_batch(prefill_uuids)
					...
					if local_prefill_indices:
						...
						with torch.inference_mode():
							if self.enable_prepack:
								self._prefill.run_prefill_prepacked(local_prefill_indices)
							else:
								self._prefill.run_prefill(local_prefill_indices)
			...
			while (self.global_batch.has_prefilled() or
			   ...
			   self.global_batch.has_on_hold()):
				...
				self._decode.load_model(max_num_seq_estimate, self.comm)
				...
				with torch.inference_mode():
					...
					self._decode.run_continuous(new_tokens, decode_uuids, local_decode_indices)
				...
				if needs_prefill:
					...
					if in_decode_uuids:
						self._rebalancer.put_on_hold(in_decode_uuids)
					break
	...
	def _decoding_legacy_modes(
		...
		while new_token_idx < self.max_decoding_length and (decode_uuids or batch):
			...
			if new_token_idx > 0 and new_token_idx % self.DECISION_INTERVAL == 0:
				...
				decode_uuids, batch, completed_uuids = self._completion.check_and_handle(
				...
				if decode_uuids:
					decode_uuids, batch = self._decode.try_load_new(decode_uuids, batch)

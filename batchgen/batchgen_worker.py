...
class BatchGenWorker:
	...
	def _append_decode_kv_to_host_aux_async(
		...
		Appends decode-time auxiliary (indexer) KV to the auxiliary host worker
		view. Shares the same pending task list as the primary path for unified
		flushing.
		"""
	...
	def prefill(self, batch: list[int]):
		"""Handle prefill for a batch (delegates to worker.prefill.run_prefill)."""
		return run_prefill(self, batch)

	def prefill_prepacked(self, batch: list[int]):
		"""Prepacked prefill (delegates to worker.prefill.run_prefill_prepacked)."""
		return run_prefill_prepacked(self, batch)

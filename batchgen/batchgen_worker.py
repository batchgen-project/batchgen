...
class BatchGenWorker:
	...
	def _initialize_core_components(self, num_queries: int) -> None:
		...
		if "gpt-oss" in self.model_name.lower() or "gpt_oss" in self.model_name.lower():
			try:
				...
				logging.info(f"Rank {self.rank}: Loading GPT-OSS tokenizer via tiktoken (o200k_harmony)")
	...
	def _config_decoding_for_batch(
		...
		logging.info("[DECODE DEBUG] Before torch.cuda.synchronize()")
		torch.cuda.synchronize(self.torch_device)
		logging.info("[DECODE DEBUG] After torch.cuda.synchronize()")
		torch.cuda.empty_cache()
		logging.info("[DECODE DEBUG] After torch.cuda.empty_cache()")
		free_mem_bytes, total_mem_bytes = torch.cuda.mem_get_info(self.local_rank)
		free_mem_gb = free_mem_bytes / (1024 ** 3)
		total_mem_gb = total_mem_bytes / (1024 ** 3)
		used_mem_gb = total_mem_gb - free_mem_gb

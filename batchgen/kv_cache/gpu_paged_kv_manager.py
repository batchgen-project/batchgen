...
class _GPUPageTableManager:
    ...
    def rebuild(
        ...
        shape_match = (
            self.gpu_table is not None and self.gpu_table.shape[0] == num_slots
        )
        cols_sufficient = (
            self.gpu_table is not None
            and self.gpu_table.shape[1] >= desired_pages
        )
        ...
        if BATCHGEN_CB_DEBUG:
            final_shape = (
                self.gpu_table.shape if self.gpu_table is not None else None
            )
    ...
    def _update_cuda_graph_table(
        ...
        if (
            ...
            or max_required > self.graph_max_pages_per_sequence
        ):
...
class GPUPagedKVCacheManager:
    ...
    def clear_page_table(self) -> None:
        ...
        max_pages = (
            mgr.max_pages_per_sequence if mgr.max_pages_per_sequence > 0 else 1
        )
        ...
        mgr.seq_id_to_slot = {}
        mgr.slot_to_seq_id = []
        mgr._slot_index_tensor = torch.empty(
            0, dtype=torch.int32, device=mgr.device
        )
        mgr._slot_to_seq_id_tensor = torch.empty(
            0, dtype=torch.int64, device=mgr.device
        )
    ...
    def rebuild_page_table(self, sequence_ids: Sequence[int]) -> torch.Tensor:
        ...
        old_shape = (
            self._gpu_page_table_manager.gpu_table.shape
            if self._gpu_page_table_manager.gpu_table is not None
            else None
        )
    ...
    def update_layer_decode_new_token(
        ...
        layer_idx: int,
        batch_slice: Optional[
            tuple
        ] = None,  # (start_idx, end_idx) for micro-batching
        ...
        debug_kv_write = (
            layer_idx == 0
            and os.environ.get("BATCHGEN_DEBUG_KV_WRITE", "0") == "1"
        )
        if debug_kv_write:
            print(f"\n[GPU KV WRITE L0] === Before Triton Kernel ===")
            print(
                f"[GPU KV WRITE L0] batch_size={batch_size}, k_tokens.shape={k_tokens.shape}"
            )
            print(
                f"[GPU KV WRITE L0] slot_indices[:5]={slot_indices[: min(5, len(slot_indices))].tolist()}"
            )
            print(
                f"[GPU KV WRITE L0] token_indices[:5]={token_indices[: min(5, len(token_indices))].tolist()}"
            )
            print(f"[GPU KV WRITE L0] page_table.shape={page_table_view.shape}")
            print(
                f"[GPU KV WRITE L0] page_size_tokens={self.config.page_size_tokens}"
            )
            print(
                f"[GPU KV WRITE L0] k_tokens[0,:8]={k_tokens[0, :8].tolist()}"
            )
            if v_tokens is not None:
                print(
                    f"[GPU KV WRITE L0] v_tokens[0,:8]={v_tokens[0, :8].tolist()}"
                )
            else:
                print(
                    f"[GPU KV WRITE L0] v_tokens=None (BUG: V should be written for GQA!)"
                )
        ...
        if debug_kv_write:
            ...
            k_written_head0 = k_tokens[
                0, 0:head_dim
            ].tolist()  # head 0, all 64 dims
            k_read_head0 = k_cache_layer[
                gpu_page, offset, 0, :
            ].tolist()  # head 0, all 64 dims

            print(f"[GPU KV WRITE L0] === After Triton Kernel ===")
            print(
                f"[GPU KV WRITE L0] seq0: slot={seq0_slot}, pos={seq0_pos}, page_idx={page_idx}, offset={offset}, gpu_page={gpu_page}"
            )
            print(
                f"[GPU KV WRITE L0] k_tokens[0, 0:8] (head0 dims 0-7) = {k_written_head0[:8]}"
            )
            print(
                f"[GPU KV WRITE L0] k_cache[{gpu_page}][{offset}][0,:8] (head0 dims 0-7) = {k_read_head0[:8]}"
            )

            # Check if ALL dims of head 0 match
            k_match = all(
                abs(w - r) < 1e-3 for w, r in zip(k_written_head0, k_read_head0)
            )
            print(
                f"[GPU KV WRITE L0] K head0 write {'SUCCEEDED' if k_match else 'FAILED'}"
            )
            if not k_match:
                # Find first mismatch
                for i, (w, r) in enumerate(zip(k_written_head0, k_read_head0)):
                    if abs(w - r) >= 1e-3:
                        print(
                            f"[GPU KV WRITE L0] K head0 MISMATCH at dim {i}: written={w}, read={r}"
                        )
                        break
            ...
            if v_cache_layer is not None and v_tokens is not None:
                ...
                print(
                    f"[GPU KV WRITE L0] v_tokens[0, 0:8] (head0 dims 0-7) = {v_written_head0[:8]}"
                )
                print(
                    f"[GPU KV WRITE L0] v_cache[{gpu_page}][{offset}][0,:8] (head0 dims 0-7) = {v_read_head0[:8]}"
                )
                v_match = all(
                    abs(w - r) < 1e-3
                    for w, r in zip(v_written_head0, v_read_head0)
                )
                print(
                    f"[GPU KV WRITE L0] V head0 write {'SUCCEEDED' if v_match else 'FAILED'}"
                )
                if not v_match:
                    for i, (w, r) in enumerate(
                        zip(v_written_head0, v_read_head0)
                    ):
                        if abs(w - r) >= 1e-3:
                            print(
                                f"[GPU KV WRITE L0] V head0 MISMATCH at dim {i}: written={w}, read={r}"
                            )
                            break
    ...
    def copy_kv_to_tensor(self, sequence_id: int) -> torch.Tensor:
        ...
        if slot is None:
            raise ValueError(
                f"copy_kv_to_tensor: sequence {sequence_id} not found in page table"
            )
        ...
        if state is None:
            raise ValueError(
                f"copy_kv_to_tensor: sequence {sequence_id} has no state"
            )
        ...
        for page_offset, page_idx in enumerate(page_indices):
            ...
            k_output[:, page_offset, :, :, :] = self._k_cache[
                :, page_idx_int, :, :, :
            ]
    ...
    def copy_tensor_to_kv(
        self, sequence_id: int, k_tensor: torch.Tensor
    ) -> None:
        ...
        if state is None:
            raise ValueError(
                f"copy_tensor_to_kv: sequence {sequence_id} has no state"
            )
        ...
        for page_offset, page_idx in enumerate(page_indices):
            ...
            # Copy all layers for this page
            # _k_cache shape: [num_layers, num_pages, page_size, num_heads, head_dim]
            self._k_cache[:, page_idx_int, :, :, :] = k_tensor[
                :, page_offset, :, :, :
            ]
    ...
    def resolve_physical_layer(self, logical_layer_id: int) -> int:
        ...
        if logical_layer_id < 0:
            raise IndexError(
                "resolve_physical_layer: logical layer id must be >= 0"
            )
    ...
    def _resolve_page_table_max_pages_per_sequence(self) -> int:
        ...
        if self._engine_config is not None:
            ...
            if max_prompt is not None and max_decode is not None:
                token_capacity = max(
                    token_capacity, int(max_prompt) + int(max_decode)
                )
            ...
            prepack_capacity = (
                self._engine_config.Module_Batching_Config.prepack_row_capacity
            )

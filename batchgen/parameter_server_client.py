"""
Client library for communicating with the BatchGen Parameter Server.
...
class ParameterServerClient:
    ...
    def connect(self):
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to connect to parameter server at {self.host}:{self.port}: {e}"
            )
    ...
    def send_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not size_data or len(size_data) < 4:
                raise ConnectionError(
                    "Connection closed by server before receiving response size"
                )
            ...
            while bytes_received < response_size:
                chunk = self.socket.recv(
                    min(8192, response_size - bytes_received)
                )
                if not chunk:
                    raise ConnectionError(
                        f"Connection closed by server during data transfer after receiving {bytes_received}/{response_size} bytes"
                    )

                # Copy chunk into the correct position in the response data buffer
                response_data[bytes_received : bytes_received + len(chunk)] = (
                    chunk
                )
            ...
            raise ConnectionError(
                "Timeout while communicating with parameter server"
            )
        except socket.error as e:
            self.socket = None  # Mark as disconnected
            raise ConnectionError(
                f"Socket error communicating with parameter server: {e}"
            )
        except Exception as e:
            self.socket = None  # Mark as disconnected
    ...
    def load_model(
        ...
        if hf_cache_dir is not None:
            request["hf_cache_dir"] = str(hf_cache_dir)
        if cache_dir is not None:
            request["cache_dir"] = str(cache_dir)
        if converted_ckpt_dir is not None:
            request["converted_ckpt_dir"] = str(converted_ckpt_dir)

        response = self.send_request(request)

        if response.get("status") != "success":
            raise RuntimeError(
                f"Failed to load model: {response.get('message', 'Unknown error')}"
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if response.get("status") != "success":
            raise RuntimeError(
                f"Failed to get model info: {response.get('message', 'Unknown error')}"
            )

        # Check if we need to get the skeleton state dict from file
        skeleton_state_dict_file = response.get("skeleton_state_dict_file")

        if skeleton_state_dict_file and os.path.exists(
            skeleton_state_dict_file
        ):
            try:
                logging.info(
                    f"Loading skeleton state dict from file: {skeleton_state_dict_file}"
                )
                ...
                logging.info(
                    f"Successfully loaded skeleton state dict with {len(skeleton_state_dict)} keys"
                )
            ...
            if skeleton_state_dict_shm_name:
                ...
                search_paths = [
                    ...
                    os.path.join(
                        tempfile.gettempdir(), skeleton_state_dict_shm_name
                    ),
                    # 2. Legacy backup directory
                    os.path.join(
                        os.getcwd(),
                        "shared_memory_backup",
                        skeleton_state_dict_shm_name,
                    ),
                ]
                ...
                for file_path in search_paths:
                    if os.path.exists(file_path):
                        try:
                            logging.info(
                                f"Loading skeleton state dict from: {file_path}"
                            )
                            ...
                            if file_path.endswith(".pt"):
                                ...
                                with open(file_path, "rb") as f:
                                    ...
                                    for i in range(
                                        0, serialized_size, chunk_size
                                    ):
                                        end = min(
                                            i + chunk_size, serialized_size
                                        )
                                        chunk = f.read(end - i)
                            ...
                            logging.warning(
                                f"Error loading from {file_path}: {e}, trying next location..."
                            )
                            continue

                if not file_found:
                    raise RuntimeError(
                        f"Skeleton state dict file not found in any location: {search_paths}"
                    )

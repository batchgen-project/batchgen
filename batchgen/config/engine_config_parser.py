...
def parse_config_from_json(config_path: str) -> EngineConfig:
    ...
    if "Basic_Config" in config_dict:
        _parse_basic_config(engine_config.Basic_Config, config_dict["Basic_Config"])
    # else:
    ...
    if "Module_Batching_Config" in config_dict:
        _parse_module_batching_config(engine_config.Module_Batching_Config, config_dict["Module_Batching_Config"])
    # else:
    ...
    if "GPU_Buffer_Config" in config_dict:
        _parse_gpu_buffer_config(engine_config.GPU_Buffer_Config, config_dict["GPU_Buffer_Config"])
    # else:
    ...
    if "KV_Storage_Config" in config_dict:
        _parse_kv_storage_config(engine_config.KV_Storage_Config, config_dict["KV_Storage_Config"])
    # else:
...
def _parse_basic_config(basic_config: BasicConfig, config_dict: Dict[str, Any]) -> None:
    """
    ...
    if basic_config.weight_dtype:
        basic_config.weight_dtype_torch = BasicConfig._str_to_torch_dtype(basic_config.weight_dtype)
    
    if basic_config.kv_dtype:
        basic_config.kv_dtype_torch = BasicConfig._str_to_torch_dtype(basic_config.kv_dtype)
    
    if basic_config.activation_dtype:
        basic_config.activation_dtype_torch = BasicConfig._str_to_torch_dtype(basic_config.activation_dtype)
...
def _parse_module_batching_config(module_config: ModuleBatchingConfig, config_dict: Dict[str, Any]) -> None:
    """
...
def _parse_gpu_buffer_config(gpu_config: GPUBufferConfig, config_dict: Dict[str, Any]) -> None:
    """
...
def _parse_kv_storage_config(kv_config: KVStorageConfig, config_dict: Dict[str, Any]) -> None:
    """

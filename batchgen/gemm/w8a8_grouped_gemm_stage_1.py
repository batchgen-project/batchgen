...
@triton.jit
def fused_fp8_moe_persistent_kernel_v2_fixed(
    lhs_ptr, lhs_scale_ptr,
    gate_ptrs_ptr, up_ptrs_ptr,
    gate_scale_ptrs_ptr, up_scale_ptrs_ptr,
    group_idx_ptr, group_sizes_ptr, group_start_indices_ptr,
    ...
    GEMM_BLOCK_SIZE_K: tl.constexpr,
    SCALE_BLOCK_SIZE_K: tl.constexpr,
    MAX_EXPERTS_PER_CTA: tl.constexpr,  # Upper bound for loop
):
    num_groups = tl.load(num_active_experts_ptr)
    ...
    # ============================================================
    # PERSISTENT LOOP: Each CTA grabs one EXPERT at a time
    # Use bounded loop instead of while True + break
    # ============================================================
    for _ in range(MAX_EXPERTS_PER_CTA):
        # Atomically grab next expert
        group_pid = tl.atomic_add(work_counter_ptr, 1)
        
        # Guard: skip if no more work
        if group_pid < num_groups:
            gm = tl.load(group_sizes_ptr + group_pid * stride_group_sizes)
            ...
            if gm > 0:
                ...
                m_mask = abs_row_indices < M
                valid_mask = offsets_m < gm
                m_base_mask = m_mask & valid_mask
                
                # Process all N-tiles for this expert
                for n_pid in range(num_n_tiles):
                    offsets_n = n_pid * GEMM_BLOCK_SIZE_N + tl.arange(0, GEMM_BLOCK_SIZE_N)
                    n_mask = offsets_n < N
                    ...
                    # K-loop
                    for k_block_idx in range(tl.cdiv(K, GEMM_BLOCK_SIZE_K)):
                        ...
                        lhs_scale_ptrs = lhs_scale_ptr + (abs_row_indices[:, None] * stride_lhs_scale_m + k_block_idx)
                    ...
                    # Store results
                    offs_output_m = start_idx + tl.arange(0, GEMM_BLOCK_SIZE_M)
                    offs_output_n = n_pid * GEMM_BLOCK_SIZE_N + tl.arange(0, GEMM_BLOCK_SIZE_N)
                    
                    output_ptrs = output_ptr + (offs_output_m[:, None] * stride_output_m + offs_output_n[None, :] * stride_output_n)
                    output_mask = valid_mask[:, None] & (offs_output_m[:, None] < M) & (offs_output_n[None, :] < N)
...
def fused_fp8_moe_stage_1_persistent_v2(
    ...
    if num_ctas is None:
        props = torch.cuda.get_device_properties(device)
        num_sms = props.multi_processor_count
        num_ctas = min(num_sms * 2, num_groups)
    
    # Calculate max experts per CTA (with buffer for imbalance)
    max_experts_per_cta = (num_groups + num_ctas - 1) // num_ctas + 5

    work_counter = torch.zeros(1, dtype=torch.int32, device=device)
    
    grid = (num_ctas,)
    
    fused_fp8_moe_persistent_kernel_v2_fixed[grid](
        hidden_states, hidden_states_scale,
        gate_ptrs_ptr, up_ptrs_ptr,
        gate_scale_ptrs_ptr, up_scale_ptrs_ptr,
        activated_group_idx, group_sizes, group_start_indices,
        ...
        GEMM_BLOCK_SIZE_K=gate_gemm_block_size[2],
        SCALE_BLOCK_SIZE_K=scale_block_size,
        MAX_EXPERTS_PER_CTA=max_experts_per_cta,
        num_stages=num_stages,
        num_warps=num_warps
    )
    return output
...
#     gate_scale_ptrs_ptr, up_scale_ptrs_ptr,
#     group_idx_ptr, group_sizes_ptr, group_start_indices_ptr,
#     work_queue_ptr,  # [total_work_items, 3]: (expert_id, m_block, n_block)
#     total_work_items,
#     output_ptr,
#     M, N: tl.constexpr, K: tl.constexpr,
#     stride_lhs_m, stride_lhs_k,
...
# ):
#     """
#     PERSISTENT KERNEL: Each program processes multiple work items.
#     Perfect load balancing for variable group sizes!
    
#     Work queue format: [total_work_items, 3]
#     - Column 0: expert_id (which expert)
#     - Column 1: m_block_id (which M-block within expert)
#     - Column 2: n_block_id (which N-block)
#     """
#     pid = tl.program_id(0)
#     num_programs = tl.num_programs(0)
    
#     # Each program processes multiple work items with stride
#     work_idx = pid
...
#         # ====================================================================
#         group_pid = tl.load(work_queue_ptr + work_idx * 3 + 0).to(tl.int32)
#         m_block_id = tl.load(work_queue_ptr + work_idx * 3 + 1).to(tl.int32)
#         n_block_id = tl.load(work_queue_ptr + work_idx * 3 + 2).to(tl.int32)
        
#         # ====================================================================
#         # LOAD EXPERT METADATA
#         # ====================================================================
#         gm = tl.load(group_sizes_ptr + group_pid * stride_group_sizes)
#         if gm > 0:  # Only process if group is non-empty
#             group_idx = tl.load(group_idx_ptr + group_pid * stride_group_idx)
#             start_idx = tl.load(group_start_indices_ptr + group_pid * stride_group_start_indices)
...
#             # ================================================================
#             # COMPUTE THIS TILE: (expert, m_block, n_block)
#             # ================================================================
            
#             # N-dimension setup (which N-block)
#             offsets_n = n_block_id * GEMM_BLOCK_SIZE_N + tl.arange(0, GEMM_BLOCK_SIZE_N)
#             n_mask = offsets_n < N
#             num_scale_k = tl.cdiv(K, SCALE_BLOCK_SIZE_K)
#             scale_n_idx = n_block_id * GEMM_BLOCK_SIZE_N // SCALE_BLOCK_SIZE_K
            
#             # M-dimension setup (which M-block)
#             offsets_m = tl.arange(0, GEMM_BLOCK_SIZE_M)
#             sub_group_start_idx = start_idx + m_block_id * GEMM_BLOCK_SIZE_M
...
#             # Store results
#             offs_output_m = sub_group_start_idx + tl.arange(0, GEMM_BLOCK_SIZE_M)
#             offs_output_n = n_block_id * GEMM_BLOCK_SIZE_N + tl.arange(0, GEMM_BLOCK_SIZE_N)
#             output_ptrs = output_ptr + (offs_output_m[:, None] * stride_output_m + 
#                                         offs_output_n[None, :] * stride_output_n)
#             output_mask = (offs_output_m[:, None] < M) & (offs_output_n[None, :] < N) & valid_mask[:, None]
#             tl.store(output_ptrs, output, mask=output_mask)
...
# ):
#     """
#     PERSISTENT KERNEL version with dynamic work queue.
#     Perfect load balancing for variable group sizes!
#     """
#     device = hidden_states.device
#     M = hidden_states.shape[0]
...
#     BLOCK_M = gate_gemm_block_size[0]
#     BLOCK_N = gate_gemm_block_size[1]
#     BLOCK_K = gate_gemm_block_size[2]
    
#     assert BLOCK_K == scale_block_size, \
#         f"GEMM_BLOCK_SIZE_K ({BLOCK_K}) must equal SCALE_BLOCK_SIZE_K ({scale_block_size})"
...
#     # Calculate work items per expert
#     work_items_list = []
#     for expert_id in range(num_groups):
#         gm = group_sizes[expert_id].item()
#         if gm > 0:
#             num_m_blocks = triton.cdiv(gm, BLOCK_M)
#             # Each expert contributes: num_m_blocks × num_n_blocks work items
#             for m_block in range(num_m_blocks):
#                 for n_block in range(num_n_blocks):
#                     work_items_list.append([expert_id, m_block, n_block])
    
#     if len(work_items_list) == 0:
#         return output  # No work to do
    
#     # Create work queue tensor
#     work_queue = torch.tensor(work_items_list, dtype=torch.int32, device=device)
#     total_work_items = work_queue.shape[0]
    
#     # ========================================================================
#     # LAUNCH PERSISTENT KERNEL
#     # ========================================================================
#     # Heuristic: Use 80% of available SMs × waves
#     # This ensures good occupancy while allowing flexibility
#     num_programs = min(total_work_items, 1024)  # Cap at 1024 programs

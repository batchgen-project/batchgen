...
def run_tuner(batch_size=8, seq_len=13000, dim=576):
    ...
    """
    print(f"Running tuner with shape: batch={batch_size}, seq_len={seq_len}, dim={dim}")
    
    # Define block sizes to test
    block_sizes = [128, 64, 32]
    
    # Create test data
    q = torch.randn((batch_size, seq_len, dim), device='cuda', dtype=torch.bfloat16).to(torch.float8_e4m3fn)
    ...
    for block_size in block_sizes:
        ...
        scale = torch.ones((batch_size, seq_len, num_blocks), device='cuda', dtype=torch.float32)
        ...
        try:
            ...
            for _ in range(iterations):
                output = dequant_per_token_triton(q, scale, BLOCK_SIZE=block_size)
            end.record()
            torch.cuda.synchronize()
            ...
            if reference_output is None:
                ...
                print(f"  Reference implementation (block_size={block_size}): {elapsed_time:.4f} ms")
            else:
                # Check accuracy
                max_diff = (output - reference_output).abs().max().item()
                mean_diff = (output - reference_output).abs().mean().item()
                is_close = torch.allclose(output, reference_output, atol=1e-2, rtol=1e-2)
    ...
    A = torch.randn((batch_size * seq_len, dim), device='cuda', dtype=torch.bfloat16)
    ...
    try:
        ...
        scale_ref = torch.ones((batch_size, seq_len, num_blocks), device='cuda', dtype=torch.float32)
        ...
        ref_result = torch.matmul(A, dequant_ref.view(batch_size * seq_len, dim).T)
        end.record()
        torch.cuda.synchronize()
        ...
        for block_size in block_sizes:
            num_blocks = (dim + block_size - 1) // block_size
            scale_test = torch.ones((batch_size, seq_len, num_blocks), device='cuda', dtype=torch.float32)
            ...
            # Warmup
            for _ in range(5):
                dequant_out = dequant_per_token_triton(q, scale_test, BLOCK_SIZE=block_size)
                _ = torch.matmul(A, dequant_out.view(batch_size * seq_len, dim).T)
            torch.cuda.synchronize()
            ...
            start.record()
            dequant_out = dequant_per_token_triton(q, scale_test, BLOCK_SIZE=block_size)
            test_result = torch.matmul(A, dequant_out.view(batch_size * seq_len, dim).T)
            end.record()
            torch.cuda.synchronize()
            ...
            is_close = torch.allclose(test_result, ref_result, atol=1e-2, rtol=1e-2)

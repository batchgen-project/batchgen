...
def sequence_token_ids_for_prefix_commit(
    ...
    *,
    ...
    prompt_token_count = int(getattr(seq, "prompt_length"))
    prompt_tensor = getattr(seq, "input_ids").reshape(-1)
    prompt_token_ids = [
        int(token_id)
    ...
    if include_new_decode_tokens:
        decoded_start = int(getattr(seq, "reentry_decoded_baseline", 0))
        decoded_length = int(getattr(seq, "decoded_length", 0))
        decoded_tensor = getattr(seq, "decoded_tokens", None)
...
def build_sequence_prefix_commit_request(
    *,
    ...
    total_tokens = int(getattr(seq, "prompt_length")) + new_decode_tokens
    ...
    pages_by_group = collect_required_group_pages_for_commit(
        ...
        sequence_id=int(getattr(seq, "global_idx")),

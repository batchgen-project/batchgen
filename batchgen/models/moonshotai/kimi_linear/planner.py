...
def k3_deepep_low_latency(*, sharded_latent: bool, moe_exchange: str = "auto") -> bool:
    """DeepEP LL replaces the 16-rank NCCL gather + reduce_scatter of the
    resident-EP decode graph (slice 3). ``moe_exchange``: ``auto`` picks it
    when the sharded-latent gate (H200 TP8) holds and the K3 DeepEP build is
    importable; ``nccl`` forces the NCCL path (the SGLang-baseline-equivalent
    exchange); ``deepep`` requires it and raises when it is unavailable. The
    graph build fails fast if the exchange cannot be constructed."""
    if moe_exchange not in ("auto", "nccl", "deepep"):
        raise ValueError(f"k3_moe_exchange must be auto|nccl|deepep, got {moe_exchange!r}")
    if moe_exchange == "nccl":
        return False
    from batchgen.moe.deepep_ll import deepep_available
    available = sharded_latent and deepep_available()
    if moe_exchange == "deepep" and not available:
        raise RuntimeError(
            "k3_moe_exchange=deepep requires the TP-sharded latent (H200 TP8) and the "
...
class KimiLinearPlanner(BasePlanner):
    ...
    def __init__(self, is_k3: bool = False, stream_all_modules: bool = None,
        ...
        # Kimi-K3 decode MoE exchange (server --k3-moe-exchange): auto | nccl | deepep
        self.moe_exchange = str(moe_exchange or "auto")
    ...
    def _adjust_config_for_k3(self):
        ...
        self.config.Module_Batching_Config.k3_deepep_ll = k3_deepep_low_latency(
            sharded_latent=shard_latent, moe_exchange=self.moe_exchange,
        )

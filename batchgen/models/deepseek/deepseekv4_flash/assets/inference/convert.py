...
def cast_e2m1fn_to_e4m3fn(
    ...
    assert (
        scale.size(0) == out_dim and scale.size(1) == in_dim // fp4_block_size
    )
    ...
    scale = (
        scale.float()
        .view(bOut, fp8_block_size, bIn, -1)
        .transpose(1, 2)
        .flatten(2)
    )
    ## bOut, bIn, 1
    scale_max_offset_bits = scale.amax(dim=-1, keepdim=True) / (
        2**MAX_OFFSET_BITS
    )
    # bOut, bIn, 128*4
...
def main(hf_ckpt_path, save_path, n_experts, mp, expert_dtype):
    ...
    if os.path.exists(index_path):
        ...
        with open(index_path) as index_file:
            shard_names = sorted(
                set(json.load(index_file)["weight_map"].values())
            )
        input_files = [
            os.path.join(hf_ckpt_path, shard) for shard in shard_names
        ]
    else:
        ...
        with safe_open(file_path, framework="pt", device="cpu") as f:
            for name in f.keys():
                ...
                for i in range(mp):
                    ...
                    if name in state_dicts[i]:
                        raise RuntimeError(
                            f"Duplicate tensor name '{name}' from {file_path}; the input "
                            f"directory likely contains stale converted shards "
                            f"(e.g. model*-mp*.safetensors). Convert from a directory "
                            f"containing only the original HF shards."
                        )
    ...
    for i in trange(mp):
        ...
        for name in names:
            if name.endswith("wo_a.weight"):
                ...
                state_dicts[i][name] = (
                    weight.flatten(2, 3).flatten(0, 1).bfloat16()
                )

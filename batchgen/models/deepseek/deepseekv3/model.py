...
class DeepseekV3MoE(nn.Module):
    ...
    def _init_fp8_blockwise_weights(self):
        ...
        # NOTE: offloaded experts [n_persistent:E] hold NO resident GPU weights
        # (verified: construction leaves non-loaded experts empty; they stream from
        # the host ring into the shared staging buffer each step). So the per-rank
        # static is the n_persistent experts + attention + embed; the decode KV pool
        # is what remains (= total*frac - static). Raising the KV pool / in_decode
        # therefore requires a higher offload ratio (fewer persistent experts), not
        # freeing anything here.

...
"""DeepSeek-V3 model configuration for BatchGen.

This module provides the config class for DeepSeek-V3 model.
It replaces HuggingFace's PretrainedConfig with BatchGen's BaseModelConfig.

DeepSeek-V3 is a 671B parameter MoE model with:
...
@dataclass
class DeepSeekV3Config(BaseModelConfig):
    ...
    compressed_kv_dim: int = 576   # Total compressed KV dim for cache (kv_lora_rank + qk_rope_head_dim)

...
def set_basic_config(engine_config: EngineConfig, input_arguments):
    ...
    if not input_arguments.get("kv_dtype", None):
        ...
        if input_arguments.kv_dtype.lower() in ["bfloat16", "bf16"]:
            engine_config.Basic_Config.kv_dtype = "bfloat16"
        elif input_arguments.kv_dtype.lower() in [
            "fp8",
            "float8",
            "float8_e4m3fn",
        ]:
            ...
            raise ValueError(
                f"Unsupported kv_dtype: {input_arguments.kv_dtype}, only support ['bfloat16','float8_e4m3fn']"
            )
    ...
    if not input_arguments.get("attention_dtype", None):
        ...
        if input_arguments.attention_dtype.lower() in ["bfloat16", "bf16"]:
            engine_config.Basic_Config.attention_dtype = "bfloat16"
        elif input_arguments.attention_dtype.lower() in [
            "fp8",
            "float8",
            "float8_e4m3fn",
        ]:
            ...
            raise ValueError(
                f"Unsupported attention_dtype: {input_arguments.attention_dtype}, only support ['bfloat16','float8_e4m3fn']"
            )
    ...
    if not input_arguments.get("padding_length", None):
        ...
        engine_config.Basic_Config.padding_length = (
            input_arguments.padding_length
        )

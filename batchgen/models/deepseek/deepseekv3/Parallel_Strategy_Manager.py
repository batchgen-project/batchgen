...
class DeepseekV3ParallelStrategyManager:
    ...
    def _init_decoding_padding_bsz(self, padding_bsz):
        ...
        if env_max_bsz is not None:
            ...
            if self.rank == 0:
                logging.info(
                    f"[DECODE] Padding batch size: {max_rank_bsz} (from BATCHGEN_MAX_RANK_BSZ)"
                )
        else:
    ...
    def _init_ata_comms(self, padding_bsz):
        ...
        self.expert_x = torch.empty(
            (
                ...
                self.num_tokens_per_rank * num_dp,
                hidden_size,
            ),
            dtype=in_type,
            device=self.device,
        ...
        self.expert_x_scale = torch.empty(
            (
                ...
                (self.expert_x.size(2) + block_size - 1) // block_size,
            ),
            dtype=torch.float32,
            device=self.device,
        )
        ...
        self.dp_x_scale = torch.empty(
            (
                self.dp_x.size(0),
                (self.dp_x.size(1) + block_size - 1) // block_size,
            ),
            dtype=torch.float32,
            device=self.device,
        )
    ...
    def _init_mode_decoding(self):
        ...
        if self.enable_ep_offloading:
            if self.rank == 0:
                logging.info(
                    "EP offloading mode: skipping grouped GEMM init (using loop-based execution)"
                )
            return
    ...
    def _load_attn_module(self):
        for layer_idx in range(len(self.model.model.layers)):
            ...
            attn_module.kv_a_proj_with_mqa.weight.data = tensors[
            ...
            attn_module.q_a_layernorm.weight.data = tensors[
                "q_a_layernorm.weight"
            ].to(self.engine_config.Basic_Config.device_torch)
            attn_module.kv_a_layernorm.weight.data = tensors[
    ...
    def _load_shared_expert_module(self):
        for layer_idx in range(
            ...
            shared_expert.gate_proj.weight.data = tensors[
                "gate_proj.weight"
            ].to(self.engine_config.Basic_Config.device_torch)
            shared_expert.up_proj.weight.data = tensors["up_proj.weight"].to(
                self.engine_config.Basic_Config.device_torch
            )
            shared_expert.down_proj.weight.data = tensors[
                "down_proj.weight"
            ].to(self.engine_config.Basic_Config.device_torch)
    ...
    def _config_attn_module(self):
        ...
        for layer_idx in range(len(self.model.model.layers)):
            ...
            if self.engine_config.Basic_Config.gpu_arch in (
                ...
                from ....attention.mla.fa2_backend import (
                    mla_prefill_flashattention2,
                    mla_chunked_prefill_flashattention2,
                )
                from ....attention.mla.torch_backend import (
                    mla_decoding_torch,
                    mla_chunked_prefill_torch,
                )
            ...
            if persistent:
                ...
                for (
                    key,
                    value,
                ) in attn_wrapper_instance.weight_dequant_scale.items():
                    value = value.to(
                        self.engine_config.Basic_Config.device_torch
                    )
    ...
    def _load_local_routed_experts(self):
        for routed_expert_idx in self.local_routed_experts:
            ...
            self.model.model.layers[layer_idx].mlp.experts[
                expert_idx
            ].gate_proj.weight.data = tensors["gate_proj.weight"].to(
                self.engine_config.Basic_Config.device_torch
            )
            self.model.model.layers[layer_idx].mlp.experts[
                expert_idx
            ].up_proj.weight.data = tensors["up_proj.weight"].to(
                self.engine_config.Basic_Config.device_torch
            )
            self.model.model.layers[layer_idx].mlp.experts[
                expert_idx
            ].down_proj.weight.data = tensors["down_proj.weight"].to(
                self.engine_config.Basic_Config.device_torch
            )
            # del tensors
    ...
    def _load_model_skeleton(self):
        ...
        if self.rank == 0:
            logging.info(
                f"Model skeleton size: {model_skeletion_byte_size:.2f} GB"
            )
        ...
        for layer_idx in range(
            ...
            for expert_idx in range(len(layer.mlp.experts)):
                ...
                if persistent:
                    ...
                    for key, value in layer.mlp.experts[
                        expert_idx
                    ].weight_dequant_scale.items():
                        value = value.to(
                            self.engine_config.Basic_Config.device_torch
                        )
    ...
    def _config_expert_module(self):
        ...
        for layer_idx in range(
            ...
            for name in mlp_names:
                ...
                if key in self.skeleton_state_dict:
                    weight_dequant_scales[name + postfix] = (
            ...
            if persistent:
                ...
                for (
                    key,
                    value,
                ) in layer.mlp.shared_experts.weight_dequant_scale.items():
                    value = value.to(
                        self.engine_config.Basic_Config.device_torch
                    )

...
std::vector<torch::Tensor> add_rmsnorm_forward(
    torch::Tensor residual, torch::Tensor hidden,
    torch::Tensor weight, double eps);
'''


def _load_rmsnorm_module():
    """Build and cache the CUDA RMSNorm module."""
    global _rmsnorm_module
    if _rmsnorm_module is not None:
        return _rmsnorm_module

    logging.info("[cuda_rmsnorm] Building CUDA kernels...")
    os.environ.setdefault("MAX_JOBS", "8")
    os.environ.setdefault("TORCH_CUDA_ARCH_LIST", "9.0a")

    _rmsnorm_module = load_inline(
        name="cuda_rmsnorm",
        cpp_sources=[CPP_SOURCE],
        cuda_sources=[CUDA_SOURCE],
        functions=["rmsnorm_forward", "add_rmsnorm_forward"],
        extra_cuda_cflags=[
            "-O3", "-arch=sm_90a", "--ptxas-options=-v", "-lineinfo",
        ],
        verbose=False,
    )
    logging.info("[cuda_rmsnorm] Build complete.")
    return _rmsnorm_module

...
class ParameterServer:
    ...
    def create_skeleton_state_dict_shared_memory(self, skeleton_state_dict):
        ...
        try:
            ...
            logging.info(
                f"Serialized skeleton state dict size: {serialized_size} bytes"
            )
            ...
            logging.info(
                f"Writing skeleton state dict to temp file: {file_path}"
            )
            ...
            logging.info(
                f"Successfully wrote state dict to temp file, size: {actual_size} bytes"
            )
    ...
    def create_skeleton_state_dict_shared_memory_dep(self, skeleton_state_dict):
        ...
        if self.skeleton_state_dict_shm_name:
            try:
                ...
                shm = shared_memory.SharedMemory(
                    name=self.skeleton_state_dict_shm_name
                )
                shm.close()
                shm.unlink()
                self.skeleton_state_dict_shm_name = None
        ...
        try:
            ...
            logging.info(
                f"Backup file written successfully, size: {os.path.getsize(backup_file)} bytes"
            )
            ...
            logging.info(
                f"Creating shared memory segment '{shm_name}' with size {buffer_size} bytes"
            )
            shm = shared_memory.SharedMemory(
                create=True, size=buffer_size, name=shm_name
            )
            ...
            if verification_size != serialized_size:
                logging.error(
                    f"Size verification failed! Expected {serialized_size}, got {verification_size}"
                )
            else:
            ...
            logging.info(
                f"Successfully created shared memory segment '{shm_name}'"
            )
            return shm_name
    ...
    def config_hugepages(self, model_name: str = None):
        ...
        if model_name is not None:
            ...
            logging.warning(
                "No model_name provided, using default 350000 hugepages"
            )
        ...
        try:
            ...
            logging.warning(
                f"Failed to use hugepages, falling back to regular shared memory"
            )
        ...
        if self.initial_model_name:
            ...
            try:
                ...
                if result["status"] == "success":
                    logging.info(
                        f"Model {self.initial_model_name} loaded successfully in {end_time - start_time:.2f} seconds"
                    )
                    logging.info(
                        f"Model loaded with shared memory name: {self.model_info.get('shm_name')}"
                    )
                    logging.info(
                        f"Parameter server size: {self.model_info.get('parameter_server_size')} bytes"
                    )
                    ...
                    self.running = True
                    logging.info(
                        f"Parameter server is now listening on {self.host}:{self.port}"
                    )
                    end_time = time.time()
                    logging.info(
                        f"Server started in {end_time - start_time:.2f} seconds"
                    )
                        ...
                        while self.running:
                            try:
                                ...
                                if (
                                    ...
                                    logging.error(
                                        f"Error accepting client connection: {e}"
                                    )
                    finally:
                        self.cleanup()
                else:
                    self.cleanup()
                    raise RuntimeError(
                        f"Failed to preload model: {result.get('message', 'Unknown error')}"
                    )
            except Exception as e:
                logging.error(
                    f"Failed to preload model {self.initial_model_name}: {e}"
                )
    ...
    def cleanup(self):
        ...
        if hasattr(self, "_current_shm") and self._current_shm:
            try:
                logging.info(
                    f"Cleaning up current shared memory {self.skeleton_state_dict_shm_name}"
                )
        ...
        if hasattr(self, "_old_shm_segments"):
            for i, old_shm in enumerate(self._old_shm_segments):
                try:
                    logging.info(
                        f"Cleaning up old shared memory segment {i + 1}/{len(self._old_shm_segments)}"
                    )
                    old_shm.close()
                    old_shm.unlink()
                except Exception as e:
                    logging.error(
                        f"Error cleaning up old shared memory segment: {e}"
                    )
        ...
        try:
            ...
            logging.info(
                f"Running command to clean up hugepages: {' '.join(command)}"
            )
            result = subprocess.run(
            ...
            logging.warning(f"Command error: {e.stderr.strip()}")
            logging.warning(
                "Failed to clean up hugepages, you may need to manually clear by `sysctl -w vm.nr_hugepages=0`"
            )
    ...
    def handle_shutdown(self, signum, frame):
        ...
        if self.model_info.get("shm_name"):
            try:
                ...
                if os.path.exists(shm_path):
                    ...
                    logging.info(
                        f"Shared memory file {shm_path} already cleaned up"
                    )
            except Exception as e:
                logging.warning(
                    f"Shared memory cleanup not properly, you may need to manually clear by `rm -f /dev/hugepages/{shm_path}`: {e}"
                )
        ...
        if self.enable_hugetlbfs:
            ...
            try:
                ...
                logging.warning(
                    f"Unexpected error cleaning up hugepages via sysctl: {e}"
                )
            ...
            while self.running:
                try:
                    ...
                    if not size_data or len(size_data) < 4:
                        logging.info(
                            f"Client {address} closed connection (no size data)"
                        )
                    ...
                    logging.debug(
                        f"Received message size: {msg_size} bytes from {address}"
                    )

                    # Check for unreasonably large message size
                    if (
                        msg_size > 100 * 1024 * 1024
                    ):  # Limit to 100MB for incoming requests
                        logging.warning(
                            f"Rejecting oversized message from {address}: {msg_size} bytes"
                        )
                        break
                    ...
                    while remaining > 0:
                        ...
                        if not chunk:
                            logging.warning(
                                f"Connection closed by {address} during data transfer"
                            )
                            break  # Connection closed
                        data += chunk
                        remaining -= len(chunk)
                    ...
                    if len(data) < msg_size:
                        logging.warning(
                            f"Incomplete message received from {address}: got {len(data)}/{msg_size} bytes"
                        )
                        break
                    ...
                    logging.info(
                        f"Received request from {address}: {request.get('command', 'unknown')}"
                    )
                    ...
                    try:
                        ...
                        if response_size > 10 * 1024 * 1024:  # 10MB
                            logging.info(
                                f"Sending large response to {address}: {response_size} bytes"
                            )

                        # Ensure response size fits within 32-bit unsigned int
                        if response_size > 0xFFFFFFFF:  # 2^32 - 1
                            logging.error(
                                f"Response size {response_size} exceeds maximum size"
                            )
                            error_response = {
                                "status": "error",
                        ...
                        logging.debug(
                            f"Sent complete response of {response_size} bytes to {address}"
                        )

                    except Exception as e:
                        logging.error(
                            f"Error sending response to {address}: {e}"
                        )
                        break
                    ...
                    logging.error(
                        f"Unexpected error with client {address}: {e}"
                    )
                    break
    ...
    def process_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command == "ping":
            ...
            return {
                "status": "success",
                "shm_name": self.model_info.get("shm_name"),
                "tensor_meta_shm_name": self.model_info.get(
                    "tensor_meta_shm_name"
                ),
                "parameter_server_size": self.model_info.get(
                    "parameter_server_size"
                ),
                "huggingface_ckpt_name": self.model_info.get(
                    "huggingface_ckpt_name"
                ),
                "converted_ckpt_dir": self.model_info.get("converted_ckpt_dir"),
                "skeleton_state_dict_file": getattr(
                    self, "skeleton_state_dict_file", None
                ),
    ...
    def process_request_dep(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command == "ping":
            ...
            return {
                ...
                "converted_ckpt_dir": self.model_info.get("converted_ckpt_dir"),
                "skeleton_state_dict_shm_name": self.skeleton_state_dict_shm_name,
                "skeleton_state_dict_backup_file": getattr(
                    self, "skeleton_state_dict_backup_file", None
                ),
            }
    ...
    def handle_load_model(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.current_model == huggingface_ckpt_name:
            logging.info(
                f"Model {huggingface_ckpt_name} already loaded, reusing"
            )
        ...
        if converted_ckpt_dir is None:
            ...
            # V4 conversion commands used by the Blackwell benchmark suite pass the
            # final mpN directory directly (e.g. .../v4flash_converted_mp2).  Older
            # parameter-server flows passed a parent directory and expected the model
            # name to be appended.  Preserve both conventions.
            if (
                not any(
                    name.startswith("model0-mp")
                    for name in os.listdir(converted_ckpt_dir)
                )
                if os.path.isdir(converted_ckpt_dir)
                else True
            ):
                converted_ckpt_dir = os.path.join(
                    converted_ckpt_dir, huggingface_ckpt_name
                )
            if not os.path.exists(converted_ckpt_dir):
                os.makedirs(converted_ckpt_dir)
        ...
        try:
            ...
            elif "deepseek" in huggingface_ckpt_name:
            ...
            shm_name, tensor_meta_shm_name = (
                self.parameter_server_instance.Init()
            )
            ...
            parameter_server_size = (
                self.parameter_server_instance.parameter_server.byte_size()
            )
            logging.info(f"Parameter Server Size: {parameter_server_size}")

            # Create shared memory for the skeleton state dict
            skeleton_state_dict_shm_name = (
                self.create_skeleton_state_dict_shared_memory(
                    skeleton_state_dict
                )
            )
            if not skeleton_state_dict_shm_name:
                error_msg = (
                    "Failed to create shared memory for skeleton state dict"
                )
                logging.error(error_msg)
                return {"status": "error", "message": error_msg}
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Standalone Parameter Server for BatchGen"
    )
    parser.add_argument(
        "--host",
        type=str,
    ...
    parser.add_argument(
        "--port", type=int, default=10900, help="Port to listen on"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--pt-ckpt-dir",
        "--converted-ckpt-dir",
        dest="converted_ckpt_dir",
        type=str,
        default=None,
        help="Directory for converted checkpoints",
    )
    parser.add_argument(
        "--enable-hugetlbfs",
        action="store_true",
...
if __name__ == "__main__":
    ...
    logging.info(
        f"Enable hugetlbfs: {os.environ.get('BATCHGEN_ENABLE_HUGETLBFS', '0')}"
    )

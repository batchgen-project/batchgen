...
def validate_server_host(value):
    ...
    try:
        ...
        try:
            ...
            logging.warning(
                f"Could not resolve host: {value}. Server might not be reachable."
            )
...
def validate_server_port(value):
    ...
    if ivalue < 1024 or ivalue > 65535:
        raise argparse.ArgumentTypeError(
            f"Port should be between 1024 and 65535, got {ivalue}"
        )
    return ivalue
...
def server_argparser():
    parser = argparse.ArgumentParser(
        description="Server Configuration Arguments"
    )
    parser.add_argument(
        "--model_name",
        type=str,

...
def validate_cute_fused_gemm_v4(
    hidden_3d: torch.Tensor,
    weights: List[torch.Tensor],
    scales: List[torch.Tensor],
    N: int,
    ...
    """Validate CuTe fused MXFP4 GEMM V4 (double-buffered pipelined) produces correct output.

    Compares CuTe fused GEMM V4 output against reference (PyTorch dequant + matmul).

    Returns True if validation passes, False otherwise.
    """
    try:
        from batchgen.moe.cute_fused_mxfp4_gemm_v4 import cute_grouped_mxfp4_gemm_3d_v4
        from batchgen.moe.mxfp4_grouped_gemm import setup_expert_weight_pointers
        from batchgen.quantization.mxfp4 import mxfp4_dequantize
    except ImportError as e:
        print(f"  Cannot import CuTe fused GEMM V4: {e}")
        return False
    ...
    print(f"\n{'='*70}")
    print("CUTE FUSED MXFP4 GEMM V4 VALIDATION (Double-Buffered Pipelined)")
    print(f"{'='*70}")
    print(f"Checking {min(num_experts_to_check, num_experts)} experts against PyTorch reference")
    print(f"Config: {tokens_per_expert} tokens/expert, N={N}, K={K}")
    print(f"Tolerance: {tolerance*100:.1f}% relative error")
    ...
    print("\nValidating cute_fused_v4...")
    try:
        # Run kernel
        output = cute_grouped_mxfp4_gemm_3d_v4(
            hidden_3d, weight_ptrs, scale_ptrs, expert_counts,
            N, weights[0], scales[0]
        )
        torch.cuda.synchronize()
        ...
        # Print summary
        print(f"\n{'='*70}")
        print("CUTE FUSED GEMM V4 VALIDATION SUMMARY")
        print(f"{'='*70}")
        print(f"{'Version':<25} {'Status':<10} {'Avg Rel Error':<15}")
        print("-" * 70)
        print(f"{'cute_fused_v4':<25} {status:<10} {avg_rel_error:.6f} ({avg_rel_error*100:.4f}%)")
        print(f"{'='*70}")

        if all_pass:
            print("CUTE FUSED GEMM V4 PASSED VALIDATION")
        else:
            print("CUTE FUSED GEMM V4 FAILED VALIDATION")
...
def benchmark_cute_fused_mxfp4_gemm_v4(
    hidden_3d: torch.Tensor,
    weights: List[torch.Tensor],
    scales: List[torch.Tensor],
    N: int,
    ...
    """Benchmark CuTe fused MXFP4 grouped GEMM V4 (double-buffered pipelined).

    V4 improvements over V3:
    - Double-buffered shared memory for ping-pong pipelining
    - 1 sync per K-iteration (down from 3 in V3)
    - Prefetch k=0 before main K-loop
    ...
    try:
        ...
        print(f"  CuTe fused MXFP4 GEMM V4 not available: {e}")
        return float('inf')
    ...
    try:
        # Warmup
        for _ in range(warmup_iters):
            _ = cute_grouped_mxfp4_gemm_3d_v4(
                hidden_3d, weight_ptrs, scale_ptrs, expert_counts,
                N, weights[0], scales[0]
            )
        torch.cuda.synchronize()
        ...
        print(f"  CuTe fused MXFP4 GEMM V4 failed: {e}")
        import traceback
        traceback.print_exc()
        return float('inf')


def benchmark_cute_fused_mxfp4_gemm_v5(
    hidden_3d: torch.Tensor,
    weights: List[torch.Tensor],
    scales: List[torch.Tensor],
    N: int,
    ...
    """Benchmark CuTe fused MXFP4 grouped GEMM V5 (fused load+decode)."""
    try:
        from batchgen.moe.cute_fused_mxfp4_gemm_v5 import cute_grouped_mxfp4_gemm_3d_v5
        from batchgen.moe.mxfp4_grouped_gemm import setup_expert_weight_pointers
    except ImportError as e:
        print(f"  CuTe fused MXFP4 GEMM V5 not available: {e}")
        return float('inf')
    ...
    try:
        for _ in range(warmup_iters):
            _ = cute_grouped_mxfp4_gemm_3d_v5(
                hidden_3d, weight_ptrs, scale_ptrs, expert_counts,
                N, weights[0], scales[0]
            )
        torch.cuda.synchronize()
        ...
        print(f"  CuTe fused MXFP4 GEMM V5 failed: {e}")
        import traceback
        traceback.print_exc()
        return float('inf')


def benchmark_cute_fused_mxfp4_gemm_v6(
    hidden_3d: torch.Tensor,
    weights: List[torch.Tensor],
    scales: List[torch.Tensor],
    N: int,
    ...
    """Benchmark CuTe fused MXFP4 grouped GEMM V6 (simple CTAs, no double buffering)."""
    try:
        from batchgen.moe.cute_fused_mxfp4_gemm_v6 import cute_grouped_mxfp4_gemm_3d_v6
        from batchgen.moe.mxfp4_grouped_gemm import setup_expert_weight_pointers
    except ImportError as e:
        print(f"  CuTe fused MXFP4 GEMM V6 not available: {e}")
        return float('inf')
    ...
    try:
        ...
        print(f"  CuTe fused MXFP4 GEMM V6 failed: {e}")
        import traceback
        traceback.print_exc()
        return float('inf')


def benchmark_cute_fused_mxfp4_gemm_v7(
    hidden_3d: torch.Tensor,
    weights: List[torch.Tensor],
    scales: List[torch.Tensor],
    N: int,
    ...
    """Benchmark CuTe fused MXFP4 grouped GEMM V7 (warp specialization + persistent CTA)."""
    try:
        from batchgen.moe.cute_fused_mxfp4_gemm_v7 import cute_grouped_mxfp4_gemm_3d_v7
        from batchgen.moe.mxfp4_grouped_gemm import setup_expert_weight_pointers
    except ImportError as e:
        print(f"  CuTe fused MXFP4 GEMM V7 not available: {e}")
        return float('inf')
    ...
    try:
        for _ in range(warmup_iters):
            _ = cute_grouped_mxfp4_gemm_3d_v7(
                hidden_3d, weight_ptrs, scale_ptrs, expert_counts,
                N, weights[0], scales[0]
            )
        torch.cuda.synchronize()
        ...
        print(f"  CuTe fused MXFP4 GEMM V7 failed: {e}")
        import traceback
        traceback.print_exc()
        return float('inf')
...
"""
    ...
    mode_group.add_argument(
        "--compare-cute-fused-v4", action="store_true",
        help="CuTe fused MXFP4 GEMM V4 (double-buffered pipelined) vs Triton comparison"
    )
    mode_group.add_argument(
        "--compare-cute-fused-v567", action="store_true",
        help="CuTe fused MXFP4 GEMM V5/V6/V7 comparison (fused load, simple CTAs, warp specialization)"
    )
    ...
    if not any([args.quick, args.compare_all, args.compare_fp4, args.tune_gemm, args.validate, args.compare_cuda, args.compare_cute_fused, args.compare_cute_fused_v2, args.compare_cute_fused_v4, args.compare_cute_fused_v567]):
        args.quick = True
    ...
    # CuTe fused GEMM V4 comparison mode (double-buffered pipelined)
    if args.compare_cute_fused_v4:
        print(f"\n{'='*70}")
        print("CUTE FUSED MXFP4 GEMM V4 vs TRITON COMPARISON")
        print("(Double-Buffered Pipelined Architecture)")
        print(f"{'='*70}")
        print(f"GPU: {torch.cuda.get_device_name(0)}")
        print(f"Config: {config['num_experts']} experts")
        print(f"Dimensions: K={config['hidden_size']}, N={config['intermediate_size']}")
        ...
        for tpe in args.tokens:
            ...
            # Validate CuTe fused GEMM V4 first (unless skipped)
            if not args.skip_validation:
                print("\nRunning CuTe fused GEMM V4 validation...")
                validation_pass = validate_cute_fused_gemm_v4(
                    hidden_3d, weights, scales, N, "cuda"
                )
                if not validation_pass:
                    print("\nWARNING: CuTe fused GEMM V4 validation FAILED!")
                    print("Continuing with benchmark, but results may be incorrect...")
                else:
                    print("\nCuTe fused GEMM V4 validation passed.\n")

            # Benchmark Triton fused GEMM
            print("\nBenchmarking Triton fused MXFP4 GEMM...")
            triton_time = benchmark_fused_mxfp4_gemm(hidden_3d, weights, scales, N)

            # Benchmark CuTe fused GEMM V3 (for comparison)
            print("Benchmarking CuTe fused MXFP4 GEMM V3 (routed-only, FP32 accum)...")
            cute_v3_time = benchmark_cute_fused_mxfp4_gemm_v2(
                hidden_3d, weights, scales, N
            )

            # Benchmark CuTe fused GEMM V4 (double-buffered pipelined)
            print("Benchmarking CuTe fused MXFP4 GEMM V4 (double-buffered pipelined)...")
            cute_v4_time = benchmark_cute_fused_mxfp4_gemm_v4(
                hidden_3d, weights, scales, N
            )
            ...
            print(f"RESULTS (tokens_per_expert={tpe})")
            print(f"{'='*70}")
            print(f"{'Kernel':<45} {'Time (ms)':>12} {'vs Triton':>12}")
            print("-" * 70)

            if triton_time != float('inf'):
                print(f"{'Triton fused MXFP4 GEMM':<45} {triton_time:>12.3f} {'(baseline)':>12}")
            else:
                print(f"{'Triton fused MXFP4 GEMM':<45} {'FAILED':>12}")

            if cute_v3_time != float('inf') and triton_time != float('inf'):
                speedup = triton_time / cute_v3_time
                print(f"{'CuTe fused MXFP4 V3 (routed+FP32 accum)':<45} {cute_v3_time:>12.3f} {speedup:>11.2f}x")
            elif cute_v3_time != float('inf'):
                print(f"{'CuTe fused MXFP4 V3 (routed+FP32 accum)':<45} {cute_v3_time:>12.3f}")
            else:
                print(f"{'CuTe fused MXFP4 V3 (routed+FP32 accum)':<45} {'FAILED':>12}")

            if cute_v4_time != float('inf') and triton_time != float('inf'):
                speedup = triton_time / cute_v4_time
                v3_v4_speedup = cute_v3_time / cute_v4_time if cute_v3_time != float('inf') else 0
                print(f"{'CuTe fused MXFP4 V4 (double-buffered)':<45} {cute_v4_time:>12.3f} {speedup:>11.2f}x")
                if cute_v3_time != float('inf'):
                    print(f"  V4 vs V3 speedup: {v3_v4_speedup:.2f}x")
            elif cute_v4_time != float('inf'):
                print(f"{'CuTe fused MXFP4 V4 (double-buffered)':<45} {cute_v4_time:>12.3f}")
            else:
                print(f"{'CuTe fused MXFP4 V4 (double-buffered)':<45} {'FAILED':>12}")

            print(f"{'='*70}")

            results[tpe] = {
                "triton": triton_time,
                "cute_v3": cute_v3_time,
                "cute_v4": cute_v4_time,
            }

        sys.exit(0)

    # CuTe fused GEMM V5/V6/V7 comparison mode
    if args.compare_cute_fused_v567:
        print(f"\n{'='*70}")
        print("CUTE FUSED MXFP4 GEMM V5/V6/V7 COMPARISON")
        print("V5: Fused load+decode (no smem_packed, 1 sync/K-iter)")
        print("V6: Simple CTAs, no double buffering (let hardware overlap)")
        print("V7: Warp specialization + persistent CTA (producer/consumer)")
        print(f"{'='*70}")
        print(f"GPU: {torch.cuda.get_device_name(0)}")
        print(f"Config: {config['num_experts']} experts")
        print(f"Dimensions: K={config['hidden_size']}, N={config['intermediate_size']}")
        ...
        for tpe in args.tokens:
            ...
            # Benchmark CuTe V5 (fused load+decode)
            print("Benchmarking CuTe V5 (fused load+decode)...")
            cute_v5_time = benchmark_cute_fused_mxfp4_gemm_v5(
                hidden_3d, weights, scales, N
            )

            # Benchmark CuTe V6 (simple CTAs)
            print("Benchmarking CuTe V6 (simple CTAs, no double buffering)...")
            cute_v6_time = benchmark_cute_fused_mxfp4_gemm_v6(
                hidden_3d, weights, scales, N
            )

            # Benchmark CuTe V7 (warp specialization + persistent)
            print("Benchmarking CuTe V7 (warp specialization + persistent CTA)...")
            cute_v7_time = benchmark_cute_fused_mxfp4_gemm_v7(
                hidden_3d, weights, scales, N
            )
            ...
            print(f"RESULTS (tokens_per_expert={tpe})")
            print(f"{'='*70}")
            print(f"{'Kernel':<50} {'Time (ms)':>12} {'vs Triton':>12}")
            print("-" * 74)

            if triton_time != float('inf'):
                print(f"{'Triton fused MXFP4 GEMM':<50} {triton_time:>12.3f} {'(baseline)':>12}")
            else:
                print(f"{'Triton fused MXFP4 GEMM':<50} {'FAILED':>12}")

            kernel_results = [
                ("CuTe V5 (fused load+decode)", cute_v5_time),
                ("CuTe V6 (simple CTAs, no double buf)", cute_v6_time),
                ("CuTe V7 (warp specialization + persistent)", cute_v7_time),
            ]

            best_cute_time = float('inf')
            best_cute_name = None

            for name, ktime in kernel_results:
                if ktime != float('inf') and triton_time != float('inf'):
                    speedup = triton_time / ktime
                    print(f"{name:<50} {ktime:>12.3f} {speedup:>11.2f}x")
                    if ktime < best_cute_time:
                        best_cute_time = ktime
                        best_cute_name = name
                elif ktime != float('inf'):
                    print(f"{name:<50} {ktime:>12.3f}")
                    if ktime < best_cute_time:
                        best_cute_time = ktime
                        best_cute_name = name
                else:
                    print(f"{name:<50} {'FAILED':>12}")

            print(f"{'='*74}")
            if best_cute_name and triton_time != float('inf'):
                speedup = triton_time / best_cute_time
                print(f"BEST CuTe: {best_cute_name} at {best_cute_time:.3f}ms ({speedup:.2f}x vs Triton)")
            print(f"{'='*74}")

            results[tpe] = {
                "triton": triton_time,
                "cute_v5": cute_v5_time,
                "cute_v6": cute_v6_time,
                "cute_v7": cute_v7_time,
            }
